...
Fork-only content from the 2026-08-12 upstream sync: real, shipped in-tree functionality whose `changelog.d/` fragments this sync's own release-rollup commits consumed, but which no already-tagged release section above documents (two fork-local rollups predate and are superseded by the officially tagged `[0.36.0] - 2026-08-11` below: one dated 2026-08-08 whose daemon-status/worktree-list/reclaim entries are dropped here as exact duplicates of `[0.35.10]`'s, and one dated 2026-08-10 whose "orchestration tab labels are colored" entry is dropped as a duplicate of `[0.35.9]`'s). Needs a real version/date from whoever cuts the next release.
