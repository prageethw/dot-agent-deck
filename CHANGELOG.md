...
Fork-only content from the 2026-08-12 upstream sync: real, shipped in-tree functionality whose `changelog.d/` fragments this sync's own release-rollup commit consumed, but which no already-tagged release section above documents (the fork's own local rollup that originally captured it, dated 2026-08-08, predates and is superseded by the officially tagged `[0.36.0] - 2026-08-11` below — the daemon-status and worktree-list/reclaim entries that rollup also carried are dropped here as exact duplicates of `[0.35.10]`'s). Needs a real version/date from whoever cuts the next release.
