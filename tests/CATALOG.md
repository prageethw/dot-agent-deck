...
##### orchestration/seed/013 — TEXT confirmation is structurally dead for a genuine resubmission of byte-identical prompt text — the second-or-later delivery cycle #423's compaction re-assertion produces (fork #187). Expected RED.
...
##### orchestration/seed/014 — A confirmation-retry after a write that already LANDED must send a BARE submit (empty payload), never the full prompt text a second time into a composer that already holds it (fork #194, decided mechanism submit-only, fork#197 M4). Expected RED.
