...
##### dashboard/pane/004 — Card title row carries card number and a status badge, with the display name on its own body row directly beneath.
...
##### dashboard/pane/012 — The role name renders on its own first body row, in `palette::ROLE_NAME`, unconditionally — never on the title row, and never gated by the agent-type badge toggle (PRD fork#405 M1).
- **Layer:** L1 (ratatui `TestBackend` + color-aware `insta` snapshot).
- **Agent:** none (a synthetic ClaudeCode/Opus fixture with a friendly display name, plus a `display_name: None` variant for the `id_display` fallback and a both-empty (`display_name: None` + empty `session_id`) variant for the height guarantee).
- **Asserts:** with the agent-type badge toggle ON, the title row carries the `<type> (<model>)` badge and NOT the display name, while the first body row carries the display name styled `palette::ROLE_NAME` (`Color::Indexed(130)`). With the toggle OFF (the default), the title row carries neither the badge nor the name, while the first body row STILL carries the name in the same colour — the unconditional half, and the one most likely to regress. Also asserts `palette::ROLE_NAME` is distinct from every status role colour and every registry `badge_color`, that a card with `display_name: None` falls back to `id_display` on the same row, and that a card with BOTH empty renders a blank identity row (`Line::from("")`) that still occupies exactly one row — the `Dir:` row lands on row 2 either way.
- **Does not assert:** the model/badge segment's own content or toggle wiring (covered by `dashboard/agent-badge/001`/`004`); the embedded terminal pane, which keeps its single-line role title and is out of scope for this PRD; `card_height()`'s exact per-density values (covered by `card_height_001_content_derived_values` in `src/ui.rs`). This catalog ID was originally assigned `dashboard/pane/011` by the commit's own RED round; renumbered to `012` when re-curated onto `fork-only` (sync-421819) because `011` was already claimed there by the multi-byte-`session_id` test above. The idle-art overlay test this PRD's delta-review (F2) originally added as `dashboard/pane/012` was dropped in the same re-curation: `fork-only`'s upstream rebase had already removed the idle-art feature (`src/ascii_art.rs`, the `[idle_art]` config section, `IdleArtPhase`/`IdleArtEntry`/`AsciiArtResult`) by the time this commit was folded in, so the overlay F2 fix and its test no longer apply to anything in the tree.
- **Platform coverage:** mac+linux+windows.
...
##### dashboard/rename/002 — Confirming a valid new name updates the card's identity and is mirrored via the daemon `SetAgentLabel` request.
- **Layer:** L2.
- **Agent:** none.
- **Asserts:** the card shows the new name (PRD fork#405 M1 moved identity off the title row onto its own body row); a subsequent `list_agents` from a parallel daemon client returns the same `display_name`.
...
##### dashboard/agent-badge/005 — The identity row has its own width budget and cannot be starved by title content, however long an unbounded model badge grows (PRD fork#378 reviewer/auditor round 2, MEDIUM 3 / F1+F2; premise rewritten by PRD fork#405 M1).
...
- **Asserts:** the raw tail marker of the 300-char model does not survive to the rendered card title (proving truncation happened, without pinning the exact cap value — a coder decision agreed at ≈40 chars); the title row carries no identity at all (PRD fork#405 M1), and the full session id is present on the card's first body row regardless of how long the model badge is — structural immunity, not a survived truncation race, since identity is no longer sharing a budget with the badge in the first place. Because every character in the fixture is 3 bytes wide, a truncation implemented as a byte-index slice rather than a char-boundary one panics this test outright rather than merely failing an assertion.
...
##### worktree/create/003 — Two concurrent callers of the ASYNC `create_worktree` (the `issue_dispatch` scheduler's own creation path, distinct from the TUI's `create_worktree_sync`), both attaching to the SAME already-existing branch at the SAME target path (fork issue #282's "attach race", left unfixed on this path by PR #331), never both report `Created`, and git's own worktree admin state never ends up registering the path twice.
- **Layer:** fast synthetic direct-call unit test, embedded in `src/issue_dispatch_run.rs`'s own `#[cfg(test)] mod tests` — `create_worktree` is `pub(crate)` async, invisible to an external integration-test crate. Real `git` subprocesses, a real repo in a tempdir, no PTY, no daemon. Written as a sync `#[test]` driving an explicit multi-thread `tokio::runtime::Runtime` (linkage-check ties `#[spec(...)]` to the next plain `fn`, not a `#[tokio::test] async fn`), with `tokio::spawn` + `tokio::sync::Barrier` racing the two callers, matching how the real daemon's multi-thread `#[tokio::main]` schedules them.
- **Agent:** none.
...
- **Asserts:** the fixture's `.dot-agent-deck.toml` is committed first (`commit_fixture`) so tab B's open below has a ref to branch from; opening a real orchestration tab A renders the pane column's left edge at the default 34%-width boundary; since opening a tab leaves the deck in PaneInput mode focused on its start-role pane and PRD #387 M1 claims `Ctrl+l` only in command mode, each toggle press is preceded by `Ctrl+D` to enter Normal mode; `Ctrl+l` then moves that boundary to the narrower-sidebar 25%-width position; opening a SECOND real orchestration tab B in the same directory — via a TYPED Worktree slug rather than accepting the form's blank default, since issue #489 now refuses a second blank-slug open that exactly collides with tab A's own live directory, routing this open instead through the unaffected typed-slug arm (`SiblingScope::ExactCwdOnly` only gates a blank slug) — renders it ALREADY at the deck-global Narrow boundary tab A was just toggled to, not its own untoggled default (PRD #387 decision 2 — a new tab adopts the shared stage); cycling tab B through Narrow then Hidden (edge at column 0, sidebar fully collapsed — and directly asserts neither role's sidebar card marker is present on the grid at all, proving Hidden genuinely renders no sidebar content rather than merely that the pane column's edge reached column 0) and switching back to tab A (Shift+Tab, which needs no extra `Ctrl+D` since the deck is already in Normal mode from the toggle above) confirms tab A is now ALSO Hidden — toggling tab B moved the SAME shared stage tab A reads; finishing tab A's own cycle (Hidden -> Default) restores the boundary and, switching to tab B, confirms it is ALSO back at Default — proving the 3-stage loop and deck-global sharing, in both directions, together.
- **Does not assert:** persistence of the toggled state across restart (explicitly out of scope per the PRD); remapping the chord via config; Dashboard-tab coverage (the Dashboard/Orchestration cross-tab-type sharing case lives in `tabs/dashboard/001`); the blank-slug collision auto-isolation itself, which tab B's open here is careful to avoid rather than exercise (that's `orchestration/worktree/018`'s job); the CONTENT of the worktree tab B's typed-slug open creates (covered by `orchestration/worktree/005`, `worktree/reclaim/022`/`023`).
- **Platform coverage:** mac+linux.
...
##### tabs/orchestration/010 — An ACTIVE orchestration tab renders its status tint as ordinary foreground text, cued as active via BOTH BOLD and REVERSED (no UNDERLINED), with the tint filling its padding and close glyph too, and an inactive Idle orchestration tab renders palette::STATUS_IDLE (PRD #333; issue #306 reverses the earlier no-tint-on-active carve-out; PRD fork#405 M2 partially re-reverses issue #306's removal of REVERSED, scoped to `Tab::Orchestration` only; fork issue #351, maintainer decision 2026-08-15, reverses defect B so Idle is coloured too; fork issue #377 drops the UNDERLINED half of the active cue as a maintainer preference).
- **Layer:** L1 (in-process `TestBackend` render via `render_tab_bar_to_buffer`, `tests/render_tab_strip.rs`).
- **Agent:** none (synthetic `SessionStatus` values, no panes/PTYs).
- **Asserts:** an orchestration tab made the ACTIVE tab with a non-idle (`Error`) pane renders its status `fg` tint (Red) as ordinary foreground text, cued as active via `BOLD` AND `REVERSED`, with no `UNDERLINED` (fork issue #377). PRD fork#405 M2 reinstates `REVERSED` for `Tab::Orchestration` tabs specifically — a scoped, deliberate partial reversal of issue #306, which removed it from the tab bar entirely because stacking a status `fg` on `Modifier::REVERSED` would invert the color into a background at display time; M2 makes that inversion the intended highlight, but only for the orchestration tab kind. Also asserts the padding spaces and the `[×]` close glyph around the label carry the SAME status fg as the label text (the logical cell value, unaffected by REVERSED), so the tint fills the whole tab. Also asserts an INACTIVE orchestration tab whose aggregate status is `Idle` renders `palette::STATUS_IDLE` (`Color::DarkGray`) — fork issue #351, maintainer decision 2026-08-15: colour is now a total function of status, reversing defect B — and that an INACTIVE orchestration tab with a non-idle (`Error`) aggregate status still colors its label text with neither `REVERSED` nor `BOLD` nor `UNDERLINED` — pinning the active cue from both sides so an inactive tab can never become indistinguishable from an active one (reviewer finding F3 on PR #307).
- **Does not assert:** the aggregate-priority resolver (covered by `tabs/orchestration/009`); per-pane sidebar status rendering (covered by `focus/orchestration/002`); pane-column geometry (covered by `orchestration/layout/002`/`004`); the same cue on Dashboard/Mode tabs, which stay REVERSED-free (covered by `tabs/orchestration/012`/`016`, `tabs/label/002`).
- **Platform coverage:** mac+linux+windows.
...
##### tabs/orchestration/012 — An ACTIVE Dashboard tab — the tab that carries no status data — still carries the BOLD-only active cue (no UNDERLINED), no REVERSED, and no absolute foreground color (issue #306; fork issue #377 drops UNDERLINED from the cue; PRD fork#405 M2 scopes the new REVERSED cue to `Tab::Orchestration` only, so the Dashboard tab stays REVERSED-free even after M2).
- **Layer:** L1 (in-process `TestBackend` render via `render_tab_bar_to_buffer`, `tests/render_tab_strip.rs`).
- **Agent:** none (synthetic; no `tab_statuses` entry, no panes/PTYs).
- **Asserts:** a single active Dashboard tab with `tab_statuses = [None]` renders `BOLD` (and explicitly no `UNDERLINED`, fork issue #377) as its active cue, contains no `REVERSED` (the Dashboard tab is never `Tab::Orchestration`, PRD fork#405 M2), and carries `Color::Reset` (no absolute foreground color) — proving the active cue applies uniformly to the Dashboard, the one tab kind that carries no status data (fork issue #351 narrows this from "tabs PRD #333's status-tint feature does not touch" now that Mode tabs also carry status data via `tab_status_data`; see `tabs/label/001`).
- **Does not assert:** any status-tinted tab (covered by `tabs/orchestration/009`/`010`); the Idle colouring on an active orchestration tab (covered by `tabs/orchestration/014`); the REVERSED cue itself (covered by `tabs/orchestration/010`/`016`).
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/014 — An ACTIVE orchestration tab whose aggregate status resolves to Idle renders palette::STATUS_IDLE, while still carrying BOTH the BOLD and REVERSED active cue (no UNDERLINED) (issue #306 extended PRD #333 defect B to the active tab as a no-tint carve-out; fork issue #351, maintainer decision 2026-08-15, reverses that carve-out so Idle is coloured here too; fork issue #377 drops UNDERLINED from the cue; PRD fork#405 M2 adds REVERSED to the orchestration active cue regardless of the underlying status colour, Idle included).
- **Layer:** L1 (in-process `TestBackend` render via `render_tab_bar_to_buffer`, `tests/render_tab_strip.rs`).
- **Agent:** none (synthetic `SessionStatus` values, no panes/PTYs).
- **Asserts:** an orchestration tab made the ACTIVE tab with an all-`Idle` aggregate renders its label in `palette::STATUS_IDLE`, while still carrying `BOLD` and `REVERSED` and no `UNDERLINED` (fork issue #377) as its active cue — the active cue's colour is unchanged, only its modifier, and REVERSED applies regardless of which status colour is under it.
...
##### tabs/orchestration/016 — The active orchestration tab renders REVERSED (in addition to BOLD); Dashboard and Mode tabs keep BOLD only, never REVERSED, whichever of them is active (PRD fork#405 M2).
- **Layer:** L1 (in-process `TestBackend` render via `render_tab_bar_to_buffer`, `tests/render_tab_strip.rs`).
- **Agent:** none (synthetic `SessionStatus` values, no panes/PTYs).
- **Asserts:** driving one three-tab fixture (Dashboard, a Mode tab, an Orchestration tab) with `active_index` moved across all three positions in turn: (1) the orchestration tab UNSELECTED renders neither `REVERSED` nor `BOLD`, fg its ordinary status colour; (2) the SAME orchestration tab SELECTED gains both `BOLD` and `REVERSED`, fg unchanged (REVERSED inverts the colour at render time without changing the logical cell `fg` ratatui stores, so a plain `fg` equality check stays valid either way); (3) the Mode tab SELECTED gains `BOLD` only, never `REVERSED`, fg its own status colour; (4) the Dashboard tab SELECTED gains `BOLD` only, never `REVERSED`, no absolute fg. Cases (3)/(4) are the "do not change worker-tab styling" requirement M2 imposes — only `Tab::Orchestration` gets the new cue. This is a distinct catalog ID from `tabs/orchestration/015` (an unrelated, pre-existing L2 regression guard for issue #313's BOLD/no-UNDERLINED cue) because /015 was already assigned by the time this PRD was scoped.
- **Does not assert:** the padding/close-glyph fg fill (covered on the orchestration case by `tabs/orchestration/010`); the Idle-status active-orchestration case (covered by `tabs/orchestration/014`); the real terminal byte stream (covered on the BOLD/UNDERLINED half by `tabs/orchestration/015`; REVERSED has no L2 counterpart yet).
- **Platform coverage:** mac+linux+windows.
...
##### tabs/label/001 — `tab_status_data`, the pure-data extraction behind the `run_tui` tab-bar builder, keys a Mode tab by its OWN agent pane's status instead of `None` (fork issue #351: clicking a worker tab highlighted it with a neutral underline instead of the worker's status colour, because the inline builder mapped every non-Orchestration tab — Mode included — to `None`). The underline mentioned here was itself dropped as the active-tab cue afterward (fork issue #377, see `tabs/orchestration/010`/`012`/`014`); this entry's narrative is historically accurate as of #351 and is left unchanged.
...
##### tabs/label/002 — An active tab carrying `Some(&[Working])` status data — the shape `tab_status_data` produces for a Mode tab whose agent is Working — renders its label in `palette::STATUS_WORKING`, still cued active via BOLD only with no UNDERLINED and no REVERSED (fork issue #351 regression guard; fork issue #377 drops UNDERLINED from the cue; PRD fork#405 M2 scopes the new REVERSED cue to `Tab::Orchestration` only, so this Mode-tab fixture stays REVERSED-free even after M2).
- **Layer:** L1 (in-process `TestBackend` render via `render_tab_bar_to_buffer`, `tests/render_tab_strip.rs`).
- **Agent:** none (synthetic `SessionStatus` values, no panes/PTYs).
- **Asserts:** a tab made the ACTIVE tab and carrying `Some(&[Working])` renders its label foreground in `palette::STATUS_WORKING` (Green), while still carrying `BOLD` as its active cue and no `UNDERLINED` (fork issue #377) and no `REVERSED` (it is a Mode tab, not `Tab::Orchestration`, PRD fork#405 M2). The colour assignment is a regression guard, GREEN from the start — the render half (`render_tab_strip`) already colors any tab whose `tab_statuses` slot is `Some(..)` regardless of tab kind; this guards that half against regression, exercised directly via `render_tab_bar_to_buffer`.
...
##### error/socket/009 — `request_from_socket` abandons a reply line that exceeds the maximum length, instead of buffering it until the total-operation deadline expires (fork issue #101 item 2, left open by #120).
...
##### error/socket/010 — A large but legitimate reply line (256 KiB seed) is returned whole, so the cap added for `error/socket/009` cannot silently truncate a real seed.
- **Layer:** L1 (`src/hook.rs`'s `#[cfg(test)] mod tests`; same synthetic stub-daemon setup as `error/socket/003`–`009`).
...
##### orchestration/delegate/029 — Two panes reporting `work-done` for the same role name and the same cwd (fork #76's two-orchestrations collision) must not clobber each other's report.
...
##### orchestration/delegate/030 — A report already sitting at the daemon's own work-done output path must survive a subsequent, differently-sourced `work-done` call, and the collision must be announced rather than silent (upstream #331).
...
##### orchestration/delegate/031 — `dot-agent-deck work-done --task-file <path>` must be refused, client-side, when `<path>` resolves inside the daemon's own `.dot-agent-deck/work-done-*.md` output namespace (upstream #331's own proposed fix).
...
##### orchestration/delegate/036 — A Pi start role declared BEFORE a worker role in an orchestration's config can delegate before the worker's daemon-side registration exists, losing its first task with no retry (fork #92 P1, PR #93 pre-merge review).
...
##### orchestration/delegate/037 — Reordering `open_orchestration_tab`'s spawn loop (a Pi start role spawned last, per PR #93's approved fix) must not disturb `role_pane_ids`/`TabMembership.role_index`, which stay keyed by the config's DECLARED role position, not by spawn order (fork #92 P1 follow-up).
...
- **Does not assert:** the reorder mechanism itself or actual spawn call order (that's `orchestration/delegate/036`'s territory, exercised against a real daemon); layout, focus, or persisted-status ordering beyond `role_pane_ids`/`start_role_index`.
- **Platform coverage:** mac+linux.

##### orchestration/delegate/033 — `work-done --task-file` must be refused when the argument is a SYMLINK whose target resolves inside the daemon's own output namespace, even though the argument's own name does not match `work-done-*.md` (PR #90 pre-merge review P1).
- **Layer:** fast real-binary-subprocess integration (same technique as `orchestration/delegate/031` — a real spawned daemon reachable over its hook socket, and the actual `dot-agent-deck` binary run as a subprocess).
- **Agent:** none.
- **Asserts:** with a real, reachable daemon, a real file `work-done-coder.md` sits inside `.dot-agent-deck`, and a symlink at `external-report.md` (a name that does NOT match `work-done-*.md`, and does not live in `.dot-agent-deck`) points at it. `work-done --task-file external-report.md` must exit NON-ZERO — told apart from "daemon unreachable" the same way `031` is. A second, otherwise-identical call with no symlink at all must still exit 0. Fixed: `src/main.rs`'s `is_work_done_output_path` now resolves `task_file` via `resolve_work_done_candidate` (which canonicalizes as much of the path as the filesystem allows, following symlinks) BEFORE classifying it, so a symlink whose own name/location sit outside the namespace is still caught by what it resolves to.
- **Does not assert:** the exact refusal wording (stderr is not inspected — only the exit code); the intermediate-directory-symlink variant (covered by `orchestration/delegate/034`).
- **Platform coverage:** mac+linux (unix-only — spawns a real daemon subprocess; symlinks).

##### orchestration/delegate/034 — `work-done --task-file` must be refused when reached through an INTERMEDIATE DIRECTORY SYMLINK that aliases into `.dot-agent-deck`, even though the argument's own literal parent segment is not named `.dot-agent-deck` (PR #90 pre-merge review P1).
- **Layer:** fast real-binary-subprocess integration (same technique as `orchestration/delegate/031`/`033`).
- **Agent:** none.
- **Asserts:** with a real, reachable daemon, a real file `work-done-coder.md` sits inside `.dot-agent-deck`, and a directory symlink `alias` points AT `.dot-agent-deck` itself. `work-done --task-file alias/work-done-coder.md` must exit NON-ZERO, told apart from "daemon unreachable" the same way `031`/`033` are. A second, otherwise-identical call with no symlink at all must still exit 0. Fixed by the same resolution change as `033`: `resolve_work_done_candidate` canonicalizes the intermediate directory too, so what the literal parent segment ("alias") resolves to is what gets classified, not its own name.
- **Does not assert:** the exact refusal wording; the file-symlink variant (covered by `orchestration/delegate/033`); the cwd-independence of the resolved-parent comparison itself (covered by `orchestration/delegate/035`).
- **Platform coverage:** mac+linux (unix-only — spawns a real daemon subprocess; symlinks).

##### orchestration/delegate/035 — `work-done --task-file` naming the daemon's real work-done output must be refused even when the CLI process's OWN cwd is not the pane's cwd the daemon actually writes under — a regression guard for PR #90's re-review (upstream #331, fork #76).
- **Layer:** fast real-binary-subprocess integration (same technique as `orchestration/delegate/031`/`033`/`034`).
- **Agent:** none.
- **Asserts:** with a real, reachable daemon, a pane cwd holds the real `.dot-agent-deck/work-done-coder.md`, and a CHILD cwd of it (standing in for a worker that `cd`s into a subdirectory before invoking the CLI) carries a second, unrelated `.dot-agent-deck` directory of its own. Invoked FROM the child cwd, `work-done --task-file` naming the pane cwd's real output file — first as an ABSOLUTE path, then as a RELATIVE `../.dot-agent-deck/work-done-coder.md` path — must both exit NON-ZERO, told apart from "daemon unreachable" the same way `031` is. A harmless `work-done-*.md` inside the CHILD's own decoy `.dot-agent-deck` (not the pane's real output at all) is refused too — an accepted false positive, pinned deliberately because a decoy refusal is harmless while a missed real file destroys a report. A plain file outside any `.dot-agent-deck` is still accepted. This is a regression guard, not a RED pin: `031`/`033`/`034` all set the CLI's cwd equal to the namespace under test and so could not expose a cwd-anchored check; a pre-`545df7a` implementation that compared the resolved parent against `current_dir().join(".dot-agent-deck")` would pass the absolute- and relative-path cases here (the child cwd's OWN `.dot-agent-deck` resolves successfully and differs from the pane's), which this test would have caught red — the fixed check in place today compares the resolved parent's directory NAME only, anchored to no cwd at all, so it passes green.
- **Does not assert:** the symlink-defeated variants of the same rule (covered by `orchestration/delegate/033`/`034`); the exact refusal wording (stderr is not inspected — only the exit code).
- **Platform coverage:** mac+linux (unix-only — spawns a real daemon subprocess).
...
- **Asserts:** with an earlier delegation's report already parked at the daemon's own pane-digest-keyed output path (upstream #331 + fork #76 — `work_done_file_name`, computed the same way the daemon computes it, not a hardcoded filename) and nothing delegated, the worker's `work-done` produces feedback carrying the daemon's unsolicited label (`you have no outstanding delegation to that worker`); the happy-path pointer (`Read .dot-agent-deck/<file name> for their full report.`) is ABSENT; the worker's own report still reaches the orchestrator inline, framed as `[UNTRUSTED-WORKER-REPORT: … :END-UNTRUSTED-WORKER-REPORT]`; and the earlier report is still on disk byte-for-byte.
- **Does not assert:** what the orchestrator then DOES with the label (an LLM decision); the commonest sibling trace — a worker redirected by a human mid-delegation, which produces a well-formed completion for a diverged task and involves no defect (#445/#369); the ledger's own arithmetic (unit-tested on `AgentPtyRegistry::retire_delegation_commission`); the collision/archive mechanism itself (`orchestration/delegate/022`/`023`/`025`).
- **Platform coverage:** mac+linux (unix-only — raw-mode shell observer).
...
- **Asserts:** on a project config whose `worker_response_timeout_minutes = 0` leaves BOTH delegation watches arming nothing at all, a delegate followed by the worker's `work-done` still submits the unchanged pointer into the orchestrator pane, carries no unsolicited label and no inlined report frame, and writes the fresh report to the daemon's pane-digest-keyed output path. This is the case that rules out inferring "never delegated" from `DelegationRetirement::Nothing`: doing so would have silently stopped reporting completions for every project that has turned the detector off.
...
- **Layer:** fast integration (real `handle_delegate` + `handle_work_done` against daemon-owned PTYs; `cat` stand-ins).
- **Agent:** none (raw no-echo `cat` orchestrator observer plus a `cat` worker).
- **Asserts:** with `.dot-agent-deck` occupied by a regular FILE — so `create_dir_all` and the write both fail, for uid 0 as well — a delegated worker's `work-done` produces feedback stating the deck `could not write .dot-agent-deck/<file name>`, with the happy-path pointer ABSENT and the worker's report inlined inside the untrusted-report frame. The daemon still holds the summary in memory at the moment it gives up on the file, so the failure degrades to a worse-formatted report rather than to a confidently wrong one.
- **Does not assert:** the no-cwd and mkdir-only variants of the same failure, or the archive-failure variant (upstream #331 + fork #76) that also routes through this same "Unfiled" wording (all unit-tested on `write_work_done_summary`); the stale-file READ itself (the pointer's absence is what makes it unreachable); recovery of the file on a later delegation.
- **Platform coverage:** mac+linux (unix-only — raw-mode shell observer).
...
##### orchestration/identity/013 — Driving the real binary through TWO orchestration opens in the SAME directory — the first accepting the form's suggested Name with a single Enter (no field typed into), the second typing a Worktree slug instead (issue #489 now refuses a second blank-slug open that exactly collides with the first's own live directory) — lands both as visible tabs with DISTINCT labels (`<basename>-orchestrator-1`, `<basename>-orchestrator-2`) — never the identical basename-derived title recorded twice, the fork #74 collision fork#192 exists to stop (fork#192 M1.0).
...
- **Asserts:** the fixture's `.dot-agent-deck.toml` is committed first (`commit_fixture`) so the second open's Worktree provisioning below has a ref to branch from; opening the fixture's one orchestration from the Dashboard once with a blank slug (`Ctrl+n` → confirm dir → select orchestration → accept the suggested Name unedited → submit) and a second time with a TYPED Worktree slug (`Ctrl+n` → confirm dir → select orchestration → accept the suggested Name → Tab to the Worktree field → type `identityb` → submit), routing the second open through issue #489's unaffected typed-slug arm (`SiblingScope::ExactCwdOnly` only gates a blank slug) instead of being refused as an exact-cwd collision with the still-live first orchestration — the rendered tab strip contains both `" <basename>-orchestrator-1 "` and `" <basename>-orchestrator-2 "` as distinct substrings, where `<basename>` is the real launch directory's basename read from `TuiDeck::workdir()`. The second wait barrier is `wait_for_string(&second_label)` itself (fork#192 review round 2 F7 — the prior `wait_for_string(" worker ")` was vacuous there: the Dashboard already shows " worker " from the FIRST orchestration's panes before the second open even starts, since the fixture's second role is literally named "worker").
- **Does not assert:** the suggestion/refusal MECHANISMS in isolation (covered by `orchestration/identity/002`/`003`); the blank-slug collision auto-isolation itself, which the second open here is careful to avoid rather than exercise (that's `orchestration/worktree/018`'s job); the CONTENT of the worktree the second open's typed-slug arm creates, or its ownership marker (covered by `orchestration/worktree/005`, `worktree/reclaim/022`/`023`).
- **Platform coverage:** mac+linux (PTY-attached, `#[cfg(feature = "e2e")]`, as the other `e2e_orchestration_*.rs` files).

##### orchestration/identity/014 — For a given live orchestration, the string `mark_worktree_owned` writes into the worktree's `created-by:` marker and the string every role pane's `AgentSpawnOptions::owner` carries are the LITERAL SAME computed string, from one source — not two derivations of one input (fork #166 M2.4).
...
- **Asserts:** the spawned child's own stdout — read back over the attach socket via `common::wait_for_pane_text_on`, itself reading `$DOT_AGENT_DECK_WORKTREE_OWNER` out of the child's OWN environment — contains the exact owner string `create_pane_with_options` was called with. This is the join `orchestration/identity/014`'s mock cannot exercise: producer (`AgentSpawnOptions::owner`) and reader (`worktree/reclaim/024`–`028`, which set the variable by hand on a subprocess) were each tested; this is the missing middle.
- **Does not assert:** anything about the marker file or `owner_of` (covered by `orchestration/identity/014` and the `worktree/reclaim` series); LLM-agent behaviour (no agent is spawned); the interactive form/restore paths that populate `AgentSpawnOptions::owner` in the first place (covered by `orchestration/identity/001`–`008` and the PRD's M2.4 milestone text) — this test starts from an already-populated `AgentSpawnOptions`, proving only that the value, once set, survives to the child's real environment.
- **Platform coverage:** mac+linux.

##### orchestration/identity/021 — Two claim attempts for the SAME orchestration name, made to overlap via a `Barrier`, resolve to exactly one winner; the loser is refused rather than both landing. Releasing the winner's claim frees the name for a later claimant (fork issue #201 option (b): a daemon-side claim, released when the claiming pane closes — closing the window `orchestration/identity/003`'s form-open snapshot cannot see, since two forms opened close together each read a `live_orchestration_names` list captured once, before either submits).
- **Layer:** fast unit test, in-process (`src/agent_pty.rs`, `#[cfg(test)] mod tests`). Two `std::thread`s synchronized on a `std::sync::Barrier` call `AgentPtyRegistry::claim_orchestration_name` for the identical name — no daemon, no PTY, no wire-protocol round-trip.
- **Agent:** none.
- **Asserts:** exactly one of the two claims succeeds (an XOR, not "at most one" and "at least one" checked independently); calling `AgentPtyRegistry::release_orchestration_name` for the winner's pane id then lets a third, later claim for the same name succeed.
- **Does not assert:** reviewer P2-8 (fork PR #532 review round): the `Barrier` makes an overlap POSSIBLE, not guaranteed — nothing stops the two threads from serializing, and the XOR assertion is satisfied by a serialized run too. What this test actually proves is STRUCTURAL: `claim_orchestration_name` holds `self.orchestration_name_claims.lock()` across both the check and the insert with no early drop and no re-lock, so no interleaving exists to probe at all — a genuine "does this actually race under contention" property is not what an XOR-under-a-barrier demonstrates, whatever the two threads happen to do at runtime. It also does not assert the wire-protocol surface (covered by `daemon_protocol.rs`'s `claim_orchestration_name_request_serde_round_trip` / `confirm_orchestration_claim_request_serde_round_trip` / `release_orchestration_name_request_serde_round_trip`, and the daemon-side pane-id validation in `confirm_orchestration_claim_rejects_a_pane_id_the_daemon_never_minted`), or the TUI-side `Action::SpawnPane` call site — scoped to the registry method a handler would call, not the RPC itself; automatic release driven by `StopAgent`'s pane-close handler; the UI form-level advisory refusal path, which `orchestration/identity/003` already covers and this test does not touch.
- **Platform coverage:** mac+linux+windows (pure `Mutex`/`HashMap`/thread logic, no PTY).

##### orchestration/identity/022 — The non-race edges of the daemon-side name claim `identity_021` doesn't cover, all exercised by the production caller `Action::SpawnPane` (`src/ui.rs`) adds: distinct names never cross-block each other; a pane re-claiming its OWN already-held name is idempotent (succeeds) while a DIFFERENT pane is still refused; releasing a pane id that holds no claim is a silent no-op that leaves every other live claim untouched (`StopAgent`'s handler calls `release_orchestration_name` unconditionally for every closing pane, claim or not).
...
- **Does not assert:** the exclusive-race guarantee under concurrent claims (covered by `orchestration/identity/021`); the wire-protocol surface (covered by `claim_orchestration_name_request_serde_round_trip` / `confirm_orchestration_claim_request_serde_round_trip` / `release_orchestration_name_request_serde_round_trip` in `src/daemon_protocol.rs`'s `#[cfg(test)] mod tests`, uncatalogued like their `resize`/`subscribe-events` wire-shape siblings); the pre-spawn token claim and post-spawn confirm/rebind mechanics (covered by `orchestration/identity/023`/`015`); the `Action::SpawnPane` call site or `StopAgent`'s automatic-release handler themselves.
- **Platform coverage:** mac+linux+windows (pure `Mutex`/`HashMap` logic, no PTY).

##### orchestration/identity/023 — Fork issue #201 redesign: the claim now happens BEFORE any worktree/pane provisioning, keyed by a caller-minted TOKEN rather than a real pane id (no real id exists that early). Claiming by token, then confirming/rebinding onto a real pane id once the spawn succeeds, leaves the name held by that real pane id — so `StopAgent`'s EXISTING by-pane-id `release_orchestration_name` still frees it on close, with no separate release mechanism needed for the post-rebind case.
- **Layer:** fast unit test, in-process (`src/agent_pty.rs`, `#[cfg(test)] mod tests`). Straight-line calls against one `AgentPtyRegistry` — no threads, no daemon, no PTY.
- **Agent:** none.
- **Asserts:** claiming a name by a token succeeds; `confirm_orchestration_claim(token, pane_id)` rebinds it and returns `true`; the name is then refused for both the stale token and a fresh token (still held, now by the pane id); releasing by that PANE id — exactly `StopAgent`'s existing call — frees it for a new claimant.
- **Does not assert:** the daemon's validation that `pane_id` actually names a live agent before accepting the rebind (covered by `confirm_orchestration_claim_rejects_a_pane_id_the_daemon_never_minted` in `src/daemon_protocol.rs`, since `agent_records()` is a daemon-side concern the bare registry method doesn't perform); the wire-protocol shape (`confirm_orchestration_claim_request_serde_round_trip`); the `Action::SpawnPane` call site that sends the token claim before provisioning (covered by `orchestration/identity/026`) or the session-restore call site (currently uncovered — see PR #532's tester report).
- **Platform coverage:** mac+linux+windows (pure `Mutex`/`HashMap` logic, no PTY).

##### orchestration/identity/024 — A confirm/rebind naming the WRONG token (a race, or a token already confirmed once) is refused and must not disturb the claim genuinely held by the real token — proving confirm is exact-match, not "steal whatever's there". Separately: releasing by the ORIGINAL TOKEN before any confirm happens — the "spawn failed after a successful claim" case `Action::SpawnPane`'s rollback needs — frees the name immediately, using the SAME `release_orchestration_name` method the post-rebind (pane id) case uses, since the holder is just an opaque string either way and needs no separate "release by token" method.
- **Layer:** fast unit test, in-process (`src/agent_pty.rs`, `#[cfg(test)] mod tests`). Straight-line calls against one `AgentPtyRegistry` — no threads, no daemon, no PTY.
- **Agent:** none.
- **Asserts:** confirming with a token that never claimed the name returns `false` and leaves the ORIGINAL token's claim refusable by a third party; releasing by that original token then frees the name for a new claimant immediately, with no rebind ever having happened.
- **Does not assert:** the daemon-side wiring of the pre-rebind rollback release into `Action::SpawnPane`'s spawn-failure path (covered at the integration level by `orchestration/identity/027`); the confirm-side pane-id validation (`orchestration/identity/023`'s scope note applies here too).
- **Platform coverage:** mac+linux+windows (pure `Mutex`/`HashMap` logic, no PTY).

##### orchestration/identity/025 — Fork issue #201 redesign (reviewer P1-1/P1-2, auditor A1): a refused orchestration-name claim must happen BEFORE any role pane is spawned and before any tab is pushed, so a lost race leaves NOTHING behind to roll back — no ghost tab, no orphaned panes.
- **Layer:** fast synthetic direct-call unit test, embedded in `src/ui.rs`'s own `#[cfg(test)] mod tests`, driving the real `Action::SpawnPane` path against an in-process stub attach daemon (`with_crafted_response_daemon`) that answers the claim with a crafted refusal.
- **Agent:** none — `CapturingPaneController` records calls without spawning real processes.
- **Asserts:** after dispatch, zero role panes were ever spawned (`CapturingPaneController.recorded_cwds()` is empty), the `TabManager` still holds only the Dashboard tab, and the refusal reason reached `ui.status_message`.
- **Does not assert:** the ORDERING guarantee that the claim is sent before provisioning on a path that SUCCEEDS (covered by `orchestration/identity/026`, since a refused claim alone can't distinguish "claim moved earlier" from "rollback got better at cleaning up after itself"); worktree/clone provisioning specifically, since `req.dir` here is deliberately not a git repository so no worktree branch runs at all (`orchestration/worktree/003`/`004` cover worktree provisioning under the SUCCESS path); the session-restore call site — it now claims too (`src/ui.rs:13546`), but this test only exercises the fresh-spawn path, so the restore call site's refusal handling stays currently uncovered (see `orchestration/identity/023`'s own scope note).
- **Platform coverage:** mac+linux+windows (`CapturingPaneController` is a pure in-memory mock; the stub daemon uses the same cross-platform `bind_attach_listener` as `worktree_004`).

##### orchestration/identity/026 — Fork issue #201 redesign: the orchestration-name claim must be the daemon's FIRST request, sent before any role pane spawns (using a caller-minted token, since no real pane id exists yet) — the ORDERING half `identity_025`'s refusal-only test can't isolate on its own.
- **Layer:** fast synthetic direct-call unit test, embedded in `src/ui.rs`'s own `#[cfg(test)] mod tests`. A hand-rolled stub daemon (`with_ordering_probe_daemon`) records, the instant it receives its first-ever request, how many panes `CapturingPaneController` had already spawned, then answers every request `ok`.
- **Agent:** none.
- **Asserts:** exactly 0 role panes existed at the moment the daemon received its first request — under the OLD "claim after spawn" design this would read 2 (both roles of `make_orchestration("review")` already created).
- **Does not assert:** which specific op the first request carries — the `ClaimOrchestrationName`/token shape exists and is exercised on this branch, but its wire shape is covered separately by the serde round-trip tests in `src/daemon_protocol.rs`, not by this ordering probe; the confirm/rebind step onto the real pane id once provisioning succeeds (covered at the registry level by `orchestration/identity/023`); refusal behavior (`orchestration/identity/025`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/identity/027 — Fork issue #201 redesign: a role-pane spawn failure that happens AFTER the pre-spawn token claim already succeeded must release that token-held claim, so the name is immediately claimable again rather than stuck until some pane that never fully existed times out.
- **Layer:** fast synthetic direct-call unit test, embedded in `src/ui.rs`'s own `#[cfg(test)] mod tests`. `PartialFailPaneController` fails the second `create_pane_with_options` call (exercising `open_orchestration_tab`'s own pre-existing per-role rollback — close what was already spawned, then return `Err` without ever pushing a tab); `with_recording_daemon` logs the SEQUENCE of ops the daemon receives.
- **Agent:** none.
- **Asserts:** no orchestration tab is pushed; the spawn failed exactly on the second role-pane attempt; the op log contains BOTH `claim-orchestration-name` (the pre-spawn claim was genuinely sent) AND `release-orchestration-name` (the failure released it) — under the OLD "claim after spawn" design, `open_orchestration_tab` failing means no claim is ever sent at all, so the log would be empty against that design.
- **Does not assert:** the exact reason string surfaced to the user on this failure path; the confirm/rebind step, which never runs on this path since the spawn never reached success; the daemon-side pane-id validation on confirm (out of scope here — see `orchestration/identity/023`'s notes and `confirm_orchestration_claim_rejects_a_pane_id_the_daemon_never_minted` in `src/daemon_protocol.rs`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/identity/028 — Fork issue #201 round 3 (auditor B1/C2): a CONFIRMED orchestration-name claim — already bound to a real pane id, exactly as `confirm_orchestration_claim` leaves it — must be released if the underlying process exits on its own, with no client ever sending `StopAgent`. Before this fix, `StopAgent` was the ONLY releaser: `agent_records()` already filters an exited-but-not-yet-reaped entry out of the live set, so a `StopAgent` arriving after the fact would find nothing to release either, and the name stayed squatted until the daemon restarted.
- **Layer:** fast unit test, in-process (`src/agent_pty.rs`, `#[cfg(test)] mod tests`). Real spawned child process (`sleep 0.3`, no PTY-attached client), real reader thread — not a synthetic `exited` flag flip.
- **Agent:** none.
- **Asserts:** a name claimed directly by a briefly-alive agent's real `pane_id_env` (simulating the post-confirm state) is claimable by a fresh claimant once the reader thread observes the process's own EOF and flips `exited` — with no `close_agent`/`StopAgent` call anywhere in the test. The exited entry is confirmed still present in the registry first (`registry.len() == 1`), ruling out "the release only worked because the whole record got reaped".
- **Does not assert:** the wire-level `StopAgent` handler's own release call, which is unchanged and already covered by `identity_023`; the gating condition itself (`is_agent_still_registered`) that keeps a `respawn_agent_in_pane` successor's still-live claim from being wrongly released — that guard is pre-existing production code this fix reuses, not new surface; the pre-confirm token-holder case, which is a different failure (auditor C3, tracked separately, not fixed by this hook).
...
##### orchestration/identity/029 — Fork PR #532 review round (reviewer R4-1): a genuinely successful `ClaimOrchestrationName` followed by the daemon refusing the subsequent `ConfirmOrchestrationClaim` (not `ok`) must release the token-held claim — otherwise it stays squatted under a token `StopAgent` will never match, since a confirmed claim is only ever released by pane id.
- **Layer:** fast synthetic direct-call unit test, embedded in `src/ui.rs`'s own `#[cfg(test)] mod tests`. `CapturingPaneController` (both role-pane spawns succeed, as in `identity_026`); a new hand-rolled stub daemon (`with_recording_daemon_refusing_op`) answers `claim-orchestration-name` with `ok` and `confirm-orchestration-claim` with a crafted refusal, while recording the full op sequence.
- **Agent:** none.
...
- **Layer:** L2 PTY-attached (the real binary through the vt100 `TuiDeck` harness).
- **Agent:** none (fixture `tests/fixtures/orch-deck`: two `cat` stub roles, no LLM tokens spent).
- **Asserts:** a bracketed paste (`\x1b[200~…\x1b[201~`) aimed at the focused non-orchestrator worker pane does NOT reach its PTY while LOCKED — `Event::Paste` previously called `embedded.write_raw_bytes` directly, bypassing `gate_pane_input_key` entirely. One further ordering detail a careless fix gets wrong is pinned here: after building real scrollback in the worker pane, re-locking, and scrolling back via the mouse wheel (the one scroll door that survives a `PaneInput` re-entry), a further dropped paste does NOT yank the view back to live output (`reset_scrollback` must not fire on a drop). Issue #414: that second drop is proven by a POSITIVE, synchronous signal (the lock's own denial status message, `ORCHESTRATION_LOCK_STATUS_MESSAGE`, appearing on the bottom bar) rather than by the absence of the paste's sentinel within a fixed budget — the latter cannot tell "denied" apart from "delivered but not yet echoed back by the `cat` stub under CI load", a candidate explanation for Part 2's historical ~1-in-4 CI flakiness that issue #414's own thread leaves open (neither reproduced nor ruled out, alongside a separate product-defect branch). This change does not establish which cause was responsible; what it buys is determinism going forward — a future recurrence is now self-proving (the synchronous signal either appears or it doesn't) instead of resting on a timing-sensitive absence check.
- **Does not assert:** what a real agent does with a delivered paste (no real-agent variant exists for paste, unlike `orchestration/lock/012`'s keystroke coverage); the `WaitingForInput` carve-out for a paste (`orchestration/lock/022` now covers it at the unit level); that a denied paste leaves `last_pane_keystroke_at` untouched — this test used to pin that with a PTY stopwatch (a timed round trip compared against a control), which was flaky (issue #390: bounded 150ms signal, observed deltas up to 424ms on a loaded CI runner). Only the gate's half of that property moved to a deterministic replacement: `gate_paste_delivery`'s own non-stamping is now pinned at the unit level (`orchestration/lock/020`, `/021`, `/022`). The call site's half did not move anywhere — nothing pins that `run_tui`'s `Denied` arm applies no stamp of its own, and that residual is tracked as its own follow-up: [#412](https://github.com/prageethw/dot-agent-deck/issues/412) (same class as fork #359).
- **Platform coverage:** mac+linux.

##### orchestration/lock/020 — `gate_paste_delivery` denies a paste on a locked worker pane with no `WaitingForInput` status, carrying the same lock status message a denied keystroke shows.
- **Layer:** L1 (in-process, against a real orchestration tab; `src/ui.rs`).
- **Agent:** none (fixture-free — `spawn_lock_test_orchestration`'s stub roles).
- **Asserts:** on a real, freshly opened (default LOCKED) orchestration tab with the non-orchestrator worker pane focused and an empty `pane_status` map, `gate_paste_delivery` returns `PasteGateOutcome::Denied` whose `status_message.0` equals `ORCHESTRATION_LOCK_STATUS_MESSAGE` and whose `status_message.1` is exactly the injected `now`, not a fresh wall-clock read — so an implementation that re-samples the clock in the `Denied` arm cannot pass. Deterministic replacement for the gate's share of the `last_pane_keystroke_at`-on-drop half of `orchestration/lock/017`'s former Part 3 (issue #390); `Denied` carries no `keystroke_at` field at all, so that share is now partly structural. The call site's share is unpinned and tracked as [#412](https://github.com/prageethw/dot-agent-deck/issues/412).
- **Does not assert:** the delivered/unlocked counter-case, which alone would make this test pass against a `gate_paste_delivery` that always denies (`orchestration/lock/021`); the `WaitingForInput` carve-out (`orchestration/lock/022`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/lock/021 — `gate_paste_delivery` delivers a paste unchanged when the deck-global lock is off, stamping exactly the caller's injected `now`.
- **Layer:** L1 (in-process, against a real orchestration tab; `src/ui.rs`).
- **Agent:** none.
- **Asserts:** with the same real orchestration tab and focused worker pane as `orchestration/lock/020` but `ui.command_entry_locked` set `false`, `gate_paste_delivery` returns `PasteGateOutcome::Delivered` whose `bytes` equal the payload passed in unchanged and whose `keystroke_at` equals the injected `now` exactly — not a fresh wall-clock read. The pass-through half of `orchestration/lock/020`'s pair; without it a `gate_paste_delivery` that always denies would still satisfy `/020`.
- **Does not assert:** the denied/locked case (`orchestration/lock/020`); the `WaitingForInput` carve-out (`orchestration/lock/022`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/lock/022 — The `WaitingForInput` carve-out reaches `gate_paste_delivery` exactly as it reaches a keystroke through `gate_pane_input_key`.
- **Layer:** L1 (in-process, against a real orchestration tab; `src/ui.rs`).
- **Agent:** none.
- **Asserts:** on the same real LOCKED orchestration tab, with the focused worker pane's `pane_status` resolved to `WaitingForInput`, `gate_paste_delivery` returns `PasteGateOutcome::Delivered` with the payload unchanged and `keystroke_at` equal to the injected `now` — the carve-out (`orchestration/lock/006`/`013`) is not keystroke-specific, since `gate_paste_delivery` calls the exact same `gate_pane_input_key` a keystroke does. Unlike `/021`, which reaches `Delivered` through the deck-global lock being off (`gate_pane_input_key`'s second early return) and never exercises the carve-out, this is the only one of the trio that runs the genuine carve-out path — not redundant with `/021` despite both asserting `Delivered`.
- **Does not assert:** the carve-out's own denial/provenance rules, already pinned at the `gate_pane_input_key` level (`orchestration/lock/006`, `/007`, `/013`).
- **Platform coverage:** mac+linux+windows.
...
##### orchestration/newpane/004 — `Ctrl+n` is swallowed with no picker when the focused pane's own session has declared itself `HistoryOnly` (issue #521 non-live-pane hypothesis) — REFUTED; now `#[ignore]`d and tracked as issue #524.
- **Layer:** L2 PTY-attached (the real binary through the vt100 `TuiDeck` harness).
...
- **Asserts:** with the orchestrator's own pane focused on a real Orchestration tab, once its session has durably declared `live_target: {kind: process, writable: history-only}` (confirmed applied via `deck.subscribe_events()` before the keypress, to avoid racing the injected event), pressing `Ctrl+n` does NOT open the ` Select Directory ` popup, and instead the `non_live_input_feedback` swallow gate's status message ("History-only session cannot accept live input") appears — the gate is `src/ui.rs`'s `run_tui` event-read loop, guarded on `ui.mode == UiMode::PaneInput && pane_writable(focused_id) != Live`.
- **Result:** CI ran this deterministically red 3/3 tries (run `32392160186`) — the picker opens anyway, refuting the hypothesis for issue #521 (which has since been re-scoped to a confirmed, unrelated, already-fixed mechanism). An auditor reviewing the failure noticed the swallow status message ALSO appears alongside the picker — both fire together, a real but separate, minor UI-consistency bug, filed as issue #524 rather than fixed here.
- **Does not assert:** a production fix — diagnosis only, per the delegating task. `#[ignore]`d rather than deleted: it is the only e2e coverage of a global chord (as opposed to forwarded pane text/paste, already covered by `pane_input_007` and `orchestration/remit/003`) landing on a `HistoryOnly` focused pane, so it doubles as issue #524's ready-made repro. Whoever picks up #524 should un-ignore this and rewrite the assertions to match whichever single behavior the fix settles on. The test carries no `#[spec(...)]` annotation any more — Decision 26 forbids pairing `#[ignore]` with `#[spec(...)]` — so this catalog ID is on `xtask/linkage-check/m2.allowlist` instead until the test is un-ignored and re-annotated.
...
##### orchestration/newpane/006 — `Ctrl+n` with the orchestrator's own pane genuinely `unconfirmed` by the REAL shell-activity poller, not a synthetic hook declaration (issue #521 shell-activity-unconfirmed hypothesis).
- **Layer:** L2 PTY-attached (the real binary through the vt100 `TuiDeck` harness).
- **Agent:** none (fixture `tests/fixtures/newpane-unconfirmed`: a single orchestrator role running a script, written into the deck's workdir by the test itself, that forks a tight loop of very short-lived `/usr/bin/true` children — no `setsid`, so they stay in the pane's own POSIX session — then `exec`s `cat`; no LLM tokens spent).
- **Asserts:** with the orchestrator's own pane focused on a real Orchestration tab, after giving the daemon's real `ps`/`getsid`-based shell-activity poller (`run_shell_activity_monitor`, `src/daemon.rs`) several 500ms ticks to sample the fork storm, pressing `Ctrl+n` still opens the ` Select Directory ` popup. The fork storm is engineered so that, on essentially every poll tick, a descendant alive for the poller's first `ps` capture has already exited by its second, identity-confirming capture — `invalidate_unconfirmed_session_ids` (`src/platform/proc/scan.rs`, fork issue #30) then resets that row's session id to unreadable, and `descendant_shell_activity` propagates that as `None` for the whole pane rather than `Some(false)`, landing this pane's own classification in the poller's `unconfirmed` bucket (`pane_unconfirmed_streaks`, fork issue #216) — the exact log line ("this pane's process-table sample succeeded but the pane went unconfirmed") the reporter captured from a real daemon at the moment of failure. Whether this genuinely reproduces issue #521 is decided by CI, not by this description — see the delegating task's `work-done` report for the confirmed result.
- **Does not assert:** a production fix — diagnosis only, per the delegating task. That the fork-storm technique reproduces the exact same code path that put the reporter's own orchestrator pane into `unconfirmed` (their trigger is still unconfirmed) — only that once a pane is genuinely `unconfirmed` via the real poller, `Ctrl+n` behaves (or does not) the way this hypothesis needs it to for issue #521 to be explained this way. Does not directly observe the daemon's internal `unconfirmed` set or streak state — this harness has no seam for that from outside the process — so the fork storm's effect is inferred from the mechanism (`src/daemon.rs`, `src/platform/proc/scan.rs`) rather than asserted on directly.
- **Platform coverage:** linux (Unix-only: the fork-loop script is a `/bin/sh` fixture, mirroring `newpane_004`'s own `#[cfg(unix)]` gate).
...
##### orchestration/seed/011 — A REAL interactive Haiku Claude Code orchestrator, spawned through the production dispatch path with no test intervention, genuinely SUBMITS the spawn-time seed pointer and genuinely READS the file it names and acts on it (upstream issue #424, CLAUDE.md rule 4). Restored 2026-08-24 by fork issue #373 after the 2026-08-15 upstream sync dropped this file without ever merging it onto post-sync `main` (fork issue #344's "a dropped fork commit deletes its own regression net" class) — test logic unmodified from the carrier branch, verified to still compile and pass against current `main`.
- **Layer:** L2 PTY-attached, real agent (`TuiDeck` on the `orch-seed-remit-real` fixture) driving the REAL `deliver_orchestrator_prompt` spawn-time delivery path with no test intervention — every `orchestration/seed/*` test up to `010` drives that function in-process against an injected clock and a `SendResultPaneController` double, proof the client-side retry/confirmation *logic* is correct but never proof a real Claude Code process ever received the pointer, read the file it names, and acted on it (exactly the insufficiency issue #424's own bug — a fixed 500ms readiness buffer outrun by 5 MCP servers and a 10.9 KB `SessionStart` hook — exists to fix).
- **Agent:** ONE real, fully interactive Claude Code pane pinned to Haiku (`claude --model claude-haiku-4-5-20251001 --allowedTools Bash`) as the orchestrator (start) role; the `worker` role is a `cat` stand-in, since only the orchestrator's remit delivery is under test. Cost: one short interactive turn (Decision 23).
- **Asserts:** the fixture's real Claude orchestrator becomes visibly input-ready within 120s (a boot/auth/trust failure here is distinguished from a seed-delivery failure in the panic message); then the full contract in order — (1) a native `UserPromptSubmit`-mapped `Thinking` hook event, carrying the literal pointer text (`Read .dot-agent-deck/orchestrator-context.md`) as `user_prompt`, proves the spawn-time seed was genuinely SUBMITTED inside the real session, never a fabricated/synthetic event; (2) the fixture's fixed sentinel token (`ORCH-SEED-011-REMIT-OK-6d29f4`) appearing in the pane's cumulative output proves the agent genuinely READ `.dot-agent-deck/orchestrator-context.md` and ACTED on its content, rather than merely having text typed at it.
- **Does not assert:** the client-side retry/confirmation logic itself (covered synthetically by `orchestration/seed/001`-`010`/`012`-`014`); a confirmation-retry's own non-duplicating behavior (`orchestration/seed/015`/`016`); anything about the `worker` role, which never receives a delegate in this test.
- **Platform coverage:** mac+linux (real-agent tier is local-only per Decision 8 / CLAUDE.md rule 5 exception (a) — gated on the `e2e` feature so CI's `cargo test-fast` never compiles it, and this file's real-agent tier has no CI credentials, so it self-skips in CI and a local run is the only way to exercise it at all). Flaky-tolerant (real LLM + real network) per rule 4 — run once, never looped.

##### orchestration/seed/015 — A REAL interactive Haiku Claude Code orchestrator's confirmation-retry — with its ORIGINAL write's confirming CR deterministically dropped by a test-only relay before ever reaching the agent — submits the spawn-time seed pointer through its own bare CR exactly ONCE, never duplicating the prompt text (fork #194, fork#197 M4's decided submit-only mechanism). Restored 2026-08-24 by fork issue #373, same drop-without-merge as `orchestration/seed/011` above; test logic unmodified from the carrier branch, verified to still compile and pass against current `main`.
- **Layer:** L2 PTY-attached, real agent (`TuiDeck` on the `minimal` fixture, with a `[[orchestrations]]` config written dynamically into the deck's workdir) hosting the real Claude orchestrator behind `CR_SUPPRESSING_WRAPPER_PY` — a test-only PTY relay (`tests/fixtures/cr_suppressing_wrapper.py`) that deliberately drops the ORIGINAL spawn-time write's confirming CR before it ever reaches the agent, so the seed text lands in the composer but never submits. This turns "no duplication observed" into a POSITIVE proof: since the original CR is independently proven (via a per-run private marker file — see below) to have never reached the agent, any submission observed can only be the confirmation-retry's own later bare CR. `DOT_AGENT_DECK_TEST_CONFIRMATION_GRACE_PERIOD_MS` is shrunk to 250ms so the retry fires deterministically, since nothing can ever confirm the suppressed original.
- **Agent:** ONE real, fully interactive Claude Code pane pinned to Haiku (`claude --model claude-haiku-4-5-20251001 --allowedTools Bash`) as the orchestrator role, hosted behind the relay. Cost: one short interactive turn (Decision 23), ~22s measured.
- **Asserts:** the real Claude orchestrator becomes visibly input-ready within 120s; the sentinel token (`ORCH-SEED-015-RETRY-CLAUDE-OK-9f21ab`) appears in the pane's output within 90s, proving the agent read and acted on the file the pointer names; the marker file — carrying two independent cumulative byte counts (total bytes read from the daemon, total bytes actually forwarded to the agent, not a flag) — reports a difference of exactly 1, proving the relay genuinely dropped exactly one byte (the original CR) rather than merely having executed the code path that would have; the pointer reached the agent through exactly ONE native prompt-submission event; and that event's `user_prompt` contains the pointer text exactly ONCE, never concatenated with itself (issue #194's exact observed symptom — a confirmation-retry re-writing the full prompt text into a composer that already held it, so one CR submits both copies as one message).
- **Does not assert:** the same proof for Codex (`orchestration/seed/016`, which stays on the older, weaker "no duplication observed in whichever branch ran" claim — the relay technique was attempted against Codex and abandoned this round, see that test's own doc comment); the happy-path no-retry delivery (`orchestration/seed/011`).
- **Platform coverage:** mac+linux, unix-only (`#![cfg(unix)]` — the relay is a POSIX PTY-forking script). Real-agent tier is local-only per Decision 8 / CLAUDE.md rule 5 exception (a); self-skips in CI for lack of credentials. Flaky-tolerant (real LLM + real network) per rule 4 — run once, never looped. No `[reel]` marker: this is a regression proof, not a showcase.

##### orchestration/seed/016 — A REAL interactive Codex orchestrator on a cheap model, with the confirmation-retry grace period shrunk to 250ms, delivers the spawn-time seed pointer to the agent through exactly ONE native prompt-submission event containing the pointer text exactly once — true whichever branch (retry or no retry) actually ran, so this does not positively prove Codex's documented CR-as-submit behavior held for a standalone retry CR specifically, only that no duplication was observed (fork #194, fork#197 M4). Restored 2026-08-24 by fork issue #373, same drop-without-merge as `orchestration/seed/011`/`015` above; test logic unmodified from the carrier branch, compile-verified against current `main` — self-skips on this machine's Codex credentials, so "pass" is not independently verified for this entry (fork issue #373 R5/R2: the harness's Codex trust seeding also has a known, documented gap against the isolated-clone sibling path, see `common::import_codex_credentials`'s doc comment).
- **Layer:** L2 PTY-attached, real agent (`TuiDeck` on the `minimal` fixture, with a `[[orchestrations]]` config written dynamically into the deck's workdir) driving the REAL `deliver_orchestrator_prompt` spawn-time delivery path with no relay — unlike `orchestration/seed/015`'s Claude case, the CR-suppressing relay technique was attempted against Codex and abandoned this round (routing `orchestrator_command` through the relay requires bypassing `wrap_launch_command`'s rewrite into `dot-agent-deck wrap --agent codex -- codex …`, and with the wrapper bypassed Codex never visibly reacted to the relayed input at all), so this test races the real confirming hook event exactly as the module doc describes, rather than deterministically forcing the retry.
- **Agent:** ONE real, fully interactive Codex pane on a cheap model (`--sandbox workspace-write --ask-for-approval never`, low reasoning effort) as the orchestrator role. Cost: one short interactive turn (Decision 23).
- **Asserts:** the real Codex orchestrator becomes visibly input-ready within 120s; the sentinel token (`ORCH-SEED-016-RETRY-CODEX-OK-4e91cb`) appears in the pane's output within 90s; the pointer reached the agent through exactly ONE native prompt-submission event; and that event's `user_prompt` contains the pointer text exactly ONCE.
- **Does not assert:** a positive proof that Codex's documented CR-as-submit behavior (`src/agent_pty.rs:3566`, `:3668`) held for a standalone retry CR specifically — only that no duplication was observed in whichever branch actually ran; the suppressed-CR positive proof `orchestration/seed/015` gives for Claude. OpenCode and Pi are explicitly out of scope for this PRD's M4 real-agent verification (accepted, documented unknown).
- **Platform coverage:** mac+linux, unix-only (`#![cfg(unix)]`). Real-agent tier is local-only per Decision 8 / CLAUDE.md rule 5 exception (a); self-skips in CI for lack of credentials. Flaky-tolerant (real LLM + real network) per rule 4 — run once, never looped. No `[reel]` marker: this is a regression proof, not a showcase.

##### orchestration/seed/017 — `send_retry_delay`'s fixed 500ms backoff floor becomes overridable in test/debug builds via `DOT_AGENT_DECK_TEST_SEND_RETRY_BASE_MS` — the seam PRD fork#257 M1 adds. Restored narrowed on 2026-08-17: the idiom this originally mirrored, `confirmation_grace_period()`, was removed by issue #424's rewrite, and the M4 on-demand confirmation-retry proof this seam was meant to enable belongs to `unconfirmed_retry_delay`, untouched by this test (PRD fork#257).
- **Layer:** L1, unit (calls `send_retry_delay` AND `send_retry_base` directly against the process-global env var, guarded by a module-level `static Mutex` / RAII restore guard — `SEND_RETRY_BASE_ENV_LOCK` / `SendRetryBaseEnvGuard` — mirroring `config.rs`'s `CONFIG_GEN_STATE_ENV_LOCK` / `ConfigGenStateEnvGuard` split idiom. No clock injection, no pane double, no PTY, no daemon).
- **Agent:** none.
- **Asserts:** unset, `send_retry_delay(1)` returns today's fixed 500ms floor unchanged (precondition). A valid override (`"10"`) replaces the floor exactly, and the exponential doubling shape survives on the lowered base — `send_retry_delay(2) == 20ms`, `send_retry_delay(3) == 40ms` — so a future change that flattens the curve fails here even with a lowered base. Zero is a legitimate override value (retry as soon as the grace period allows), not treated as unset or clamped up. A non-numeric override falls back to the fixed floor rather than panicking. An out-of-range override (`"120000"`) still makes `send_retry_delay(1)` land on `SEND_RETRY_BACKOFF_CAP` (2s) rather than escaping into an effectively-disabled multi-minute wait — AND, reviewer P1 (fork#257), the same override's ceiling clamp is asserted directly against `send_retry_base()` (`== SEND_RETRY_BACKOFF_CAP`, deliberately NOT `== SEND_RETRY_BASE_MAX` — comparing against the constant a mutation would itself change could never fail), where nothing caps the result afterwards, so a broken or removed clamp (e.g. raising `SEND_RETRY_BASE_MAX`, or dropping the `.clamp(...)` call) now fails this test — mutation-verified on CI: temporarily setting `SEND_RETRY_BASE_MAX` to 120_000ms produced exactly one RED test (`orchestration_seed_017_send_retry_delay_honors_test_base_override`, `assertion left == right failed` at the `send_retry_base()` comparison, 3099 passed / 1 failed), then reverted.
- **Does not assert:** the warn-once behavior (a `static AtomicBool`, per M1) is pinned only structurally — this repo has no log-capture test infrastructure, so this test does not directly observe the `tracing::warn!` call or that it fires exactly once rather than per-call. `deliver_orchestrator_prompt`'s actual gating of the confirmation-retry on the overridden floor was previously covered by `orchestration/seed/018`, restored alongside this test by PR #268 — that companion test is NOT restored here: as of this restoration, `deliver_orchestrator_prompt`'s confirmation-retry is gated by `crate::prompt_delivery::unconfirmed_retry_delay` (introduced by issue #424's rewrite, postdating PR #268), which is explicitly documented as "Deliberately NOT `crate::ui::send_retry_delay`'s 2s-capped schedule" and reads no env-var override at all. Restoring `orchestration/seed/018` as designed would therefore not prove what it originally proved even once M1/M2's `send_retry_base` override lands; see fork issue #257's tracking for the follow-up decision.
- **Platform coverage:** mac+linux+windows.

#### orchestration/worktree

##### orchestration/worktree/002 — When the resolved workspace fails to provision (e.g. `dir` is not a git repository), the orchestration tab is refused and the error surfaces — never a silent fallback to the shared cwd (fork #122).
...
- **Agent:** none.
- **Asserts:** dispatching `Action::SpawnPane` for a request with a typed Name against a `dir` that is not a git repository leaves the active tab as `Tab::Dashboard` (no orchestration tab opened), spawns no role panes, and sets a non-empty `ui.status_message` describing the failure.
...
- **Does not assert:** how the workspace path was resolved (covered by `orchestration/workspace/001`) or actually created on disk (covered by `orchestration/worktree/004`); real daemon/PTY spawn; work-done file routing itself (`orchestration/route/*`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/worktree/004 — Dispatching `Action::SpawnPane` for a request with a typed Name against a real git repository actually creates the resolved sibling workspace on disk and roots every role pane in it, not in `req.dir` (fork #122 — the actual feature, as opposed to `003`'s pre-existing-mechanism characterization).
...
- **Agent:** none.
- **Asserts:** after dispatch, the resolved workspace path exists on disk as a directory; every role's `create_pane_with_options` call is recorded with the workspace path (not `req.dir`) as `cwd`; every `AppState.pane_cwd_map` entry for the orchestration's role panes equals the workspace path. `req.dir` and the workspace path are deliberately distinct directories, so the assertions cannot pass by `003`'s coincidence of the two being equal.
- **Does not assert:** how the workspace path was resolved from the typed Name (covered by `orchestration/workspace/001`); the fail-loud refusal path (`orchestration/worktree/002`); branch naming or content; real daemon/PTY spawn; work-done file routing itself (`orchestration/route/*`).
...
##### orchestration/worktree/014 — Two concurrent orchestrations against the SAME root checkout — the 2nd spawned while the 1st is still live — each provision their OWN isolated clone, distinct from the launch directory's and from each other's (PRD fork#325 M3's original Nth-concurrent gate; PRD fork#544 M2b inverts its pre-M2b "1st shares, Nth isolates" shape — isolation is unconditional now, so the 1st isolates too).
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness — inherently about real concurrent git/filesystem behavior, not renderable in an L1 widget test, per the PRD's own test plan).
- **Agent:** none (`orch-clone-gate` fixture — a single orchestration with one `solo` role that appends its own `DOT_AGENT_DECK_WORKTREE_OWNER` identity plus its own `pwd` to one shared, HOME-relative log file before staying alive on `sleep`; no real LLM tokens spent).
- **Asserts:** opening the fixture's one orchestration twice against the same launch directory, each with a distinct typed Name, the second while the first is still live: BOTH instances' actual working directories (read back from the log by owner string) resolve to a git common dir distinct from the launch directory's own AND from each other's; both are checked out on their own typed Name's branch; both carry the source checkout's own origin URL, never a local-path default.
- **Does not assert:** the fail-open-vs-fail-closed behavior of the pre-provisioning `ClaimOrchestrationName` daemon round trip when it cannot reach the daemon (`015`'s job); a 3rd orchestration colliding with two already-live ones (`016`'s job); Model B (`dispatch.rs`)'s equivalent gate, out of scope for this PRD.
- **Platform coverage:** mac+linux in practice (no `cfg` gate — the fixture's role command needs `/bin/sh`; never built for Windows because the `e2e` job is Linux-only per CLAUDE.md rule 2).

##### orchestration/worktree/015 — The pre-provisioning `ClaimOrchestrationName` daemon round trip FAILS CLOSED (refuses the spawn) when the daemon is unreachable, rather than silently proceeding as if the claim had succeeded (PRD fork#544 M2b: Model A's own call sites to the Nth-concurrent liveness gate are retired since isolation is now unconditional, so what this test pins is the claim step's own fail-closed contract — the only daemon round trip left in the spawn path).
- **Layer:** L1 (pure — `Action::SpawnPane` dispatched directly against a real `CapturingPaneController`/`TabManager`/`AppState`, no PTY, no real daemon process; the "unreachable daemon" is a `DOT_AGENT_DECK_ATTACH_SOCKET` override pointed at a path nothing is listening on, so the client's connect attempt fails immediately rather than timing out).
- **Agent:** none.
- **Asserts:** submitting an orchestration form with a typed Name against a directory that IS a genuine, valid git repository (unlike `002`'s deliberately-broken one — the claim step is the only thing that can refuse this spawn) while the attach socket is redirected to an unlistened path: the orchestration tab is never opened (`tm.active_tab()` stays `Tab::Dashboard`), no role pane is spawned, a non-empty status message surfaces the refusal, and the resolved workspace path is never created on disk — proving the refusal happens BEFORE any provisioning attempt touches the filesystem, not merely that provisioning later fails.
- **Does not assert:** the up-daemon isolation path itself (that's `014`'s job); which specific status-message wording is shown (only that one is set); Model B (`dispatch.rs`)'s equivalent gate.
...
##### orchestration/worktree/016 — Three concurrent orchestrations against the SAME root checkout — issue #325's own actual incident shape, not `014`'s reduced 2-orchestration case — all provision their own isolated clone (PRD fork#544 M2b: unconditional isolation means the 1st is no longer the exception), and reproduce closed both of the issue's concrete failures reframed for that shape: no orchestration's clone can `git worktree remove` another's directory (including the launch directory's own), and a shallow fetch performed inside any one orchestration's own isolated clone cannot reach the launch directory's or any other orchestration's object store (PRD fork#325 M5, updated by PRD fork#544 M2b).
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness — inherently about real concurrent git/filesystem behavior, not renderable in an L1 widget test, per the PRD's own test plan).
- **Agent:** none (`orch-clone-gate` fixture, reused from `014` — a single orchestration with one `solo` role that appends its own `DOT_AGENT_DECK_WORKTREE_OWNER` identity plus its own `pwd` to one shared, HOME-relative log file before staying alive on `sleep`; no real LLM tokens spent).
- **Asserts:** opening the fixture's one orchestration three times against the same launch directory (distinct typed Names, the 2nd and 3rd each while an earlier one is still live): structurally, ALL THREE resolve to a `git rev-parse --git-common-dir` distinct from the launch directory's own AND from each other's — no two of the four (launch dir plus three orchestrations) sharing one. Behaviorally, reproduces issue #325's own two concrete failures directly: (1) `git worktree remove` attempted cross-repo between every pair of the three orchestrations, AND from an isolated clone against the LAUNCH DIRECTORY's own worktree — issue #325's original incident shape, literally — fails outright every time, each specifically with a `… is not a working tree` stderr (asserted, not merely that the removal did not succeed), leaving every target's directory, branch, and HEAD commit byte-identical to before the attempt; (2) a real `git fetch --depth 1` performed inside the 1st orchestration's own isolated clone makes ONLY that clone's object store shallow — the launch directory itself, and the 2nd/3rd orchestrations' isolated clones, all remain full, non-shallow repositories with unchanged HEAD commits, the exact inverse of the pre-M2b shape where the 1st shared the launch directory's common dir and a shallow fetch there was visible from the launch directory too.
- **Does not assert:** the fail-open-vs-fail-closed behavior of the pre-provisioning `ClaimOrchestrationName` daemon round trip when it cannot reach the daemon (`015`'s job); the origin-URL/branch-naming fixes in isolation (`014`'s own assertions, extended here rather than re-derived); recovery/repair of a shallow repository once corrupted (issue #325's own `git fetch --unshallow` repair procedure — out of scope, this test proves the corruption cannot spread, not how to undo it once it happens); Model B (`dispatch.rs`)'s equivalent gate; that `worker-agent-deck worktree reclaim`, production's own wrapper around the same `git worktree remove` this test invokes directly, also declines to touch an isolated clone (that is `isolated_clone_report`'s `KIND_ISOLATED_CLONE` verdict, covered by M4a's own tests).
- **Platform coverage:** mac+linux in practice (no `cfg` gate — the fixture's role command needs `/bin/sh`, matching `014`'s fixture; never built for Windows because the `e2e` job is Linux-only per CLAUDE.md rule 2).

#### orchestration/workspace

##### orchestration/workspace/001 — The FIRST orchestration ever opened against a root checkout (no concurrent sibling orchestration exists at all) still provisions its own isolated, named workspace — never a `git worktree add` sibling sharing the launch directory's own object store, and never the launch directory itself — at `<root-checkout-basename>-<sanitized-name>`, derived from the typed Name alone (PRD fork#544 M2/M2b: M2b retires the Nth-concurrent-only isolation gate so isolation is unconditional; M2 retires the separate Worktree-slug field and `orchestrator-N` auto-numbering as the path's identity in favor of the orchestration's own Name).
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness, reusing `orchestration/worktree/014`/`016`'s `orch-clone-gate` fixture — inherently about real git/filesystem behavior, not renderable in an L1 widget test, per the PRD's own test plan reasoning).
- **Agent:** none (`orch-clone-gate` fixture's `solo` role appends its own `DOT_AGENT_DECK_WORKTREE_OWNER` identity plus its own `pwd` to one shared, HOME-relative log file before staying alive on `sleep`; no real LLM tokens spent).
- **Asserts:** opening the fixture's one orchestration exactly once, with the form's suggested Name cleared and replaced with a fixed literal, submitted with a BLANK Worktree-slug field: the role pane's actual working directory (read back from the log by owner string) equals `work.with_file_name("<launch-dir-basename>-<the-typed-name>")` exactly, and its `git rev-parse --git-common-dir` differs from the launch directory's own — proving both the naming formula and the now-unconditional isolation. Today's actual code, for this exact scenario, resolves the pane's cwd to the launch directory itself (`dir_str = req.dir.display().to_string()` in the `None`/`Ok(false)` arm of `Action::SpawnPane`'s dispatch) — both assertions are RED against it.
- **Does not assert:** the Nth-concurrent case (that stays `014`'s/`016`'s job, unaffected by this PRD); resume of an existing named workspace across a session close/reopen (PRD fork#544 M3, out of scope here — M2/M2b only change WHERE a workspace is created and WHEN isolation applies, not whether an existing one is ever reused); ownership-marker content or schema (M4); the "forget this workspace" retire action (M5); `record_worktree` registration (M6); post-merge sync (M7).
- **Platform coverage:** mac+linux in practice (no `cfg` gate — the fixture's role command needs `/bin/sh`, matching `014`'s/`016`'s own fixture; never built for Windows because the `e2e` job is Linux-only per CLAUDE.md rule 2).

##### orchestration/workspace/002 — The workspace-naming formula (`<root-checkout-basename>-<sanitized-name>`) is a pure function of the root checkout's own basename and the typed Name alone — run twice, as two fully independent scenarios (two separate `TuiDeck` launches, the second only after the first is torn down) typing the identical Name against two different root checkouts, each run's resulting workspace directory independently matches the formula for ITS OWN root checkout, rather than depending on session-local counter state such as `auto_generate_worktree_slug`'s `orchestrator-N` (PRD fork#544 M2).
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness, same `orch-clone-gate` fixture as `001`).
- **Agent:** none (same fixture/mechanism as `001`).
- **Asserts:** two sequential, independent single-orchestration runs (same shape as `001`'s flow) typing the SAME fixed Name against two DIFFERENT root checkouts (distinct random tempdir basenames, confirmed distinct as a sanity precondition): run 1's resulting pane directory equals `<run-1's-own-basename>-<the-shared-name>`, and run 2's independently equals `<run-2's-own-basename>-<the-shared-name>` — the identical derivation rule holding in each scenario on its own terms, not merely that some slug text happens to repeat.
- **Does not assert:** literal identity of the two runs' absolute paths (impossible — the two root checkouts have different basenames by construction, since each is a freshly-generated tempdir); the resume/reuse question of what happens if the SAME root checkout is reopened under the SAME Name a second time while the first workspace still exists on disk (that is PRD fork#544 M3's `Resumed` arm, explicitly out of scope for M2 — today's code would only ever attempt fresh creation, since nothing implements reuse yet).
- **Platform coverage:** mac+linux in practice (no `cfg` gate, matching `001`).

##### orchestration/workspace/003 — Reopening an orchestration under the IDENTICAL typed Name a second time, after closing its whole tab, RESUMES the exact same workspace directory rather than refusing it — an uncommitted change and a local-only commit made in the workspace before closing are both still present, byte-identical, after resuming (PRD fork#544 M3: the `Resumed` outcome, the headline behavior the milestone adds).
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness, same `orch-clone-gate` fixture as `001`/`002`).
- **Agent:** none (same fixture/mechanism as `001`).
- **Asserts:** first open resolves to `<basename>-<name>` (matching `001`); an uncommitted file and a locally-committed file are written directly into that directory; the whole tab is closed via Ctrl+D → Ctrl+W → confirm, and the workspace directory is confirmed to still exist on disk afterward; a second open under the identical Name resolves to the SAME directory (read back from the shared log's second appended line, last-line-wins by owner since both opens share one owner string); the uncommitted file's content is byte-identical after reopening; the workspace's HEAD commit (the local-only commit) is byte-identical after reopening. Today's code has no `Resumed` outcome, so the second `provision_isolated_clone_sync` call sees the directory present and reports the generic `AlreadyClaimed` — the New Pane form never closes, and `deck.wait_for_absence("New Agent")` times out: RED against it.
- **Does not assert:** the fetch-only-mutation guarantee on resume (`004`); refusal when the Name is held by a live orchestration (`005`); the three-part eligibility check's individual branches — stranger directory / wrong ancestry / unhealthy (`006`-`008`, direct-function tests); the concurrent-open race (`009`, direct-function test); ownership-marker schema (M4); the "forget this workspace" retire action (M5); post-merge sync (M7).
- **Platform coverage:** mac+linux in practice (no `cfg` gate, matching `001`).

##### orchestration/workspace/004 — Resuming a closed workspace performs EXACTLY one read-only `git fetch` and ZERO other git mutation: the checked-out branch's own HEAD commit is byte-identical before and after resuming, even though the real `origin` remote has genuinely advanced with new commits while the workspace was closed, and the `origin/<default-branch>` remote-tracking ref is brought up to date with that advance (PRD fork#544 M3's Decisions table: "Fetch, yes (read-only). Integrate, never automatically.").
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness, same `orch-clone-gate` fixture as `001`-`003`, plus a REAL separately-advanceable local repository as `origin` — a genuine local clone the resumed workspace's own `git fetch origin` can actually reach, unlike `014`/`016`'s unreachable `https://example.invalid/...` placeholder).
- **Agent:** none (same fixture/mechanism as `001`).
- **Asserts:** the launch directory's `origin` is set to a real local repository before the first open; the isolated clone's `origin` is confirmed to be that same real URL; the whole tab is closed; two extra commits are made directly in the real upstream repository while the workspace is closed; reopening under the identical Name resumes the same directory (as `003`); the resumed directory's checked-out-branch HEAD is byte-identical to before closing; `git rev-parse refs/remotes/origin/<default-branch>` in the resumed directory resolves and equals the upstream's now-advanced HEAD, proving a genuine `git fetch origin` ran. Today's code has no resume arm at all, so the second open hits the generic `AlreadyClaimed` refusal and the New Pane form never closes: RED against it.
- **Does not assert:** preservation of uncommitted/local-only content (`003`, though the same zero-mutation property is exercised there too); any fast-forward, rebase, or merge of the checked-out branch (explicitly never — the PRD's own zero-mutation guarantee, so there is nothing to assert here beyond HEAD staying unchanged); refusal paths (`005`-`008`).
- **Platform coverage:** mac+linux in practice (no `cfg` gate, matching `001`).

##### orchestration/workspace/005 — Reopening a Name currently held by a LIVE orchestration (never closed) refuses as "name in use" — the New Pane form stays open, no second tab opens, no role pane is spawned for the refused attempt, and critically no numeric-suffix fallback sibling directory is ever created on disk (PRD fork#544 M3's eligibility check (a): reuse of the EXISTING fork#192 live-name query as the FIRST gate, ahead of the new resume logic — "the SAME check that already exists, not new logic").
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness, same `orch-clone-gate` fixture as `001`-`004`).
- **Agent:** none (same fixture/mechanism as `001`).
- **Asserts:** the first open creates and stays live (never closed); a second open attempt under the identical Name, from the Dashboard, leaves the New Pane form on screen (`"New Agent"` still rendered) rather than opening a second tab; no `<basename>-<name>-2`-shaped sibling directory exists on disk; the shared owner+pwd log carries exactly ONE line (the refused attempt never spawned a role pane); the first orchestration's own workspace directory remains intact. This check already exists pre-M3 (fork#192), so this test may already be GREEN against today's code — it is written as a regression-safety pin for M3's own eligibility-check ordering (this gate must keep firing FIRST, ahead of the new resume logic, once M3 lands), not as a guaranteed pre-M3 RED.
- **Does not assert:** the resume arm itself (`003`/`004`); any of the M3 eligibility check's other three branches, which only run once the live-name gate has already passed (`006`-`008`); the concurrent-open race between two near-simultaneous NON-colliding-live opens (`009`).
- **Platform coverage:** mac+linux in practice (no `cfg` gate, matching `001`).

##### orchestration/workspace/006 — `provision_isolated_clone_sync` called against a `clone_dir` that already exists on disk but was never created by this deck — no M4b provenance artifact exists for it, e.g. a directory a human happened to create at the same derived path with its own unrelated git history — refuses to silently attach and, once implemented, names the refusal distinguishably as a stranger directory (PRD fork#544 M3's compatibility table: `exists, live=no, evidence=no → refuse: stranger directory`).
- **Layer:** L1 (in-process — direct calls to `provision_isolated_clone_sync`; real `git` subprocess and filesystem against a tempdir, no PTY, no daemon, no real agent — mirroring the existing `provision_isolated_clone_sync_sets_origin_and_branch_when_source_has_origin` sibling tests' own real-git-subprocess style in this same file).
- **Agent:** none.
- **Asserts:** a `clone_dir` seeded with its own independent git history (never touched by `provision_isolated_clone_sync`) is not silently reported as `Created`; the outcome's `Debug` text contains `"stranger"`; the stranger directory's HEAD commit and working tree are byte-identical before and after the refused call. `DOT_AGENT_DECK_STATE_DIR` is pinned per-test (via the existing `ScopedEnvVar` helper) so the M4b evidence lookup this assertion depends on resolves deterministically. Today's code has no eligibility check at all — `clone_dir.exists()` alone returns the generic `AlreadyClaimed`, whose `Debug` text is just `"AlreadyClaimed"` — RED against it (no `"stranger"` substring).
- **Does not assert:** the exact `IsolatedCloneOutcome` variant shape M3 introduces (deliberately asserted via `Debug`-text substring rather than a variant match, so this test remains compilable against both pre-M3 and post-M3 code without a production-code change); the ancestry check (`007`); the health probe (`008`); the attach-lock race (`009`); anything about the New Pane form or a real daemon (`003`-`005`).
...
##### orchestration/workspace/007 — A `clone_dir` that DOES carry genuine M4b ownership evidence (created by a real prior `provision_isolated_clone_sync` call) is reopened against a SECOND, entirely unrelated source repository at the same derived path — the same Name typed against a different underlying project — and refuses distinguishably as a wrong/stale repo rather than silently attaching the new source's session onto foreign history (PRD fork#544 M3's compatibility table: `exists, live=no, evidence=yes, ancestry=no → refuse: wrong/stale repo`; closes the Problem Statement #4 gap — content-free M4b evidence alone can't prove WHICH repository a directory belongs to).
- **Layer:** L1 (in-process — direct calls to `provision_isolated_clone_sync`; real `git` subprocess and filesystem, no PTY, no daemon, no real agent).
- **Agent:** none.
- **Asserts:** a clone genuinely created from source A is not silently reported as `Created` when a second call for the identical `clone_dir` is made against unrelated source B (no shared history, no shared origin); the outcome's `Debug` text contains one of `"ancestry"` / `"wrong repo"` / `"stale"` / `"unrelated"`; the existing clone's HEAD commit is untouched by the refused attempt. `DOT_AGENT_DECK_STATE_DIR` pinned per-test, as `006`. Today's code has no ancestry check — the present-directory short-circuit reports the generic `AlreadyClaimed` regardless of which source is passed: RED against it (none of the expected substrings present).
- **Does not assert:** the no-evidence stranger case (`006`); the health probe (`008`); the attach-lock race (`009`); the exact ancestry-check algorithm (merge-base probe vs. some other mechanism) — only the observable refusal and its wording.
- **Platform coverage:** mac+linux+windows, matching `006`.

##### orchestration/workspace/008 — A `clone_dir` with genuine M4b evidence AND matching ancestry (both would otherwise pass) is corrupted — its `.git` directory removed entirely, simulating a directory mid-delete or otherwise unhealthy — before a second `provision_isolated_clone_sync` call against the SAME source; the health probe's failure refuses distinguishably, and the refusal never auto-deletes, repairs, or replaces the unhealthy directory (PRD fork#544 M3's compatibility table: `exists, live=no, evidence=yes, ancestry=yes, health=unhealthy → refuse: workspace unhealthy`; the Decisions table's explicit "never auto-delete" rule).
- **Layer:** L1 (in-process — direct calls to `provision_isolated_clone_sync`; real `git` subprocess and filesystem, no PTY, no daemon, no real agent).
- **Agent:** none.
- **Asserts:** a genuinely-created, then `.git`-corrupted clone is not silently reported as `Created`; the outcome's `Debug` text contains one of `"unhealthy"` / `"corrupt"` / `"health"`; the directory still exists on disk afterward (never auto-deleted); `.git` is still absent (the refusal did not silently repair or replace it); the working-tree file (`README.md`) is still present. `DOT_AGENT_DECK_STATE_DIR` pinned per-test, as `006`. Today's code has no health probe — the present-directory short-circuit reports the generic `AlreadyClaimed` regardless of the directory's actual git health: RED against it (none of the expected substrings present).
- **Does not assert:** the no-evidence stranger case (`006`); the ancestry check (`007`); the attach-lock race (`009`); the exact health-probe command (`git rev-parse HEAD` vs. some other check) — only the observable refusal, its wording, and the never-delete guarantee.
- **Platform coverage:** mac+linux+windows, matching `006`.

##### orchestration/workspace/009 — Two near-simultaneous `provision_isolated_clone_sync` calls, racing to resume the SAME already-eligible directory (created once via a real prior call, all three calls sharing the IDENTICAL creator string), split into exactly one `Resumed` and exactly one `Rejected(Contested)` per trial — never both, never neither — where pre-M3 code yields two identical `AlreadyClaimed` outcomes with no winner at all (PRD fork#544 M3's Design step 3: the attach-lock reuse "closes a race that resume introduces... two racers passing the eligibility check would otherwise attach live sessions to the same tree concurrently").
- **Layer:** L1 (in-process — two-thread barrier-synchronized direct calls to `provision_isolated_clone_sync`, mirroring the existing `provision_isolated_clone_sync_concurrent_calls_never_both_create` sibling test's own pattern in this file — including its shared `"racer"` creator string — but racing a RESUME of an already-present directory rather than a fresh CREATE; real `git` subprocess and filesystem, no PTY, no daemon, no real agent).
- **Agent:** none.
- **Asserts:** across 10 trials, each racing two threads against one pre-created, resume-eligible directory (barrier-synchronized so both start at nearly the same instant), with the setup call and both racers all opened under the identical creator string: exactly one of the two threads resolves as `Ok(Resumed { .. })` and exactly one resolves as `Ok(Rejected(Contested))`. `DOT_AGENT_DECK_STATE_DIR` pinned per-test, as `006`. Review-findings fix round (round 3): the original fixture used mismatched creators (`"setup"`/`"racer-a"`/`"racer-b"`), which made B2's own new `NameCollision` check refuse the race's winner too — nothing ever resumed, and the prior `assert_ne!` on the two outcomes' `Debug` strings passed for a reason unrelated to what the test claims to prove (the process-global registry mutex alone distinguishes `NameCollision` from `Contested`, so the test would stay green even with the attach lock removed entirely). One shared creator string restores the race-specific property: M3's resume arm yields exactly one `Resumed` and exactly one `Contested`, where pre-M3 code yields two identical `AlreadyClaimed`. Today's pre-M3 code has no `Resumed` outcome and no winner/loser distinction on resume at all — `clone_dir.exists()` alone makes BOTH racers report the identical generic `AlreadyClaimed`: RED against it.
- **Does not assert:** the eligibility check's individual branches (`006`-`008`, which this race deliberately bypasses by pre-creating an already-eligible directory); the `NameCollision` mismatched-creator case, deliberately excluded here by construction — that's `026`'s own point; a genuine two-PROCESS (as opposed to two-thread) race — real two-client PTY concurrency against a shared daemon/state-dir proved impractical to set up cleanly in this harness for the resume case specifically (see the tester's `work-done` report for PRD fork#544 M3's RED round); the exact lock primitive's internal behavior beyond the observable serialization; the attach lock's own contribution to the split — the process-global registry mutex alone produces this split, so this test stays green if the attach lock is removed.
- **Platform coverage:** linux (`#[cfg(unix)]`, matching `provision_isolated_clone_sync_concurrent_calls_never_both_create`'s own precedent — many trials of real `git` subprocess clones on a shared CI runner; never built for Windows).

##### orchestration/workspace/010 — Resuming the SAME already-eligible directory TWICE in a row, in one still-running process, with `resumed_isolated_clones()`'s registration released in between (mirroring what `src/ui.rs`/`src/dispatch.rs` now do immediately after `provision_isolated_clone_sync` returns) succeeds both times rather than the second resume being incorrectly refused as `Contested`; a THIRD attempt made WITHOUT releasing in between still correctly loses to `Contested`, proving `009`'s race protection survives the fix (PRD fork#544 M3 fix round: closes the gap where nothing ever released a resume registration, so a same-Name workspace could only ever be resumed once per process, contradicting the PRD's own stated purpose of repeatable resume).
- **Layer:** L1 (in-process — sequential direct calls to `provision_isolated_clone_sync` and `release_resumed_isolated_clone_registration`; real `git` subprocess and filesystem, no PTY, no daemon, no real agent).
- **Agent:** none.
- **Asserts:** create, then resume once (succeeds), then release the registration, then resume again (must succeed as `Resumed`, not `Rejected(Contested)`); a further resume attempted without an intervening release must still be refused as `Rejected(Contested)`. `DOT_AGENT_DECK_STATE_DIR` pinned per-test, as `009`. Pre-fix code never releases a registration at all, so the second resume in this sequence was RED against it (`Rejected(Contested)` instead of `Resumed`).
- **Does not assert:** the attach-lock race itself (`009`); the eligibility check's individual branches (`006`-`008`); which of the two real call sites (`src/ui.rs`, `src/dispatch.rs`) calls the release function — both do, exercised only indirectly here via the shared function they both call.
- **Platform coverage:** mac+linux+windows — no threading, no `#[cfg(unix)]` needed, matching `006`-`008` rather than `009`'s own precedent.

##### orchestration/workspace/011 — After a real `provision_isolated_clone_sync` call creates a fresh isolated clone (writing the M4b provenance artifact as a side effect), the artifact's raw on-disk content contains a hash of the root checkout's (`source_dir`'s) own canonical path, the orchestration Name typed for the workspace, and the clone's own canonical path — all as real, extractable data rather than a content-free presence marker (PRD fork#544 M4: "add a schema tag and a hash of the root checkout's canonical path... record the orchestration Name and canonical workspace path as explicit structured data").
- **Layer:** L1 (in-process — one real `provision_isolated_clone_sync` call plus a direct `std::fs::read_to_string` of the resulting artifact; real `git` subprocess and filesystem, no PTY, no daemon, no real agent).
- **Asserts:** the artifact's content is not merely the old literal `deck` marker, and separately contains the `fnv1a64` hash (hex, same keying scheme `isolated_clone_provenance_path` already uses for the clone path) of the canonicalized source dir, the literal Name string, and the canonicalized clone dir path. `DOT_AGENT_DECK_STATE_DIR` pinned per-test, as `006`-`010`. RED today: the artifact is still literally `b"deck\n"`, so it fails on the very first assertion.
- **Does not assert:** an exact wire format/encoding (none is specified in the PRD beyond "content-minimal, no untrusted payload") — see the tester's `work-done` report for PRD fork#544 M4's RED round for the proposed `key=value` line encoding these assertions are written against; the schema tag itself (`013` covers that).
- **Platform coverage:** mac+linux+windows.

##### orchestration/workspace/012 — A pre-M4 provenance artifact — the literal bytes `b"deck\n"` written directly to disk, bypassing the writer entirely, simulating a workspace created by a build that predates the M4 fields — is still treated as valid M4b ownership evidence: reopening the same Name against the same source still resumes rather than being refused as a stranger directory (PRD fork#544 M4: "Read path must tolerate markers written by builds that predate these fields... same discipline fork#192's M1.2 applied to `display_title`").
- **Layer:** L1 (in-process — create via a real `provision_isolated_clone_sync` call, overwrite the resulting artifact with the raw pre-M4 bytes, release the resume registration, then call `provision_isolated_clone_sync` again; real `git` subprocess and filesystem, no PTY, no daemon, no real agent).
- **Asserts:** the second call reports `Resumed`, not `Rejected(ResumeRejection::Stranger)`, against a hand-written pre-M4 artifact. `DOT_AGENT_DECK_STATE_DIR` pinned per-test, as `006`-`011`. This exercises EXISTING M3 behavior — `resume_existing_isolated_clone`'s eligibility check (b) has only ever tested file presence via `is_file()`, never content — so this may already be green; the tester's `work-done` report states which, plainly.
- **Does not assert:** the other two eligibility checks (ancestry, health — `007`/`008`); anything about what the NEW writer produces (`011`/`013` cover that) — this test deliberately never calls the writer at all.
- **Platform coverage:** mac+linux+windows.

##### orchestration/workspace/013 — After a real `provision_isolated_clone_sync` call writes today's provenance artifact, the artifact's schema tag round-trips as real, readable data rather than being merely present-but-unread (PRD fork#544 M4: "a schema tag... so evidence written under a future naming-scheme change doesn't silently vouch for content created under today's rule").
- **Layer:** L1 (in-process — one real `provision_isolated_clone_sync` call plus a direct `std::fs::read_to_string` of the resulting artifact; real `git` subprocess and filesystem, no PTY, no daemon, no real agent).
- **Asserts:** a `schema=<value>` line is present in the artifact's content and its value equals this test's own stand-in constant for "today's schema" (`"2"`, the tester's proposed value — see the `work-done` report). No second, genuinely pre-M4 schema value exists yet to compare against; per the PRD's own explicit carve-out, this only proves today's tag is real extractable data, not merely present. RED today: no `schema=` line exists at all.
...
##### orchestration/workspace/018 — Opening an orchestration via the real `Action::SpawnPane` → `provision_isolated_clone_sync` path (Model A), then closing its whole tab, surfaces the daemon's tab-close "kept" notice for the isolated clone — the only end-to-end observable proof that Model A's provisioning registered the workspace via `record_worktree`, the same as Models B/C already do (PRD fork#544 M6 Design step 6: "Call `record_worktree` for Model A's isolated clones").
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness, same `orch-clone-gate` fixture as `001`-`005`).
- **Agent:** none (same fixture/mechanism as `001`).
- **Asserts:** opening the orchestration under a fixed Name provisions the workspace at the derived path (as `001`); closing the whole tab via Ctrl+D → Ctrl+W → confirm returns to an empty Dashboard; the status-bar "kept" notice text ("it is an isolated clone, not a linked worktree") appears on screen afterward — this only fires if the daemon's tab-close cleanup handler found a registered `WorktreeRegistry` entry for the workspace, since the registry itself exposes no other query surface; the workspace directory still exists afterward. Today Model A never calls `record_worktree`, so the close-cleanup handler's `take_worktree` finds no entry, `remove_worktree` never runs, and no notice is ever broadcast: RED against it (`wait_for_string` times out waiting for the notice).
- **Does not assert:** the underlying `RemovalPolicy::IsolatedClone` → `Kept` guarantee itself, pinned directly and independent of any PTY/daemon by `019`; which wire mechanism carries the registration across the client/daemon boundary; naming derivation (`001`/`002`); resume (`003`/`004`); the live-name refusal (`005`).
- **Platform coverage:** mac+linux in practice (no `cfg` gate, matching `001`).

##### orchestration/workspace/019 — `remove_worktree`, called against a real, NAMED isolated-clone workspace (provisioned via `provision_isolated_clone_sync` exactly as Model A creates one) under `RemovalPolicy::IsolatedClone`, always reports `Kept` and leaves the workspace directory and its checked-out branch untouched (PRD fork#544 M6 Design step 6: "Pin with a test that tab-close never deletes a named workspace").
- **Layer:** L1 (in-process — one real `provision_isolated_clone_sync` call plus a direct `remove_worktree` call; real `git` subprocess and filesystem, no PTY, no daemon, no real agent).
- **Agent:** none.
- **Asserts:** a freshly-provisioned, named workspace handed to `remove_worktree` under `RemovalPolicy::IsolatedClone` returns `RemoveOutcome::Kept(KeptReason::IsolatedClone)`; the workspace directory still exists afterward; its checked-out branch's HEAD commit is byte-identical before and after. `DOT_AGENT_DECK_STATE_DIR` pinned per-test, as `006`-`017`. This is a REGRESSION GUARD, not a fresh RED: `remove_worktree`'s `RemovalPolicy::IsolatedClone` arm already reports `Kept` unconditionally today (Problem Statement #3) — already GREEN, pinning the guarantee explicitly under this milestone's own catalog family rather than relying solely on `src/dispatch.rs`'s pre-existing `isolated_clone_is_always_kept_on_tab_close_even_when_clean` (written for Model C / issue #490) to cover it by proxy.
- **Does not assert:** whether the workspace was ever actually registered in a live `WorktreeRegistry` in the first place, i.e. whether `remove_worktree` is ever reached at all on Model A's real tab-close path — that's `018`'s own point; the `worktree reclaim --yes` heuristic path (`src/worktree_reclaim.rs`), which is explicitly out of scope for this milestone (issue #546).
- **Platform coverage:** mac+linux+windows, matching `006`.

##### orchestration/workspace/020 — The just-merged workspace, with a completely clean tree whose checked-out branch's HEAD is now fully contained in a genuinely advanced `origin/main` (a real fast-forward merge, not a stand-in), auto-switches to `main` and fast-forwards it to match `origin/main` exactly (PRD fork#544 M7 Decisions table: "This workspace, once its own branch is confirmed merged... auto-switch to updated main (checkout + fast-forward) — conditionally, only if the tree carries nothing beyond what was just merged").
- **Layer:** L1 (in-process — real `git` subprocesses against three real local repositories: an "origin", a clone checked out on a feature branch with one real commit, and origin fast-forwarded onto that commit to simulate the merge landing; no PTY, no daemon, no real agent).
- **Agent:** none.
- **Asserts:** calling the new `sync_merged_workspace_to_main(clone_dir, "main")` against a workspace whose feature-branch HEAD equals origin's now-advanced main returns `Ok(PostMergeSyncOutcome::SwitchedToMain)`; afterward the workspace is checked out on `main`; local `main`'s HEAD commit equals origin's advanced main exactly; the merged file is present in the working tree. `sync_merged_workspace_to_main` and `PostMergeSyncOutcome` do not exist yet (M7 has not implemented them) — a compile-time RED (`cannot find function`/`cannot find type`), mirroring `orchestration/workspace/014`-`017`'s own precedent (M5 also had no existing production entry point to extend behaviorally).
- **Does not assert:** the refusal paths (`021`/`022`); the other-live-workspace read-only-fetch behavior (`023`); how/where this function is actually invoked from the merge phase (`release`'s `/prd-done` flow) — that wiring is left to the coder's own judgment call per this milestone's design latitude.
- **Platform coverage:** mac+linux+windows, matching `006`.

##### orchestration/workspace/021 — The just-merged workspace carries a genuine UNCOMMITTED change the merge never captured — the single biggest silent-data-loss risk this PRD's Risks section names — and `sync_merged_workspace_to_main` refuses the auto-switch entirely, leaving the checked-out branch, its HEAD, and the uncommitted edit itself completely untouched, reporting a non-empty note rather than silently discarding it (PRD fork#544 M9's own coverage list: "refusal to auto-switch when the tree carries anything beyond the merge (uncommitted change OR an extra local commit) — leaves the workspace completely untouched, surfaces a note/signal the caller can observe").
- **Layer:** L1 (in-process — same real three-repository merge simulation as `020`, plus a real uncommitted edit written directly to the working tree after the simulated merge).
- **Agent:** none.
- **Asserts:** `sync_merged_workspace_to_main` against a workspace with a real uncommitted change (the tree is dirty per `git status --porcelain`) returns `Ok(PostMergeSyncOutcome::LeftUntouched { reason })` with a non-empty `reason`, never `SwitchedToMain`; the checked-out branch name, HEAD commit, and the uncommitted file's exact byte content are all identical before and after the call. Compile-time RED, same reason as `020`.
- **Does not assert:** the extra-local-commit half of "carries anything beyond the merge" — isolated separately by `022` so a clean-tree-only implementation can't accidentally pass this test while still missing that case; the successful-switch path (`020`); the other-live-workspace behavior (`023`).
- **Platform coverage:** mac+linux+windows, matching `006`.

##### orchestration/workspace/022 — The just-merged workspace has a fully CLEAN tree but carries a real LOCAL COMMIT made after the content the merge captured (e.g. the orchestration kept working in the workspace before the merge phase ran) — `sync_merged_workspace_to_main` refuses the auto-switch here too, never discarding or stranding the extra commit, isolating the "extra local commit" signal from `021`'s "uncommitted change" signal so a clean-tree-only safety check can't slip through (PRD fork#544 M9's own coverage list, same bullet as `021`; PRD's own Decisions table: "only if the tree carries nothing beyond what was just merged").
- **Layer:** L1 (in-process — same real three-repository merge simulation as `020`, plus one additional real commit made in the clone after the simulated merge, with the tree otherwise fully clean per `git status --porcelain`, confirmed by the test itself before calling the function under test).
- **Agent:** none.
- **Asserts:** `sync_merged_workspace_to_main` against a clean-but-ahead-of-the-merge workspace returns `Ok(PostMergeSyncOutcome::LeftUntouched { reason })` with a non-empty `reason`, never `SwitchedToMain`; the checked-out branch name and the extra commit's HEAD SHA are identical before and after the call — the extra commit is never discarded (e.g. by a hard reset to `origin/main`) nor silently stranded on a branch switch. Compile-time RED, same reason as `020`.
- **Does not assert:** the uncommitted-change half of "carries anything beyond the merge" (`021`'s own point); the successful-switch path (`020`); the other-live-workspace behavior (`023`).
- **Platform coverage:** mac+linux+windows, matching `006`.

##### orchestration/workspace/023 — On an UNRELATED merge — this workspace's own branch did NOT just merge — only a proactive, read-only `git fetch` runs against another live workspace: the `origin/main` remote-tracking ref updates to reflect the genuine advance, but the checked-out local branch and working tree are completely untouched (PRD fork#544 M7 Decisions table: "Other live workspaces, on an unrelated merge: proactive read-only fetch only, branch untouched").
- **Layer:** L1 (in-process — real `git` subprocesses: an "origin", a THIRD independent clone that commits and gets fast-forwarded into origin to simulate an unrelated merge, and a SEPARATE "other" clone standing in for another live orchestration's own workspace, against which the function under test is actually called).
- **Agent:** none.
- **Asserts:** after origin's main genuinely advances (confirmed distinct from the "other" workspace's stale `origin/main` remote-tracking ref beforehand), calling the new `fetch_other_live_workspace(other_clone)` returns `Ok(())`; the checked-out branch name and local HEAD commit in `other_clone` are byte-identical before and after; the unrelated merge's new file is NOT present in `other_clone`'s working tree; `other_clone`'s own `origin/main` remote-tracking ref DOES now equal origin's advanced HEAD — proving the fetch genuinely ran rather than the assertions above passing vacuously because nothing happened at all. `fetch_other_live_workspace` does not exist yet — a compile-time RED, same reason as `020`.
- **Does not assert:** the just-merged workspace's own conditional switch behavior (`020`-`022`); where/how this function is actually invoked from the merge phase.
- **Platform coverage:** mac+linux+windows, matching `006`.

##### orchestration/workspace/024 — A freshly created workspace's `origin/main` remote-tracking ref reflects the TRUE, current state of the real origin repository at creation time — not merely a copy of the root checkout's (`source_dir`'s) own stale knowledge of it, and not merely the clone-time snapshot of `source_dir`'s local `main` branch a plain `git clone` alone would produce (PRD fork#544 M9's own coverage list, "creation-time freshness against origin"; the Decisions table's "Should a fresh clone start from up-to-date main?" row).
- **Layer:** L1 (in-process — real `git` subprocesses against a real, independently-fetchable local "origin" repository and a `source_dir` cloned from it, mirroring `020`-`023`'s own real-fetchable-origin technique rather than `provision_isolated_clone_sync_sets_origin_and_branch_when_source_has_origin`'s unreachable `.invalid` URL, which no fetch could ever prove).
- **Agent:** none.
- **Asserts:** `source_dir` is a real clone of `origin_repo`, so its own `origin/main` ref is frozen at clone time; `origin_repo` is then advanced with a further real commit entirely behind `source_dir`'s back (`source_dir` never re-fetches, confirmed as a setup precondition); a fresh `provision_isolated_clone_sync(source_dir, clone_dir, ...)` call succeeds, and the resulting workspace's own `origin/main` ref equals `origin_repo`'s advanced HEAD, not the stale commit `source_dir`'s own knowledge (and a plain unmodified `git clone` of `source_dir`) would otherwise produce. This is a REGRESSION GUARD, not a fresh RED: the creation path's post-checkout `git fetch origin` (PRD fork#544 M2 fix round, `issue_dispatch_run.rs`'s "issue-dispatch: could not fetch origin for freshness after isolated clone" comment) already runs unconditionally whenever the source has an origin — already GREEN, closing the M9 coverage-list gap left by the existing sibling test's unreachable placeholder URL, which could never actually prove a fetch occurred.
...
- **Agent:** none.
- **Asserts:** a workspace created via `provision_isolated_clone_sync` and then resumed AFTER `source_dir` advances with a real new commit the clone's own object database has never received must not resolve as `Rejected(AncestryMismatch)` — pinning the CORRECT direction as a property (resume succeeds, or at minimum is not misreported as a wrong repo); a resume attempt (successful or refused) must never itself mutate the workspace's checked-out HEAD. GREEN: `isolated_clone_ancestry_matches_source` now runs a symmetric `git merge-base <source HEAD> HEAD`, preceded by a best-effort fetch of `source_dir`'s own new commits into the workspace, so an ordinary advance on `source_dir` resolves as shared history rather than a mismatch.
...
##### orchestration/workspace/028 — `sync_merged_workspace_to_main` must refuse a `default_branch` git itself would never let a real branch be created with (`--detach`) up front, before touching the workspace at all, rather than reaching a checkout/switch call that has to be made SAFE against it (PRD fork#544 review-findings fix round, auditor A5; RE-SCOPED in fix round 3, auditor F3 / reviewer S4, once the earlier fix round's missing-`--`-separator bug this test originally pinned was closed by switching to `git switch --`).
...
- **Agent:** none.
- **Asserts:** with `refs/remotes/origin/--detach` planted at the clone's own current HEAD and `default_branch = "--detach"`, the call returns `Err` naming `"--detach"` — pinned via `git check-ref-format --branch`, which itself rejects `"--detach"` as not a valid branch name, run before anything else in the function — and the workspace is left completely untouched: still on its starting branch (not detached, not switched onto anything), and no real local branch literally named `--detach` was ever created via the `update-ref` route further down.
- **Does not assert:** the ordinary (non-adversarial) `SwitchedToMain` path, already covered by `020`; the sibling `merge` call's own `--` separator (already present, not under test here); the missing-upstream-tracking case for a validly-named `default_branch`, covered by `032`; the checkout/switch call's own `--` end-of-options separator (auditor's original A5 finding) — no longer covered by ANY test now that the up-front `check-ref-format` validation makes that code path unreachable for an adversarial `default_branch` like `--detach`.
...
##### orchestration/workspace/031 — `isolated_clone_ancestry_matches_source`'s best-effort fetch failing for a reason unrelated to the two repositories actually being unrelated (here, a permission error reading the source's own new commit object) must not be reported the identical way as a genuine, proven wrong-repo/stranger mismatch (PRD fork#544 review-findings fix round, reviewer S1: the function's own code comment already distinguishes exit 1 = proven mismatch from exit 128 = inconclusive, but the `match` collapses both into the same `AncestryMismatch`, whose user-facing text advises destructive removal of a workspace that is provably genuinely derived from source).
- **Layer:** L1 (in-process — real `git` subprocesses; forces the fetch failure by stripping read permission from the source's own new commit's loose object file, `#[cfg(unix)]`, verified directly against the real `git` binary before writing the test that `rev-parse HEAD` — a plain ref resolution — is unaffected while the fetch, which must transmit that exact object, fails).
- **Agent:** none.
- **Asserts:** after `source_dir` genuinely advances past the clone (as `025`) and the new commit's object is made unreadable, resuming must not resolve as `Rejected(AncestryMismatch)` — pinning the property that an inconclusive probe is distinguishable from a proven mismatch, not a specific new enum variant, since coder's exact naming is left to them; a resume attempt (successful or refused) must never itself mutate the workspace's checked-out HEAD. `DOT_AGENT_DECK_STATE_DIR` pinned per-test, as `006`-`030`. GREEN: `isolated_clone_ancestry_matches_source` now runs `merge-base` first with no prior fetch, and only falls back to a best-effort fetch-then-retry on a genuinely inconclusive exit 128; anything still inconclusive after that resolves as the new `ResumeRejection::AncestryUnverifiable`, never folded into `AncestryMismatch`.
- **Does not assert:** the proven wrong-repo case where ancestry genuinely never held (`007`, `025`, unaffected by this fix); which specific new `ResumeRejection` variant (or message form) coder chooses to distinguish the inconclusive case.
- **Platform coverage:** linux (`#[cfg(unix)]` — the fixture relies on POSIX file-permission enforcement, which the Windows permission model does not provide the same way).

##### orchestration/workspace/032 — `sync_merged_workspace_to_main`'s missing-local-branch path creates it via a bare `git update-ref -- refs/heads/<b> origin/<b>` (chosen specifically to sidestep `check-ref-format`'s DWIM-creation validation, per Fix 3 / `028`), which — unlike ordinary DWIM checkout — configures no upstream tracking at all, contradicting the PRD's own Decisions table reason for the read-only fetch mechanism ("so ahead/behind info stays accurate") (PRD fork#544 review-findings fix round, reviewer S3).
- **Layer:** L1 (in-process — real `git` subprocesses, mirroring `020`'s own real-fast-forward-merge simulation technique, with the clone's local `main` branch explicitly deleted before the sync call so `local_branch_exists` is genuinely false going in — the reviewer's own note that this is the ordinary case, not an edge one).
- **Agent:** none.
- **Asserts:** after a successful `sync_merged_workspace_to_main` call that creates the local `main` via the `update-ref` path (confirmed by setup: local `main` did not exist beforehand, and the call reaches `SwitchedToMain`), `git rev-parse --abbrev-ref @{upstream}` for that branch resolves cleanly afterward rather than erroring. `DOT_AGENT_DECK_STATE_DIR` not needed (no isolated-clone provenance involved, matching `020`-`030`). GREEN: a `git branch --set-upstream-to=origin/<b> -- <b>` call now follows the `update-ref` immediately, so `branch.main.remote`/`branch.main.merge` end up set.
...
- **Does not assert:** the `None`-owner case (an orchestration that owned no worktree restores with no identity — implicit in `session/restore/016`/`017`'s snapshots, which all set `owner: None`); a pre-M3.0 snapshot with no `owner` key at all deserializing to `None` (covered by `config/saved-session/001`); the live-create path that first populates `owner` onto the snapshot when the tab is opened (not independently pinned — `orchestration_identity_014` and `orchestration_identity_015` cover the same `creator` local reaching the marker/env, but not this specific snapshot-field write); real daemon/PTY spawn.
- **Platform coverage:** mac+linux+windows.

##### session/restore/020 — On the restore path, `DOT_AGENT_DECK_WORKTREE_OWNER` reaches EVERY restored role pane's genuinely spawned process environment — not merely a test double's recorded field, and not only the start role (PR #215 review round follow-up, findings-215-restart-manual.md).
...
##### session/live/013 — Rehydration preserves the PRD #370 synthetic-`Working` provenance, so a card reconnected mid-`ShellBusy` is still reverted by the paired `ShellIdle` (fork issue #21).
...
##### session/live/014 — A `ShellIdle` broadcast in the window between the `ListAgents` snapshot and the TUI's event stream coming up still reaches the rebuilt card (fork issue #36).
...
- **Does not assert:** the provenance-marker half of the story (`session/live/013` — the marker surviving the wire); recovery of events lost during a LATER subscription outage (`session/live/015`, `session/live/016` — issues #49/#28); rendered card output; a real daemon's timing.
- **Platform coverage:** mac+linux.

##### session/live/015 — A `SubscribeEvents` connection that dies mid-session with the daemon's documented `KIND_STREAM_END "lagged"` tear-down is recovered by the reconnect loop draining a fresh `list_agents` snapshot, not left stuck on the pre-outage status (issues #49/#28).
- **Layer:** L1 client/wire integration — the real production reconnect bootstrap (`reconnect::run_event_subscriber` + `EmbeddedPaneController::hydrate_from_daemon` + `AppState::seed_hydrated_session`) driven against a mock daemon over a real Unix socket.
...
- **Does not assert:** the BOOTSTRAP snapshot/subscribe window (`session/live/014`, fork issue #36 — a different, already-closed hazard); the daemon-side `"lagged"` reason itself, pinned in isolation by `daemon/protocol/001`; the no-reason transport-drop shape of the same hazard (`session/live/016`); rendered card output; a real daemon's timing.
- **Platform coverage:** mac+linux.

##### session/live/016 — A `SubscribeEvents` connection that dies mid-session WITHOUT a `lagged` reason — the connection just drops, standing in for a daemon restart or a bare transport failure — is recovered by the same reconnect re-hydration as the `lagged` case (issues #49/#28).
- **Layer:** L1 client/wire integration — same production reconnect bootstrap and mock-daemon harness as `session/live/015`.
- **Agent:** none (same mock daemon; no PTY, no LLM).
- **Asserts:** identical setup and outage-window construction to `session/live/015`, except the mock daemon's forced tear-down sends no `KIND_STREAM_END` frame at all — it just drops the connection (EOF), the shape the approved design ("always re-hydrate on reconnect", not just on `lagged`) exists to cover. The reconnected card must still converge on `Idle` off a fresh `list_agents` snapshot.
- **Does not assert:** the `lagged`-reason shape of the same hazard (`session/live/015`); the BOOTSTRAP snapshot/subscribe window (`session/live/014`); which client-side mechanism recovers the snapshot (only the observable end state — the status the TUI holds for the card — not internal call counts).
- **Platform coverage:** mac+linux.

##### session/live/017 — Rehydration preserves a session's known model across a detach/reconnect, and the reconnected card still renders it (PRD fork#378 reviewer/auditor round 2, HIGH 1 / F8).
...
##### scheduler/idle-worker/014 — After the orchestrator's process ends ON ITS OWN — no StopAgent, so no close-transition sweep — issue #465's EOF sweep retires the worker's record anyway, before an unrelated agent inheriting the pane id can receive anything (PRD #126 M1 audit finding 2; issue #465 F1 re-scope).
...
- **Asserts:** two preconditions that stop it passing for the wrong reason — the orchestrator pane is NOT in a close transition after the exit (so the close-time sweep is provably not what suppresses the prompt), and the successor owned the pane before the delegation's deadline (so a stray timer had a live target to mis-deliver to) — then, positively, that `AgentPtyRegistry::retire_outstanding_delegation` reports nothing left to retire for the worker's pane (proving the issue #465 EOF sweep on the orchestrator's own reader thread already drained it, since the orchestrator's registry entry lingers `contains_key`-present — only its `exited` flag is set — until that same thread observes EOF) — then that after two further timeout windows the successor's PTY carries its own readiness marker, zero occurrences of the daemon clause, and no fragment of the dead orchestration's role name.
- **Does not assert:** the pane-reuse-after-`StopAgent` path (covered by `scheduler/idle-worker/008`); the orchestration-membership half of the delivery revalidation; the identity-gate path this test covered before issue #465 added the EOF sweep — a reachable, deliberately-uncovered registry entry now covers that (`scheduler/idle-worker/018`).
- **Platform coverage:** mac+linux.

##### scheduler/idle-worker/018 — Removing the ORCHESTRATOR's registry entry directly (bypassing `begin_pane_close` entirely, so neither the close-transition sweep nor issue #465's EOF sweep ever runs) still leaves an unrelated agent inheriting its pane id receiving nothing — the `write_and_submit_guarded` agent-id gate is the only guard left standing on this path (issue #465 F1 review).
- **Layer:** fast integration; `AgentPtyRegistry::close_agent` is called directly on the orchestrator's registry id (removes the entry, then kills the child — never through `begin_pane_close`/`StopAgent`), after which a raw/no-echo `cat` takes the freed `pane_id_env`.
- **Agent:** none (`cat` worker stand-in; the successor is raw/no-echo so any submitted byte is directly observable in its scrollback).
- **Asserts:** a precondition that stops it passing for the wrong reason — the orchestrator pane is NOT in a close transition after `close_agent` (proving neither sweep ran: the close-transition sweep because `begin_pane_close` was never called, and the issue #465 EOF sweep because the entry was already gone before the killed child's own EOF, so `is_agent_still_registered` reads false) — and that the successor owned the pane before the delegation's deadline; then that after two further timeout windows the successor's PTY carries its own readiness marker, zero occurrences of the daemon clause, and no fragment of the dead orchestration's role name.
- **Does not assert:** the natural-exit path where the EOF sweep DOES run (covered by `scheduler/idle-worker/014`); the pane-reuse-after-`StopAgent` path (covered by `scheduler/idle-worker/008`); the orchestration-membership half of the delivery revalidation (the successor is spawned without `tab_membership`, so that check legitimately abstains and the agent-id gate is what refuses).
- **Platform coverage:** mac+linux.

##### scheduler/idle-worker/019 — A respawn driven directly through `AgentPtyRegistry::respawn_agent_for_pane` carries an already-armed outstanding idle-worker delegation AND silence watch forward instead of sweeping either (issue #465 F3).
- **Layer:** fast integration; a real, non-zero `WORKER_RESPONSE_TIMEOUT_MS` (and its derived silence-watch window) arms both records via a real `AppState::handle_delegate`, then `respawn_agent_for_pane` is called directly on the worker's pane — the same primitive `dispatch_one_owned` uses for a `clear = true` delegate, isolated here from the SessionStart/readiness machinery that wraps it in the real delegate path.
- **Agent:** none (`cat` worker stand-in, respawned to a fresh `cat`).
- **Asserts:** after the respawn completes, both `AgentPtyRegistry::retire_outstanding_delegation` and `AgentPtyRegistry::retire_silence_watch` report a record was still present for the worker's pane — proving `respawn_agent_for_pane`'s no-sweep contract (the doc comment's claim that an outstanding delegation "carries forward" to whichever agent next occupies the pane) actually holds, rather than being an assumption every `clear = true` test to date happened to disable both watches around.
- **Does not assert:** which agent identity the carried-forward record ends up bound to, or that a subsequent timer correctly delivers to the NEW agent (that is the ordinary idle-worker/silence-watch delivery path, covered elsewhere); the real `clear = true` delegate path's SessionStart-wait/readiness-buffer sequencing around the respawn (covered by `orchestration/delegate/007`/`008`/`010`/`011`).
- **Platform coverage:** mac+linux.
...
##### scheduler/idle-worker/020 — A worker pane whose process exits on its own (PTY EOF), never calling work-done, has its armed idle-worker delegation AND silence watch retired promptly — not left armed for the rest of their timeout window (issue #465 M1) — and the orchestrator is told, promptly, via a new fixed-text notice (issue #465 M2).
...
- **Asserts:** a fixture precondition that the worker pane is still live immediately after `delegate()` arms both the outstanding idle-worker delegation and the silence watch (so the retirement below is caused by the EOF that follows, not by the pane never having held a live agent); then, once the registry observes the pane is no longer live, that a short fixed grace period later (1s, against an 8s armed window — far short of the 120-minute/30-second production defaults) both `AgentPtyRegistry::retire_outstanding_delegation` and `AgentPtyRegistry::retire_silence_watch` report nothing left to retire for that pane, proving `pump_reader`'s EOF observation already swept both records; and, waiting up to a further 3s (returning as soon as it appears, not sleeping out the full window), that the orchestrator pane's scrollback carries the daemon-authored "exited without work-done" notice naming the exited worker's pane id.
- **Does not assert:** the exact notice prose beyond its daemon-provenance opening clause and the pane id; the idempotent late-work-done-vs-exit race (covered by `021`).
- **Platform coverage:** mac+linux.

##### scheduler/idle-worker/021 — A worker that calls work-done immediately before its process exits on its own must NOT also produce the EOF-triggered "exited without work-done" notice (issue #465 M3).
- **Layer:** fast integration; the worker is the same PTY-backed exiting shell stub as `020` (`printf` a readiness marker, `sleep 1`, `exit 0`), so the only difference from `020` is that `work_done()` fires while the stub is still alive, before its own `sleep 1` elapses.
- **Agent:** none (same deterministic exiting shell stand-in as `020`).
- **Asserts:** a fixture precondition that the worker pane is still live immediately after `delegate()` (so the race below has both paths genuinely available); then, after `AppState::handle_work_done` retires both records and the stub's own natural exit follows, that a 1s settle period later the orchestrator pane's scrollback carries neither the M2 exit notice nor the PRD #126 idle-worker prompt for this worker's role — the EOF sweep that follows `work_done()` finds nothing left to sweep, exactly once, and does not treat an already-retired record as a fresh "exited without reporting" event.
- **Does not assert:** the reverse ordering (EOF strictly before any `work-done`, which `020` already covers) or a race won by `close_agent`/`StopAgent`'s own close-transition sweep, which removes the registry entry before EOF and so never reaches `pump_reader`'s EOF branch at all — deliberately out of scope, see the test's own comment on why `close_agent` was rejected as the exit mechanism here.
- **Platform coverage:** mac+linux.
...
##### scheduler/live/003 — A live-surfaced scheduled card shows the schedule's friendly name, not the truncated spawn pane-id (PRD #127 finding #2 regression).
- **Layer:** L2 (real TUI driven via PTY; observed on the rendered vt100 grid). Fixture global `schedules.toml` via `DOT_AGENT_DECK_SCHEDULES`; fired with the `RunNow` control message over the deck's attach socket. The schedule's `working_dir` basename (`runbox`) is deliberately unrelated to its name (`morning-digest`) so the friendly name can only reach the grid through the card's identity row — not the Dir line. (PRD fork#405 M1 moved identity off the title onto its own body row; this test's needle is a whole-grid match and does not pin which row.)
- **Agent:** none (a plain `cat` command — no hooks; the card surfaces via the new-agent broadcast as in `scheduler/live/001`).
- **Asserts:** after a fire into the attached daemon, the agent is registered under its friendly name (precondition) and the card surfaces live (its Dir line shows the cwd basename), AND the card shows the friendly name `morning-digest` — matching a reconnect — and NOT the truncated spawn pane-id form (`… sched-morni…`).
- **Does not assert:** the surfacing path itself (covered by `scheduler/live/001`); focus survival (covered by `scheduler/live/002`); identity after a reconnect (which already masks the bug via startup hydration); the card's status badge / body layout.
- **Platform coverage:** mac+linux.
...
- **Asserts:** after the placeholder surfaces with the friendly name `morning-digest` and the real hook supersedes it (the "No agent" placeholder becomes a live ClaudeCode card), the card STILL shows `morning-digest` (matching a reconnect) and has NOT reverted to the session-id hash form (`9f8e7d6c-5b…`).
- **Does not assert:** the surfacing path itself (covered by `scheduler/live/001`); focus survival (covered by `scheduler/live/002`); the no-hook case (covered by `scheduler/live/003`); identity after a reconnect (which already masks the bug via startup hydration); the card's status badge / body layout.
- **Platform coverage:** mac+linux.
