...
- **Note:** M1.0/M1.1 landed — `claim_issue` writes the `in-progress` label via `gh issue edit --add-label` and posts the claim comment via `gh issue comment`; GREEN today across all assertions, including the two-field identity — production already canonicalises the worktree path before rendering it (`canonicalize_identity_path`), so this only strengthens the test itself to require both identity fields instead of the branch alone, closing the gap where a regression that dropped or substituted the path would have passed undetected.
- **Platform coverage:** mac+linux.
