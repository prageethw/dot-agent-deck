...
- **Does not assert:** the suggestion/refusal MECHANISMS in isolation (covered by `orchestration/identity/002`/`003`); the blank-slug collision refusal itself, which the second open here is careful to avoid rather than exercise (that's `orchestration/worktree/016`'s job); the CONTENT of the worktree the second open's typed-slug arm creates, or its ownership marker (covered by `orchestration/worktree/005`, `worktree/reclaim/022`/`023`).
