...
##### dashboard/grid/001 — All 7 role cards paint on a narrow, short 99×33 terminal (issue #437).
...
##### dashboard/grid/003 — Dashboard title shows an explicit hidden-card count when even the fitted layout can't show everything (issue #437).
- **Layer:** L1 (in-process `TestBackend` render).
- **Agent:** none.
...
##### orchestration/delegate/031 — A `delegate` that resolves to ZERO targets (a role name with no registered pane) must not report success (fork #92 P1).
...
##### orchestration/delegate/032 — A `delegate` that self-targets the orchestrator's OWN role name also resolves to ZERO targets and must not report success (fork #92 P1).
- **Layer:** fast real-CLI-subprocess integration (same technique as `orchestration/delegate/031`).
- **Agent:** none.
- **Asserts:** with the orchestrator pane registered under role `orchestrator` and a `coder` worker also present, running `delegate --to orchestrator` from the orchestrator pane exits NON-ZERO. `delegate_targets` deliberately excludes any pane in `orchestrator_pane_ids` from role resolution, so this resolves to nothing even though a pane with that exact role name exists — the same false-positive-confirmation hazard as `orchestration/delegate/031`, from a different resolution path.
- **Does not assert:** exit-code value, message wording, or `DelegateResponse` field shape.
- **Platform coverage:** mac+linux (unix-only — spawns a real daemon subprocess).

##### orchestration/delegate/033 — A `delegate --to coder --to coder` (a duplicated target role) must confirm the role exactly ONCE, not once per request (fork #92 P2).
- **Layer:** fast real-CLI-subprocess integration (same technique as `orchestration/delegate/031`).
- **Agent:** none.
...
##### orchestration/identity/010 — For a given live orchestration, the string `mark_worktree_owned` writes into the worktree's `created-by:` marker and the string every role pane's `AgentSpawnOptions::owner` carries are the LITERAL SAME computed string, from one source — not two derivations of one input (fork #166 M2.4).
...
- **Asserts:** the spawned child's own stdout — read back over the attach socket via `common::wait_for_pane_text_on`, itself reading `$DOT_AGENT_DECK_WORKTREE_OWNER` out of the child's OWN environment — contains the exact owner string `create_pane_with_options` was called with. This is the join `orchestration/identity/010`'s mock cannot exercise: producer (`AgentSpawnOptions::owner`) and reader (`worktree/reclaim/024`–`028`, which set the variable by hand on a subprocess) were each tested; this is the missing middle.
...
- **Does not assert:** the daemon-side wiring of the pre-rebind rollback release into `Action::SpawnPane`'s spawn-failure path (covered at the integration level by `orchestration/identity/018`); the confirm-side pane-id validation (`orchestration/identity/014`'s scope note applies here too).
- **Platform coverage:** mac+linux+windows (pure `Mutex`/`HashMap` logic, no PTY).

##### orchestration/identity/016 — Fork issue #201 redesign (reviewer P1-1/P1-2, auditor A1): a refused orchestration-name claim must happen BEFORE any role pane is spawned and before any tab is pushed, so a lost race leaves NOTHING behind to roll back — no ghost tab, no orphaned panes.
...
##### orchestration/identity/017 — Fork issue #201 redesign: the orchestration-name claim must be the daemon's FIRST request, sent before any role pane spawns (using a caller-minted token, since no real pane id exists yet) — the ORDERING half `identity_016`'s refusal-only test can't isolate on its own.
...
##### session/live/015 — Rehydration preserves a session's known model across a detach/reconnect, and the reconnected card still renders it (PRD fork#378 reviewer/auditor round 2, HIGH 1 / F8).
...
##### scheduler/idle-worker/016 — A worker pane whose process exits on its own (PTY EOF), never calling work-done, has its armed idle-worker delegation AND silence watch retired promptly — not left armed for the rest of their timeout window (issue #465 M1) — and the orchestrator is told, promptly, via a new fixed-text notice (issue #465 M2).
...
- **Does not assert:** the exact notice prose beyond its daemon-provenance opening clause and the pane id; the idempotent late-work-done-vs-exit race (covered by `017`).
- **Platform coverage:** mac+linux.

##### scheduler/idle-worker/017 — A worker that calls work-done immediately before its process exits on its own must NOT also produce the EOF-triggered "exited without work-done" notice (issue #465 M3).
- **Layer:** fast integration; the worker is the same PTY-backed exiting shell stub as `016` (`printf` a readiness marker, `sleep 1`, `exit 0`), so the only difference from `016` is that `work_done()` fires while the stub is still alive, before its own `sleep 1` elapses.
- **Agent:** none (same deterministic exiting shell stand-in as `016`).
