...
##### orchestration/worktree/014 — The Nth-concurrent orchestration against the SAME root checkout, spawned while an earlier one is still live, provisions its OWN isolated clone rather than a `git worktree add` sibling of the shared checkout — the 1st orchestration is unaffected (PRD fork#325 M3, the `Action::SpawnPane`-time repurposing of `live_orchestration_in_same_cwd` from a cosmetic warning into a correctness gate).
...
- **Asserts:** opening the fixture's one orchestration twice against the same launch directory, each with a distinct typed worktree slug, the second while the first is still live: the 1st instance's actual working directory (read back from the log by its owner string) shares the launch dir's `git rev-parse --git-common-dir`; the 2nd instance's does NOT — it must resolve to a distinct git common dir (its own object store), proving it was NOT provisioned as a linked worktree of the shared checkout the way `create_worktree_sync` does today unconditionally.
- **Does not assert:** the fail-open-vs-fail-closed behavior of the gate's own live-orchestration query when the daemon is down/unreachable — the PRD flags this as its own, separately-resolved design decision (Design step 4), and this test only exercises the up-daemon path; a 3rd orchestration colliding with two already-live ones (the PRD's own stated goal for M5's eventual full incident-shape coverage — this is M3's simplest 2nd-against-a-live-1st reproduction); Model B (`dispatch.rs`)'s equivalent gate, which has no live-orchestration query today and needs one added separately.
...
##### orchestration/worktree/015 — The Nth-concurrent-orchestration gate FAILS CLOSED (refuses the spawn) when it cannot reach the daemon to confirm no live sibling orchestration exists, rather than silently proceeding as if none does (PRD fork#325 M3's own Design step 4 decision — `014`'s own "Does not assert" explicitly leaves this case to a separate test).
...
- **Agent:** none.
- **Asserts:** submitting an orchestration form with a typed worktree slug against a directory that IS a genuine, valid git repository (unlike `002`'s deliberately-broken one — the daemon-unreachable gate is the only thing that can refuse this spawn) while the attach socket is redirected to an unlistened path: the orchestration tab is never opened (`tm.active_tab()` stays `Tab::Dashboard`), no role pane is spawned, a non-empty status message surfaces the refusal, and the resolved worktree/clone path is never created on disk — proving the refusal happens BEFORE any provisioning attempt (shared or isolated) touches the filesystem, not merely that provisioning later fails.
...
##### orchestration/worktree/016 — Three concurrent orchestrations against the SAME root checkout — issue #325's own actual incident shape, not `014`'s reduced 2-orchestration case — reproduce and prove closed both of the issue's concrete failures: one orchestration cannot `git worktree remove` another's directory it doesn't own, and a shallow fetch performed against the shared checkout cannot reach either isolated clone's object store (PRD fork#325 M5).
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness — inherently about real concurrent git/filesystem behavior, not renderable in an L1 widget test, per the PRD's own test plan).
...
- **Asserts:** opening the fixture's one orchestration three times against the same launch directory (distinct typed worktree slugs, the 2nd and 3rd each while an earlier one is still live): structurally, the 1st instance shares the launch dir's `git rev-parse --git-common-dir` exactly as today, while the 2nd and 3rd each resolve to their OWN distinct common dir — no two of the three sharing one, proving the 2nd and 3rd are isolated from EACH OTHER, not just from the 1st. Behaviorally, reproduces issue #325's own two concrete failures directly: (1) `git worktree remove` attempted cross-repo in EVERY direction the incident named — from the 1st orchestration's directory against the 2nd's and 3rd's, from the 2nd's against the 3rd's, AND from the 2nd's (an isolated clone) against the 1st's own shared-checkout worktree, the exact direction issue #325 reported (an orchestration deleting worktrees belonging to the root checkout) — fails outright every time, each specifically with a `… is not a working tree` stderr (asserted, not merely that the removal did not succeed), leaving every target's directory, branch, and HEAD commit byte-identical to before the attempt; (2) a real `git fetch --depth 1` performed in the 1st orchestration's directory makes the launch directory's shared object store shallow (`git rev-parse --is-shallow-repository` becomes `true`, read from a DIFFERENT worktree than the one that fetched, exactly as the issue's own repro), while the 2nd and 3rd orchestrations' isolated clones remain full, non-shallow repositories with unchanged HEAD commits.
- **Does not assert:** the fail-open-vs-fail-closed behavior of the gate's own live-orchestration query when the daemon is down/unreachable (`015`'s job); the origin-URL/branch-naming fixes in isolation (`014`'s own assertions, extended here rather than re-derived); recovery/repair of a shallow repository once corrupted (issue #325's own `git fetch --unshallow` repair procedure — out of scope, this test proves the corruption cannot spread, not how to undo it once it happens); Model B (`dispatch.rs`)'s equivalent gate, which has no live-orchestration query today; that `worker-agent-deck worktree reclaim`, production's own wrapper around the same `git worktree remove` this test invokes directly, also declines to touch an isolated clone (that is `isolated_clone_report`'s `KIND_ISOLATED_CLONE` verdict, covered by M4a's own tests).
...
##### orchestration/worktree/018 — A blank-slug orchestration spawn against a root checkout that a live sibling orchestration's own directory exactly matches auto-generates a slug and provisions an isolated clone, instead of refusing (issue #521: the fix direction agreed for issue #489's original blank-slug refusal — the blank-slug `None` arm should match the typed-slug `Some(worktree_path)` arm's existing auto-isolate behavior (`014`) rather than asking the user to type a slug by hand). Formerly `orchestration/worktree/017` pinned the opposite (refusal) behavior for this identical scenario; `017` was removed by this fix — the two assertions could not coexist — and `018` is now the sole spec test for this scenario.
...
- **Agent:** none.
- **Asserts:** submitting a blank-slug (`orchestration_worktree_path: None`) orchestration form against a genuine, one-commit git repo, while a stubbed daemon reports a live sibling orchestration whose `orchestration_cwd` is that same repo: an orchestration tab opens (`tm.active_tab()` is not `Tab::Dashboard`), every role pane is spawned, each role pane's cwd is a freshly created directory distinct from the source repo with its own real `.git` directory (proving a genuine isolated clone, not a linked worktree sharing the source's object store), and the status message does not carry the manual-slug refusal wording the removed `017` used to pin.
- **Does not assert:** the exact auto-generated slug/branch naming convention the fix chose (`auto_generate_worktree_slug`'s `orchestrator-N` pattern, mirroring `NewPaneFormState::suggest_orchestration_name`'s `<foldername>-orchestrator-N` — this test only checks the resulting clone is distinct and real, not the literal slug text); the typed-slug collision path itself (that's `014`'s job); the daemon-unreachable fail-closed path (that's `015`'s job); origin-URL fixup or ownership-marker content of the auto-isolated clone (covered by `014`'s and `provision_isolated_clone_sync`'s own unit tests in `issue_dispatch_run.rs`).
- **Platform coverage:** mac+linux+windows (no `#[cfg(unix)]` — the in-process stub attach server and its Windows named-pipe fallback are shared with `014`/`015`'s helpers; the real `git clone`/`git checkout` subprocesses it also runs are already exercised cross-platform by `provision_isolated_clone_sync`'s own tests).
