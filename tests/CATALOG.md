...
- **Asserts:** opening a real orchestration tab A renders the pane column's left edge at the default 34%-width boundary; since opening a tab leaves the deck in PaneInput mode focused on its start-role pane and PRD #387 M1 claims `Ctrl+l` only in command mode, each toggle press is preceded by `Ctrl+D` to enter Normal mode; `Ctrl+l` then moves that boundary to the narrower-sidebar 25%-width position; opening a SECOND real orchestration tab B in the same directory renders it at its OWN default 34%-width boundary regardless of tab A's now-Narrow stage; cycling tab B through Narrow then Hidden (edge at column 0, sidebar fully collapsed) and switching back to tab A (Shift+Tab, which needs no extra `Ctrl+D` since the deck is already in Normal mode from the toggle above) confirms tab A's stage is STILL Narrow — untouched by tab B's presses; finishing tab A's own cycle (Narrow -> Hidden -> Default) restores its original 34%-width boundary, proving the 3-stage loop and per-tab isolation together.
...
- **Does not assert:** the render-loop call site itself actually applying the result to a live pane controller / render pass (would mirror `tabs/orchestration/011`'s technique for `auto_focus_waiting_pane`, deferred — not required by PRD #373 M1's stated scope); the 30-second inactivity timer (PRD #373 M2); the blocked-keystroke reset (covered by `tabs/orchestration/019`).
- **Platform coverage:** mac+linux+windows.
...
- **Does not assert:** the render-loop call site wiring `UiState::last_pane_keystroke_at` into the `now`/`last_activity_at` arguments each frame (coder's follow-up, not this unit test); the blocked-keystroke reset on a locked pane (covered by `tabs/orchestration/019`).
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/014 — The M2 30-second inactivity snap-back's `src/ui.rs` render-loop wiring — the three `last_pane_keystroke_at` focus-landing stamp sites plus the per-frame `else if` call site — actually reaches the pane controller, not just `TabManager::auto_focus_after_inactivity` in isolation; also pins the M2 cross-tab scope-leak fix (`441f043`), where a Dashboard-tab card click must NOT be able to suppress an otherwise-due Orchestration-tab snap-back (PRD #373 M2, fork-only; closes the coverage gap flagged after `tabs/orchestration/013`, mirroring `tabs/orchestration/011`'s technique for the M2 wiring).
- **Layer:** L1 (in-process unit test; `src/ui.rs`'s own `#[cfg(test)]` module, same as `tabs/orchestration/011`, because `mirror_selection_into_focus` is module-private).
- **Agent:** none (`OpenTabPC` mock `PaneController`; synthetic `SessionStatus` map and `Instant`/`Duration` values, no panes/PTYs).
- **Asserts:** against a real `TabManager`-opened 2-role Orchestration tab, calling the REAL `mirror_selection_into_focus` to land focus on the non-orchestrator `coder` role — with no `Action::ForwardToPane` ever run in the test — stamps `ui.last_pane_keystroke_at` on its own, exactly as a forwarded keystroke would; replaying `src/ui.rs`'s real per-frame `else if` chain (`auto_focus_waiting_pane` → `auto_focus_all_clear` → `auto_focus_after_inactivity`, reading the active tab's OWN `last_role_pane_activity_at` field, mirroring `src/ui.rs:10377-10398` verbatim) with a synthetic 31-second-stale timestamp applies the snap-back through the SAME pane controller `mirror_selection_into_focus` focused `coder` on, landing it on the orchestrator role. It then pins the fixed scoping behavior: `Action::SelectCard` (`src/ui.rs:7265-7287`) calls `mirror_selection_into_focus` unconditionally regardless of active-tab kind — only the `selected_session_id` write is gated on `Tab::Dashboard` — so a plain Dashboard-tab card click stamps the GLOBAL `ui.last_pane_keystroke_at`, but must leave the Orchestration tab's OWN `last_role_pane_activity_at` untouched. With `coder` re-focused and its own `last_role_pane_activity_at` genuinely 31+ seconds stale, a Dashboard-tab click (mirroring the real `switch_tab_with_focus` / `TabManager::restore_focus_on_switch_in` tab-switch sequence) resets only the global timestamp; switching back to the Orchestration tab and replaying the per-frame chain again shows the snap-back STILL fires — `coder` loses focus to the orchestrator role — because the chain reads the per-tab field's genuine staleness, not the fresh-but-irrelevant Dashboard stamp.
- **Does not assert:** the resolver's own resolution rules for `auto_focus_waiting_pane`/`auto_focus_all_clear` (covered by `tabs/orchestration/010`/`012`) or `auto_focus_after_inactivity`'s own timing rules in isolation (covered by `tabs/orchestration/013`); any visible rendering (no `compute_frame_layout`/`render_frame` pass here, unlike `tabs/orchestration/011` — this test asserts on the pane-controller-level focus outcome only); the blocked-keystroke reset (covered by `tabs/orchestration/019`); the `dispatch_normal_mode_key` arrow/j/k role-pane cycling caller of `mirror_selection_into_focus`, which does NOT stamp `last_role_pane_activity_at` (a real remaining gap — covered by `tabs/orchestration/015`).
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/015 — The arrow/Tab/j/k role-pane cycling path (`dispatch_normal_mode_key` → `mirror_selection_into_focus`, `src/ui.rs:5067-5080`, invoked from `run_tui` at `src/ui.rs:8951`) is a FOURTH caller of `mirror_selection_into_focus` beyond the three the M2 wiring fix named, and does not stamp `Tab::Orchestration::last_role_pane_activity_at` when it lands focus on a different role pane (PRD #373 M2, fork-only; gap flagged by `coder` after `441f043`).
- **Layer:** L1 (in-process unit test; `src/ui.rs`'s own `#[cfg(test)]` module, same as `tabs/orchestration/011`/`014`).
- **Agent:** none (`OpenTabPC` mock `PaneController`; synthetic `Instant`/`Duration` values, no panes/PTYs).
...
- **Does not assert:** the resolver's own resolution rules (covered by `tabs/orchestration/010`/`012`/`013`) or the render-loop `else if` chain's consumption of the field (covered by `tabs/orchestration/014`); any visible rendering; the blocked-keystroke reset (covered by `tabs/orchestration/019`).
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/016 — Pins the fix for a review finding: the per-frame three-way chain (`auto_focus_waiting_pane` → `auto_focus_all_clear` → `auto_focus_after_inactivity`, `src/ui.rs` ~10546-10575) must not fight `auto_focus_waiting_pane` once it has already steered focus onto a role that is STILL `WaitingForInput` — `auto_focus_after_inactivity` now no-ops while `TabManager`'s `had_waiting_pane` edge-trigger flag is set, honoring the PRD's "no fighting between this behavior and `auto_focus_waiting_pane`" success criterion (PRD #373 M2, fork-only; fixed in `23dc7a6`).
- **Layer:** L1 (in-process unit test; `src/tab.rs`, alongside `orchestration_012`/`013`).
- **Agent:** none (mock `PaneController`; synthetic `SessionStatus` map and `Instant`/`Duration` values, no panes/PTYs).
- **Asserts:** with a real `TabManager`-opened 3-role Orchestration tab and `last_role_pane_activity_at` set to a known `t0` (simulating activity recorded before the waiting episode started — critically, never refreshed by `auto_focus_waiting_pane`'s own programmatic steer, since that path isn't one of the gated stamp sites), a role pane becomes `WaitingForInput` and the first mirrored per-frame call (`auto_focus_waiting_pane` → `auto_focus_all_clear` → reading `last_role_pane_activity_at` straight off the active tab → `auto_focus_after_inactivity`, exactly mirroring the real `src/ui.rs` call site verbatim) steers focus onto it. With that role STILL `WaitingForInput` 31+ seconds later, replaying the same mirrored chain now correctly leaves focus on the still-waiting role — `auto_focus_waiting_pane` and `auto_focus_all_clear` both no-op (already-focused / still-waiting) as before, but `auto_focus_after_inactivity` also no-ops instead of falling through, because `had_waiting_pane` is still `true`.
- **Does not assert:** the fix's internal implementation shape (e.g. the specific `had_waiting_pane` gate the coder added to `auto_focus_after_inactivity`) — this test pins the CORRECT OUTCOME, not the mechanism; the resolver's own resolution rules in isolation (covered by `tabs/orchestration/010`/`012`/`013`); the render-loop wiring's application to a live pane controller (covered by `tabs/orchestration/014` for the non-fighting case); the blocked-keystroke reset (covered by `tabs/orchestration/019`).
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/017 — Pins the fix for a review finding: `send_config_gen_prompt` (`src/ui.rs:6953-6992`) now stamps `Tab::Orchestration::last_role_pane_activity_at` when it focuses a role pane and records the focus via `TabManager::record_focus`, matching the other gated sites (`Action::SelectCard`/`FocusCard`/`Focus`/`ForwardToPane`, paste, `dispatch_normal_mode_key`) — a role pane reached via this path no longer inherits a stale (or `None`) timestamp and can no longer misbehave on the very next frame's 30s inactivity check (PRD #373 M2, fork-only; fixed in `23dc7a6`).
- **Layer:** L1 (in-process unit test; `src/ui.rs`'s own `#[cfg(test)]` module, same as `tabs/orchestration/011`/`014`/`015`).
- **Agent:** none (`OpenTabPC` mock `PaneController`, no panes/PTYs).
- **Asserts:** against a real `TabManager`-opened 2-role Orchestration tab, driving the REAL `send_config_gen_prompt` targeting the non-orchestrator `coder` role pane focuses that pane on the real pane controller, then confirms `Tab::Orchestration::last_role_pane_activity_at` is `Some` and sub-second-fresh as a result of that single call — this call site is now gated/stamped the same way the sibling sites are.
- **Does not assert:** the fix's internal implementation shape; the resolver's own resolution rules (covered by `tabs/orchestration/010`/`012`/`013`); the render-loop `else if` chain's consumption of the field (covered by `tabs/orchestration/014`); any visible rendering; the blocked-keystroke reset (covered by `tabs/orchestration/019`).
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/018 — Pins the fix for a review finding: `dispatch_normal_mode_key` (`src/ui.rs:5141-5166`) no longer stamps `last_role_pane_activity_at` unconditionally after calling `mirror_selection_into_focus` — the stamp is now gated on the same condition `mirror_selection_into_focus` checks internally, whether the key actually changed `ui.selected_index`, so an unrelated/unbound key no longer refreshes the clock or indefinitely defers a legitimate snap-back (PRD #373 M2, fork-only; fixed in `23dc7a6`).
- **Layer:** L1 (in-process unit test; `src/ui.rs`'s own `#[cfg(test)]` module, same as `tabs/orchestration/011`/`014`/`015`/`017`).
- **Agent:** none (`OpenTabPC` mock `PaneController`; synthetic `Instant`/`Duration` values, no panes/PTYs).
- **Asserts:** against a real `TabManager`-opened 2-role Orchestration tab with focus manually landed on the non-orchestrator `coder` role and its `last_role_pane_activity_at` set to a known stale `Instant`, driving the REAL `dispatch_normal_mode_key` with `z` — unbound to any Normal-mode action by default — confirms `ui.selected_index` did not move (proving the key is a genuine selection no-op) then asserts `last_role_pane_activity_at` is UNCHANGED (still the exact original stale `Instant`, not refreshed) — confirming the stamp is now correctly gated: a no-op key can no longer refresh the clock or defer a legitimate 30s snap-back.
- **Does not assert:** the resolver's own resolution rules; the render-loop `else if` chain's consumption of the field (covered by `tabs/orchestration/014`); the positive stamping case for a key that DOES move selection (covered by `tabs/orchestration/015`); any visible rendering; the blocked-keystroke reset (covered by `tabs/orchestration/019`).
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/019 — A keystroke DROPPED by #374's command-entry lock still counts as activity: it stamps `Tab::Orchestration::last_role_pane_activity_at` and therefore resets the 30-second inactivity snap-back, so a human typing at a locked non-orchestrator role pane is treated as engaged rather than idle (PRD #373 M3, fork-only — the combined interaction with #374, shipped alongside M1/M2).
- **Layer:** L1 (in-process unit test; `src/ui.rs`'s own `#[cfg(test)]` module, same as `tabs/orchestration/011`/`014`/`015`/`017`/`018`).
- **Agent:** none (`OpenTabPC` mock `PaneController`; synthetic `Instant`/`Duration` values, no panes/PTYs).
- **Asserts:** against a real `TabManager`-opened Orchestration tab that starts LOCKED (asserted, so the test can never silently become the forwarded case) with focus landed on the non-orchestrator `coder` role and its `last_role_pane_activity_at` set to a known stale `Instant` 31 seconds old, driving the REAL `handle_key_event` with a plain `x` in `UiMode::PaneInput` leaves the lock's own status message up (proving the keystroke was dropped, not forwarded), refreshes `last_role_pane_activity_at` to a sub-second-fresh `Instant` strictly newer than the stale one, and — the outcome that matters — makes `TabManager::auto_focus_after_inactivity` return `None` for that refreshed timestamp, so the snap-back that was due a moment earlier is now deferred.
- **Does not assert:** the exact lock status-message wording as a contract (it is read here only as the drop marker; `orchestration/lock/004` owns the gating behavior itself); the forwarded-keystroke stamp on an UNLOCKED tab (covered by `tabs/orchestration/014`); the resolver's own timing rules in isolation (covered by `tabs/orchestration/013`); any visible rendering.
- **Platform coverage:** mac+linux+windows.
...
- **Layer:** L1 (in-process unit test; `src/tab.rs`, alongside `orchestration_012`/`013`/`016`).
...
- **Does not assert:** the mechanism's internal shape (the `all_clear_pending` latch is an implementation detail — this pins the outcome); the multi-frame waiting episode (covered by `tabs/orchestration/012`); the inactivity branch (covered by `tabs/orchestration/013`/`021`); the render-loop's application of the result to a live pane controller (covered by `tabs/orchestration/014`).
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/021 — The M2 inactivity snap-back can never yank focus off a role pane that is genuinely `WaitingForInput`, exercised through the real per-frame chain — including the case where the waiting role was ALREADY focused, so the chain falls all the way through to the inactivity branch on the very first frame of the waiting episode with an already-overdue clock (PRD #373 M2, fork-only; the safety half of the `observe_waiting_panes` blocker fix).
- **Layer:** L1 (in-process unit test; `src/tab.rs`, alongside `orchestration_012`/`013`/`016`/`020`).
- **Agent:** none (mock `PaneController`; synthetic `SessionStatus` map and `Instant`/`Duration` values, no real sleeps and no panes/PTYs).
- **Asserts:** with a real `TabManager`-opened 3-role Orchestration tab, focus manually on the non-orchestrator `alpha` and that tab's `last_role_pane_activity_at` set to a `t0` that is 31 seconds stale relative to the `now` fed to the chain, driving the full three-branch chain mirrored verbatim (unconditional `observe_waiting_panes`, then `auto_focus_waiting_pane` → `auto_focus_all_clear` → `auto_focus_after_inactivity`): on the frame `alpha` itself starts waiting, `auto_focus_waiting_pane` no-ops (already focused) and `auto_focus_all_clear` has no edge, so the inactivity branch IS reached with an overdue clock — and must return `None`, leaving focus on `alpha`; a second still-waiting frame likewise; once `alpha` resolves, the all-clear edge fires and returns the orchestrator role, proving the guard releases rather than wedging auto-focus shut.
- **Does not assert:** `auto_focus_after_inactivity`'s own timing rules in isolation (covered by `tabs/orchestration/013`); the multi-frame variant where the waiting role was not already focused (covered by `tabs/orchestration/016`); any visible rendering.
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/022 — Pins the fix for a review finding: deliberately switching BACK to an Orchestration tab refreshes that tab's `last_role_pane_activity_at`, so the role pane the switch restores gets the full 30-second grace interval instead of being snapped away on the first frame back (PRD #373 M2, fork-only).
- **Layer:** L1 (in-process unit test; `src/ui.rs`'s own `#[cfg(test)]` module, same as `tabs/orchestration/011`/`014`/`015`/`017`/`018`/`019`).
- **Agent:** none (`OpenTabPC` mock `PaneController`; synthetic `Instant`/`Duration` values, no panes/PTYs).
- **Asserts:** against a real `TabManager`-opened 2-role Orchestration tab whose remembered focus is the non-orchestrator `coder` role and whose `last_role_pane_activity_at` is a known 31-second-stale `Instant`, driving the REAL `switch_tab_with_focus` to the Dashboard and back restores `coder` on the pane controller and leaves `last_role_pane_activity_at` sub-second-fresh and strictly newer than the stale value; replaying the real per-frame chain immediately afterwards then leaves the pane controller focused on `coder` rather than the orchestrator.
- **Does not assert:** the Dashboard/Mode legs of the switch (they carry no such clock, and the switch's own focus-restore contract is owned by `tabs/orchestration/005`/`dashboard/selection/*`); a switch that restores no pane; any visible rendering.
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/023 — Pins the fix for a review finding: `Action::SelectCard` only stamps `Tab::Orchestration::last_role_pane_activity_at` when the click actually MOVES the selection — the same gate `tabs/orchestration/018` pinned for unbound keys — so repeated clicks on an already-selected card can no longer defer a legitimate 30-second snap-back indefinitely (PRD #373 M2, fork-only).
- **Layer:** L1 (in-process unit test; `src/ui.rs`'s own `#[cfg(test)]` module, same as `tabs/orchestration/011`/`014`/`015`/`017`/`018`/`019`/`022`).
- **Agent:** none (`OpenTabPC` mock `PaneController`; synthetic `Instant`/`Duration` values, no panes/PTYs).
- **Asserts:** against a real `TabManager`-opened 2-role Orchestration tab with a known stale `last_role_pane_activity_at` and `ui.selected_index` already on the `coder` card (index 1), driving the REAL `dispatch_action` with `Action::SelectCard(1)` leaves the selection where it was and leaves `last_role_pane_activity_at` EXACTLY unchanged; a follow-up `Action::SelectCard(0)`, which genuinely moves the selection onto the orchestrator role's card, does refresh it to a sub-second-fresh `Instant` — the positive control proving the gate isn't over-tight.
...
##### tabs/dashboard/001 — `Ctrl+l` cycles a Dashboard tab's sidebar/pane-column split through the three PRD #361 stages — default 33/67, narrower-sidebar 25/75, and Hidden (sidebar collapsed, 0/100) — looping back to default, and an open Orchestration tab's stage is unaffected by the Dashboard tab's cycling and vice versa (PRD #387 M1 scopes the chord to command mode).
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness).
...
- **Asserts:** the deck launches in PaneInput mode on the live Dashboard pane, so a `Ctrl+D` enters Normal (command) mode before any toggle; a Dashboard tab with one live pane then renders the pane column's left edge at the default 33%-width boundary; `Ctrl+l` cycles it through the narrower-sidebar 25%-width position and the Hidden 0%-width (sidebar fully collapsed) position and back to 33%; opening a SECOND, real Orchestration tab in the same directory (which re-lands the deck in PaneInput mode on its own start-role pane) renders it at its OWN default 34%-width boundary regardless of the Dashboard tab's stage; another `Ctrl+D` plus `Ctrl+l` toggles the Orchestration tab to Narrow, and switching back to the Dashboard tab (Shift+Tab, no extra `Ctrl+D` needed since the deck is already in Normal mode from that toggle) confirms the Dashboard tab's split is STILL Default — untouched by the Orchestration tab's toggle, proving cross-tab-type isolation in both directions.
