...
##### worktree/reclaim/018 — Two pending worktrees whose names differ only in one non-UTF-8 byte render as two DIFFERENT bullets, so the operator can tell which directory `--yes` would delete.
- **Layer:** fast synthetic real-binary-subprocess integration (as `worktree/reclaim/001`).
...
##### worktree/reclaim/019 — A worktree created through the deck's REAL creation path reads as deck-owned and reaches the unattended `remove` verdict, while an otherwise-identical hand-made sibling stays foreign.
...
- **Does not assert:** the marker file's *content* or format (the ownership gate never parses the document; the unit tests in `src/issue_dispatch_run.rs` cover the write's placement, tree-cleanliness and idempotency, and `020` covers its length); the `reclaim` removal path for this fixture (`002`); retro-marking of pre-existing worktrees, which is deliberately not done at all.
- **Platform coverage:** mac+linux.

##### worktree/reclaim/020 — A zero-byte ownership marker — the residue a torn `std::fs::write` leaves — must not resolve to ownership, so the worktree falls to `ask` rather than to the unattended `remove`.
- **Layer:** fast synthetic real-binary-subprocess integration (as `worktree/reclaim/001`), with `019`'s addition: the subject worktree is created by calling the production `issue_dispatch_run::create_worktree` in-process, so the marker arrives from the creation path and truncating it is the only thing that changes between the two measurements.
