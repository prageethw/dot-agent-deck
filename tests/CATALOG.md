...
##### tabs/orchestration/016 — Review finding: the per-frame three-way chain (`auto_focus_waiting_pane` → `auto_focus_all_clear` → `auto_focus_after_inactivity`, `src/ui.rs` ~10546-10575) fights `auto_focus_waiting_pane` once it has already steered focus to a role that is STILL `WaitingForInput` — the inactivity branch yanks focus to the orchestrator anyway, violating the PRD's "no fighting between this behavior and `auto_focus_waiting_pane`" success criterion (PRD #373 M2, fork-only).
...
- **Asserts:** with a real `TabManager`-opened 3-role Orchestration tab and `last_role_pane_activity_at` set to a known `t0` (simulating activity recorded before the waiting episode started — critically, never refreshed by `auto_focus_waiting_pane`'s own programmatic steer, since that path isn't one of the gated stamp sites), a role pane becomes `WaitingForInput` and the first mirrored per-frame call (`auto_focus_waiting_pane` → `auto_focus_all_clear` → reading `last_role_pane_activity_at` straight off the active tab → `auto_focus_after_inactivity`, exactly mirroring the real `src/ui.rs` call site verbatim, including the absence of any "any role still waiting" gate as of this writing) steers focus onto it. With that role STILL `WaitingForInput` 31+ seconds later, replaying the same mirrored chain must leave focus on the still-waiting role — this FAILS today (RED) because `auto_focus_waiting_pane` and `auto_focus_all_clear` both correctly no-op (already-focused / still-waiting), letting the chain fall through to `auto_focus_after_inactivity`, which fires and snaps focus to the orchestrator instead.
- **Does not assert:** the eventual fix's shape (e.g. any new "skip the inactivity branch while a role is waiting" gate/helper method the coder introduces) — this test pins the CORRECT OUTCOME, not a specific fix; the resolver's own resolution rules in isolation (covered by `tabs/orchestration/010`/`012`/`013`); the render-loop wiring's application to a live pane controller (covered by `tabs/orchestration/014` for the non-fighting case); the blocked-keystroke reset (PRD #373 M3, deferred until #374).
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/017 — Review finding: `send_config_gen_prompt` (`src/ui.rs:6953-6992`) focuses a role pane and records the focus via `TabManager::record_focus` but never stamps `Tab::Orchestration::last_role_pane_activity_at`, unlike the other gated sites (`Action::SelectCard`/`FocusCard`/`Focus`/`ForwardToPane`, paste, `dispatch_normal_mode_key`) — a role pane reached via this path can inherit a stale (or `None`) timestamp and misbehave on the very next frame's 30s inactivity check (PRD #373 M2, fork-only).
...
- **Asserts:** against a real `TabManager`-opened 2-role Orchestration tab, driving the REAL `send_config_gen_prompt` targeting the non-orchestrator `coder` role pane focuses that pane on the real pane controller, then `Tab::Orchestration::last_role_pane_activity_at` is `Some` and sub-second-fresh as a result of that single call — which FAILS today (RED) because this call site is not gated/stamped the way the sibling sites are.
- **Does not assert:** the eventual fix's implementation shape; the resolver's own resolution rules (covered by `tabs/orchestration/010`/`012`/`013`); the render-loop `else if` chain's consumption of the field (covered by `tabs/orchestration/014`); any visible rendering; the blocked-keystroke reset (PRD #373 M3, deferred until #374).
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/018 — Review finding: `dispatch_normal_mode_key` (`src/ui.rs:5141-5166`) stamps `last_role_pane_activity_at` UNCONDITIONALLY after calling `mirror_selection_into_focus`, regardless of whether the key actually changed `ui.selected_index` — the stamp sits OUTSIDE `mirror_selection_into_focus`'s own `selected_index`-changed gate, so an unrelated/unbound key still refreshes the clock and indefinitely defers a legitimate snap-back (PRD #373 M2, fork-only).
- **Layer:** L1 (in-process unit test; `src/ui.rs`'s own `#[cfg(test)]` module, same as `tabs/orchestration/011`/`014`/`015`/`017`).
- **Agent:** none (`OpenTabPC` mock `PaneController`; synthetic `Instant`/`Duration` values, no panes/PTYs).
- **Asserts:** against a real `TabManager`-opened 2-role Orchestration tab with focus manually landed on the non-orchestrator `coder` role and its `last_role_pane_activity_at` set to a known stale `Instant`, driving the REAL `dispatch_normal_mode_key` with `z` — unbound to any Normal-mode action by default — confirms `ui.selected_index` did not move (proving the key is a genuine selection no-op) then asserts `last_role_pane_activity_at` is UNCHANGED (still the exact original stale `Instant`, not refreshed) — which FAILS today (RED) because the current code stamps unconditionally regardless of whether the key moved role focus.
- **Does not assert:** the eventual fix's implementation shape (e.g. gating the stamp on `mirror_selection_into_focus`'s own before/after selection comparison); the resolver's own resolution rules; the render-loop `else if` chain's consumption of the field (covered by `tabs/orchestration/014`); the positive stamping case for a key that DOES move selection (covered by `tabs/orchestration/015`); any visible rendering; the blocked-keystroke reset (PRD #373 M3, deferred until #374).
- **Platform coverage:** mac+linux+windows.
