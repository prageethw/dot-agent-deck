...
- **Agent:** none.
- **Asserts:** after a successful `sync_merged_workspace_to_main` call that creates the local `main` via the `update-ref` path (confirmed by setup: local `main` did not exist beforehand, and the call reaches `SwitchedToMain`), `git rev-parse --abbrev-ref @{upstream}` for that branch resolves cleanly afterward rather than erroring. `DOT_AGENT_DECK_STATE_DIR` not needed (no isolated-clone provenance involved, matching `020`-`030`). RED today: `update-ref` writes only the ref itself, so `branch.main.remote` stays unset and `@{upstream}` fails to resolve.
- **Does not assert:** the ordinary DWIM path where the local branch already exists (already tracked correctly, uncovered by this bug); the exact fix mechanism (`--set-upstream-to` after `update-ref`, or writing `branch.<b>.remote`/`branch.<b>.merge` directly) — only the observable property that the branch ends up tracking its remote; the adversarial ref-name case, already covered by `028`.
