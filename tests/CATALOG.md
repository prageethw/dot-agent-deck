...
##### worktree/reclaim/024 — Two pending worktrees whose names differ only in one non-UTF-8 byte render as two DIFFERENT bullets, so the operator can tell which directory `--yes` would delete.
- **Layer:** fast synthetic real-binary-subprocess integration (as `worktree/reclaim/001`).
...
##### worktree/reclaim/025 — A worktree created through the deck's REAL creation path reads as deck-owned and reaches the unattended `remove` verdict, while an otherwise-identical hand-made sibling stays foreign.
