...
##### daemon/protocol/002 — A retry reusing the ORIGINAL `delivery_id` must replay forever and never reach the PTY again (PRD #20 R20-004); a retry minting a FRESH `delivery_id` — what `deliver_orchestrator_prompt` actually does — must reach the PTY every time (issue #424 round 2 corrected).
- **Layer:** L1 client/daemon-internal integration (drives the REAL production sequence — `AgentPtyRegistry::admit_delivery` → `compute_write_and_submit_outcome` → `AgentPtyRegistry::record_delivery_outcome`, exactly mirroring the `WriteAndSubmit` dispatch's idempotent-delivery arm in `src/daemon_protocol.rs` — against a REAL `/bin/sh` PTY spawned through `AgentPtyRegistry::spawn_agent`; no mock, no daemon socket).
- **Agent:** none (a real PTY running `/bin/sh`; no LLM).
- **Asserts:** two halves of the contract, both driven through the production ledger sequence against the same real PTY. Half 1 — a SAME `delivery_id` is admitted THREE times with an identical fingerprint (same `expected_agent_id`/pane/text); the first admission is a genuine write (`SendResult::Applied`, marker appears once in scrollback), and the second and third must both REPLAY the cached outcome with no further write ever reaching the PTY (marker count stays at 1). Three admissions, not two, because round 2's regression only shows up on the third: Phase 1 of `admit_delivery` consumed the cached `Applied` result on the second admission (so the second call still replayed, but cleared the cache), which let the third admission find no cached result and proceed to a genuine second write — degrading the one-write-per-`delivery_id` guarantee to two writes total. Half 2 — two DIFFERENT `delivery_id`s share an identical fingerprint (mirroring `deliver_orchestrator_prompt`, `src/ui.rs`, minting a fresh id per confirmation retry); both must independently reach the PTY (marker count reaches 2, one per id). Round 1's fix operates entirely client-side against `PaneController`, which has no ledger behind it at all, so this test is the only one that can observe whether the daemon's actual ledger honors either half.
- **Does not assert:** the UI-level retry/backoff/confirmation logic around WHEN a retry is attempted, or which id-minting strategy the caller uses beyond "fresh per retry" (covered by `orchestration/seed/001`/`004`/`005`/`006`, which — being unable to reach this seam — can only pin what happens client-side).
- **Platform coverage:** mac+linux (spawns a real PTY running `/bin/sh`, which does not exist on Windows — gated `#[cfg(unix)]` like its sibling `guarded_send_rechecks_live_attach_after_writer_lock`).
...
- **Layer:** L1 (in-process seed-prompt readiness consumer with a controllable `PaneController`).
- **Agent:** none.
- **Asserts:** for a pane that never signals `SessionStart`, at the exact instant `process_pending_seed_prompts`'s 10s fallback threshold is first crossed, no write has happened yet — the fallback must start the same `SPAWN_TIME_READINESS_BUFFER` wait a real `SessionStart` would, not bypass it. This PR fixes it: `buffer_elapsed` now honors the same buffer wait for the fallback instead of `if timeout_ready { true } else { ... }` firing the write with a zero buffer the instant the 10s mark is crossed.
...
- **Agent:** none.
- **Asserts:** across two simulated render frames — frame 1 performs the `Applied` write, frame 2 runs 5s later with still no submit observed — the render loop's own per-frame re-entry gate (`orchestrator_prompt.is_some() && !ui.orchestration_prompted.contains(id)`, mirrored inline from the real call site in `src/ui.rs`) stays OPEN after the unconfirmed write instead of closing on bytes-written alone; a second write actually reaches the PTY on frame 2 (`attempts == 2`); and the role's status never reports `Working` while no submit has ever been observed. **Issue #424 round 2 (strengthening):** `SendResultPaneController` now mirrors `AgentPtyRegistry::admit_delivery`/`record_delivery_outcome` — a repeat `delivery_id` that already produced a cached `Applied` outcome is REPLAYED (no call to the underlying write, `attempts` does not advance), exactly like the real daemon's delivery ledger. Round 1's double called the underlying write unconditionally on every invocation, so `attempts == 2` was true of a bare RPC counter and false of production (the real ledger swallows a same-`delivery_id` retry — see `daemon/protocol/002`); this test is unchanged in its assertion but the double it runs against is no longer more permissive than the thing it models.
- **Does not assert:** the confirmed-delivery / no-duplicate-write half (covered by `orchestration/seed/002`); the 10s no-SessionStart fallback's own buffer bypass (covered by `orchestration/seed/003`); how a REAL daemon ledger treats a same-`delivery_id` retry vs. a fresh-`delivery_id` retry (covered at that seam by `daemon/protocol/002`, which drives the real ledger — this UI-level test can only observe the client-side retry/gate logic against a double, not the daemon's ledger); a real PTY / real Claude Code submit-CR timing reproduction (covered at that layer by `tests/spawn_time_role_prompt_submit_after_session_start.rs`); the exact reconciliation mechanism the coder implements (left open, per the issue's own correction that `UserPromptSubmit` cannot carry a minted `delivery_id`).
- **Platform coverage:** mac+linux+windows.
...
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` directly against a `SendResultPaneController`, with an injected clock and an `AppState` snapshot whose session status AND `last_user_prompt` are mutated together between frames to model a real submit; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** frame 1 performs an `Applied` write with no submit yet — the role's status must NOT already report `Working` at this point (symmetric with `001`'s premature-finalization defect); the session's status is then set to `SessionStatus::Thinking` (the daemon's own `UserPromptSubmit` → `EventType::Thinking` mapping, `src/hook.rs:111` / `src/state.rs`) AND `last_user_prompt` is set to an exact match of the sent pointer text, together modeling a real submit landing for that pane — fork #197 M3 step 2 prep (issue #194's own PRD): cycle 1's baseline is `None`, so this confirms via TEXT on its own, not only via LEVEL, matching the stand-in to what M3 step 2 ships once LEVEL is removed; frame 2 runs 5s later with that confirmation now visible in the snapshot — the role's status finalizes to `Working`, the re-entry gate closes (`orchestrator_prompt` is `None` and the tab is in `ui.orchestration_prompted`), and NO second write occurs (`attempts` stays at `1`).
- **Does not assert:** the unconfirmed-retry half (covered by `orchestration/seed/001`); the 10s fallback (covered by `orchestration/seed/003`); a real PTY / real Claude Code submit-CR timing reproduction (`tests/spawn_time_role_prompt_submit_after_session_start.rs`).
- **Platform coverage:** mac+linux+windows.
...
##### orchestration/seed/004 — A `Thinking` session status not attributable to OUR prompt must not confirm delivery, even across the ordinary `Thinking -> Working(tool) -> Thinking` sequence — but a later GENUINE submit of our prompt still must (issue #424 round 4, F12 headline + F13 depth fix; round 2 auditor finding).
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` directly across FOUR frames against a `SendResultPaneController`, with an injected clock and an `AppState` snapshot whose session status/`last_user_prompt` are mutated between frames; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** frame 1: with the session already `Thinking` before any write is attempted (modeling, without needing a real hook harness, any of the auditor-verified false-positive paths in `src/hook.rs` that produce `EventType::Thinking`/`SessionStatus::Thinking` with no submit behind it — an unrecognised OpenCode `session.status` value at boot, `:376-383`; `permission.replied`, `:388`; `PostCompact`/`PostCompaction`, `:124`/`:128`) — the `Applied` write must not finalize on bytes alone. Frame 2: an ORDINARY tool call starts (`SessionStatus::Working`, which `EventType::ToolStart` sets unconditionally, `src/state.rs:4020-4048`) — a FALLING EDGE under round 3's own clearing rule — and must not itself finalize. Frame 3: the tool call ends and the agent returns to `Thinking`, still with nothing (no `last_user_prompt` match) attributing it to OUR prompt — F12's headline defeat, reachable in ordinary operation with no submit anywhere in the sequence: must NOT confirm (`role_statuses[0] != Working`, the re-entry gate stays open). Frame 4 (positive control, round 4's own design): `last_user_prompt` becomes an EXACT match for the prompt we sent, positive identification per round 4's redesign — this MUST confirm (`prompt.is_none()`, `role_statuses[0] == Working`), proving the frame-3 fix cannot be "never confirm again this cycle". Also fixes F13: round 2's two-frame version of this test never changed status at all, so the falling-edge clear it added in round 3 never executed even once, and `confirmed` is computed BEFORE the clear — a clear can only change behavior on a SUBSEQUENT frame. Frames 2/3 are that missing pair, the same depth `daemon/protocol/002` needed three admissions for.
- **Does not assert:** which specific hook.rs event produced the pre-existing `Thinking` in frame 1 (that distinction lives below the `AppState`/`SessionStatus` boundary this test drives); the EXACT mechanism the coder uses for positive identification (`last_user_prompt` matching, a `last_activity` comparison, or something else) — frame 4 models `last_user_prompt` matching specifically, per round 4's own design note that this is the clearest available signal, but only requires that SOME implementation recognize it as genuine; the replacement-session route of F12 (covered by `orchestration/seed/007`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/seed/005 — Once a write has genuinely landed (`Applied`), the deadline must finalize as delivered-unconfirmed (`Working`, with an honest status message) rather than as a failure — a prompt the agent actually received must never be reported as lost (issue #424 round 2, regression vs. `main`). Extended for fork #197 M4 (fork #194) to cover the lost-write half: a confirmation-retry must be GENUINELY attempted (not silently skipped to avoid duplication) and must ride the decided bare-CR mechanism.
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` across THREE frames against a `SendResultPaneController`, with an injected clock; frame 1 lands the write within the deadline, frame 2 (added) runs past `CONFIRMATION_GRACE_PERIOD` and drives the one budgeted confirmation-retry, frame 3 runs past `AUTOMATIC_PROMPT_DEADLINE` with no confirmation ever observed; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** frame 1's `Applied` write leaves the prompt awaiting confirmation (`ui.orchestration_awaiting_confirmation` holds the tab, `attempts == 1`). Frame 2, past the grace period with no confirmation observed, drives the confirmation-retry — reading `SendResultPaneController`'s `texts()` accessor asserts exactly TWO writes reached the double (the original plus one genuinely ATTEMPTED retry — fork #194's PRD names this the single most important thing for review and audit to probe, since a coder "fix" that satisfies `orchestration/seed/014`'s no-duplication requirement by skipping the retry entirely, rather than sending a bare CR, would regress THIS assertion instead) and that the retry's payload is the EMPTY string (a bare submit, fork#197 M4's decided submit-only mechanism — never the full prompt text again); the cycle must still be awaiting confirmation after the retry alone (the double never changes session status, so a bare CR confirms nothing on its own — mirroring the accepted, explicitly-recorded risk that a bare CR against a write genuinely lost downstream cannot rescue it). Frame 3, run after the deadline with the session's status never having become `Thinking`, must finalize the prompt (`prompt.is_none()`) with `role_statuses[0] == OrchestrationRoleStatus::Working` and a status message that says delivered+unconfirmed — NOT one containing "not delivered" or "abandoned", unchanged by the M4 retry-mechanism change. Round 1's deadline check (top of `deliver_orchestrator_prompt`) fires unconditionally on elapsed time regardless of whether `orchestration_awaiting_confirmation` already holds this tab, and `abandon_orchestrator_prompt` never touches `role_statuses` — so a prompt that genuinely landed is retried, then reported as a loss the same way a prompt that NEVER landed would be, a regression from `main`, which reported `Working` for a single successful delivery.
- **Does not assert:** the deadline-with-no-write-ever-landed case (already covered by `deliver_orchestrator_prompt_bounds_deadline_and_terminal_outcomes` above); the exact wording of the status message beyond containing both "deliver" and "unconfirm" while not containing "not delivered"/"abandoned"; whether the daemon-side ledger issue (`daemon/protocol/002`) is also fixed — this test only pins how a write's OWN landed/not-landed history should be reported once the deadline hits; the real-agent proof that a standalone bare CR actually submits on Claude AND Codex (covered by `orchestration/seed/015`/`016`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/seed/006 — The first retry must not even be ATTEMPTED before a grace period longer than a plausible hook round trip — the current 500 ms `send_retry_delay(1)` is shorter than a hook round trip on a loaded machine (issue #424 round 2).
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` directly against a `SendResultPaneController`, reading its `rpc_calls()` counter — which counts every invocation regardless of ledger outcome, unlike `attempts` — with an injected clock; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** frame 1 performs the `Applied` write (`rpc_calls == 1`); frame 2, run 550 ms after the write (just past the CURRENT `send_retry_delay(1)` == 500 ms backoff) with no confirmation observed, must NOT attempt a second RPC (`rpc_calls` stays `1`). Round 1's backoff schedule allows a retry the instant 500 ms elapses, which is shorter than a plausible hook round trip on exactly the loaded machines issue #424 reproduces on; once a retry genuinely reaches the PTY (the fix `daemon/protocol/002` requires), firing this early means a duplicate seed execution, not just a wasted RPC.
- **Does not assert:** the exact grace-period duration the coder picks, only that it is observably longer than the old 500 ms window; whether the eventual retry, once attempted, reaches a real PTY (`daemon/protocol/002`); the unconfirmed-retry / confirmed-finalize / deadline halves (`orchestration/seed/001`/`002`/`005`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/seed/007 — A `Thinking` sample from a REPLACEMENT session on the same pane must not confirm a write made to the session that died — F12's second route (issue #424 round 4).
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` directly against a `SendResultPaneController`, with an injected clock; the write lands against session A, which is then removed from the `AppState` snapshot and replaced with a hand-built `SessionState` carrying a DIFFERENT `session_id`, the SAME `pane_id`, and `status: Thinking`; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** frame 1's `Applied` write against session A (ordinary, `Idle` at write time — no falling edge needed) must not finalize on bytes alone. Session A then disappears (agent crash/reload) and session B appears on the SAME `pane_id`, already `Thinking` — B's own boot status, never a submit of our prompt. Frame 2 must NOT confirm: `role_statuses[0] != Working`, the re-entry gate stays open. `pane_thinking`'s `snapshot.sessions.values().any(|s| s.pane_id == pane_id && s.status == Thinking)` matches by `pane_id` only, blind to session identity, so a replacement session's own unrelated `Thinking` reads as confirmation of the write made to the session that died.
- **Does not assert:** the falling-edge/tool-call route of F12 (covered by `orchestration/seed/004`); the genuine-confirmation positive control (covered there too, and by `orchestration/seed/002`); which daemon-side signal (`SessionEnd`, a new `agent_id` on `SessionStart`) actually produces this pane_id-preserving session swap in production — this test drives the `AppState`/`SessionStatus` boundary directly, the way `orchestration/seed/004` does.
- **Platform coverage:** mac+linux+windows.

##### orchestration/seed/008 — The one-retry-per-cycle budget must be spent the moment a confirmation-retry is ATTEMPTED, regardless of its outcome — not only when that retry itself lands `Applied`/`Queued` (issue #424 round 3 reviewer F6 / auditor M1, `src/ui.rs:3647-3663`, previously untested).
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` directly against a NEW `SequencedSendResultPaneController`, which returns a DIFFERENT `SendResult` per call — `Applied` then `NoLiveTarget` then, only if the fix regresses, a third `Applied` — with an injected clock; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** frame 1 lands the write (`rpc_calls == 1`). Frame 2, past `CONFIRMATION_GRACE_PERIOD` with no confirmation observed (the session stays `Idle` throughout, isolating this from the confirmation mechanic), attempts the one budgeted retry, which lands `NoLiveTarget` — retryable, not terminal, not `Applied`/`Queued` (`rpc_calls == 2`). Frame 3, well past the grace period again, must NOT attempt a third RPC (`rpc_calls` stays `2`): `entry.retried` is set the moment the frame-2 retry is ATTEMPTED (`src/ui.rs:3660-3663`), before its outcome is known, so a non-`Applied` retry outcome still spends the budget. The round-3 bug this guards against: if `retried` were only set inside the `Applied`/`Queued` branch, a `NoLiveTarget` retry would leave it `false`, and frame 3 would mint yet another fresh `delivery_id` the ledger has never seen and retry again — up to ~30 times before the deadline.
- **Does not assert:** whether the retry, once attempted, reaches a real PTY (`daemon/protocol/002`); the confirmation mechanic itself (`orchestration/seed/001`/`002`/`004`/`007`); the terminal-outcome-after-landed-write path (`orchestration/seed/009`/`010`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/seed/009 — An `Ambiguous` outcome on a confirmation-retry, after an EARLIER write already landed, must still report delivered-unconfirmed — the positive control for `orchestration/seed/010` (issue #424 round 3 reviewer F14).
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` directly against a `SequencedSendResultPaneController` returning `Applied` then `Ambiguous`, with an injected clock; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** frame 1's `Applied` write leaves the prompt awaiting confirmation. Frame 2, past the confirmation grace period, attempts the retry and lands the TERMINAL `Ambiguous` outcome (a partial write) — because an earlier write already landed (`orchestration_awaiting_confirmation` holds the tab), this must finalize as delivered-unconfirmed: `prompt.is_none()`, `role_statuses[0] == Working`, and a status message containing both "deliver" and "unconfirm" while containing neither "not delivered" nor "abandoned". Reviewer F14: the delivered-unconfirmed fallback's reasoning genuinely holds for `Ambiguous` — the target is still the SAME live one, just a partial write — so this case must NOT regress to abandonment when the coder narrows the fallback for `WrongSession` (`orchestration/seed/010`).
- **Does not assert:** the `WrongSession` case, which the SAME code path currently gets wrong (covered by `orchestration/seed/010`); the deadline-driven delivered-unconfirmed path with no retry involved (`orchestration/seed/005`); the retry-budget mechanic (`orchestration/seed/008`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/seed/010 — A `WrongSession` outcome on a confirmation-retry, after an EARLIER write already landed, must NOT be reported as delivered-unconfirmed — `WrongSession` is affirmative evidence the target was REPLACED, not a partial write to the same target (issue #424 round 3 reviewer F14; leaving the role at `Working` forever after an agent crash is #424's own symptom).
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` directly against a `SequencedSendResultPaneController` returning `Applied` then `WrongSession`, with an injected clock; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** frame 1's `Applied` write leaves the prompt awaiting confirmation (same precondition as `orchestration/seed/009`). Frame 2, past the confirmation grace period, attempts the retry and lands the TERMINAL `WrongSession` outcome — must NOT finalize as delivered (`role_statuses[0] != Working`). Today's code treats every terminal outcome identically once an earlier write has landed (the SAME branch `orchestration/seed/009` exercises for `Ambiguous`), so this is expected RED: `WrongSession` currently ALSO finalizes as delivered-unconfirmed, which is exactly the reviewer's complaint — the role gets stuck reporting `Working` after the agent that was supposed to receive the prompt has already been replaced.
- **Does not assert:** what the role status SHOULD become instead (abandoned, a new dedicated status, a retry against the new session) — left to the coder; the `Ambiguous` case, which the SAME code path currently gets right (`orchestration/seed/009`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/seed/011 — A real interactive Haiku Claude orchestrator (start) role genuinely receives its spawn-time seed pointer and acts on the file it names (upstream #424, CLAUDE.md rule 4 real-agent bar).
- **Layer:** L2, PTY-attached, real agent (drives the actual `dot-agent-deck` binary, which lazily spawns its own daemon; no synthetic hook, no fabricated `SessionStart`, no hand-seeded prompt).
- **Agent:** a real interactive `claude --model claude-haiku-4-5-20251001 --allowedTools Bash` orchestrator (start) role, spawned through the normal Ctrl+N new-pane flow with per-folder trust pre-seeded; the `worker` role stays a `cat` stand-in since only the orchestrator's remit delivery is under test.
- **Asserts:** every `orchestration/seed/001`-`010` test drives `deliver_orchestrator_prompt` in-process against an injected clock and a `SendResultPaneController` double — proof the client-side retry/confirmation logic is correct, never proof a real Claude Code process ever received the seed, read the file it points at, and acted on it (the exact gap #424's own bug, a fixed 500ms readiness buffer outrun by 5 MCP servers and a 10.9 KB `SessionStart` hook, lived in). This test opens the `orch-seed-remit-real` fixture's orchestration with no test intervention beyond opening the tab, and asserts the full contract in order: (1) a native `UserPromptSubmit`-mapped `Thinking` hook event for the orchestrator's real agent id, carrying the literal pointer text (`Read .dot-agent-deck/orchestrator-context.md`) as `user_prompt`, proves the spawn-time seed was genuinely SUBMITTED inside the real session — never a fabricated event; (2) the fixture's fixed, uniquely-named sentinel token (`ORCH-SEED-011-REMIT-OK-6d29f4`, directed by the fixture's `prompt_template`, which becomes the first section of the generated `orchestrator-context.md`) appearing in the pane's cumulative output proves the agent genuinely READ the named file and ACTED on its content, scanned over the full output history rather than a single grid frame. Expected to PASS: this validates a fix already landed (`fix/424-seed-delivery-confirmation`), not a RED-first pin.
- **Does not assert:** the client-side retry/confirmation state-machine internals (covered exhaustively by `orchestration/seed/001`-`010` against the double); re-assertion on `Compacting` (covered by `orchestration/remit/001`-`003`); delegation to the worker role or any behavior of the `worker` role itself (it is an inert `cat` stand-in here); a real PTY / real Claude Code submit-CR timing reproduction at the lower layer (`tests/spawn_time_role_prompt_submit_after_session_start.rs`).
- **Platform coverage:** mac+linux (real Claude Code interactive session; not run on Windows, and gated behind the `e2e` feature with no CI credentials — local-only, rule 5 exception (a); self-skips in CI for lack of credentials).

##### orchestration/seed/012 — `reset_delivery_cycle()` must clear every per-cycle field, so a fresh delivery cycle on the same tab/pane mints a fresh `delivery_id` and gets a real write, not a replay against the stale ledger entry (fork #197 M1, fork #188).
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` directly against a `SendResultPaneController`, with an injected clock; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** a FULL first delivery cycle on one tab/pane — frame 1's `Applied` write lands and awaits confirmation; frame 2, past the deadline with no confirmation ever observed, finalizes it as delivered-unconfirmed via `finalize_orchestrator_prompt_delivered`, which calls `reset_delivery_cycle()`. A SECOND cycle then starts on the exact same tab/pane — a new prompt, a fresh anchor and a fresh readiness timestamp, mirroring what the remit re-arm gate does (`src/ui.rs:12342`). The second cycle's write must reach the double's stand-in "PTY" as a genuine new attempt (`attempts` advances from 1 to 2) under a `delivery_id` distinct from the first, and must itself land and await its own confirmation. This is the one property the `orchestration/seed/001`-`011` family — passing UNCHANGED across M1's refactor — cannot detect: a per-cycle field forgotten at one of the (formerly three, now one) clear sites. The concrete failure it guards: if `reset_delivery_cycle()` failed to clear `prompt_delivery`, `capture_prompt_delivery` would not re-mint a `delivery_id` for cycle 2, and the double's ledger (mirroring the daemon's `AgentPtyRegistry::admit_delivery`) would short-circuit the repeat id and REPLAY cycle 1's cached `Applied` outcome — `attempts` staying at 1, no byte reaching the "PTY", and in production a silently-lost prompt.
- **Does not assert:** the confirmation mechanic itself (`orchestration/seed/001`/`002`/`004`/`007`); the retry budget (`orchestration/seed/006`/`008`); the terminal-outcome-after-landed-write paths (`orchestration/seed/009`/`010`); which specific field would be the one forgotten — only that the observable write/replay distinction catches any of them, present or future.
- **Platform coverage:** mac+linux+windows.

##### orchestration/seed/013 — TEXT confirmation is structurally dead for a genuine resubmission of byte-identical prompt text — the second-or-later delivery cycle #423's compaction re-assertion produces (fork #187).
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` directly across TWO full delivery cycles on one tab/pane against a `SendResultPaneController`, with an injected clock and an `AppState` snapshot whose session status/`last_user_prompt`/`last_activity` are mutated between frames; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** cycle 1 sends `prepare_orchestrator_prompt`'s constant pointer text and finalizes via a genuine TEXT confirmation (baseline `None`, so an exact `last_user_prompt` match confirms it) — finalizing leaves `last_user_prompt` holding that exact text behind on the session, cycle 2's leftover baseline. Cycle 2 re-sends the SAME text (byte-identical, mirroring the real constant pointer — `prepare_orchestrator_prompt` never varies it) and its write lands with the session ALREADY `Thinking`, so `pre_write_thinking=true` poisons the LEVEL path for the cycle's ENTIRE duration (never cleared mid-cycle, the same technique `orchestration/seed/004` frame 1 uses) — no later status transition can let LEVEL rescue this cycle. A genuine resubmission of the identical text is then driven through `AppState::apply_event` with an `AgentEvent` shaped like a real `UserPromptSubmit` hook delivery (`event_type: EventType::Thinking`, `user_prompt: Some(<the same byte-identical text>)`, for the session's own pane/agent id) — the only path that sets a submission-specific signal, gated on `event.user_prompt` being populated from a hook payload that actually carries a `prompt` field (`src/state.rs:4000-4005`). This confirms (`prompt.is_none()`, `role_statuses[0] == OrchestrationRoleStatus::Working`, fixed by this PR): `prompt_text_confirms` also accepts `observed_seq > baseline_seq` as a genuine-resubmit signal (fork #197 M3 step 1, issue #187), so cycle 2's identical-text resubmit confirms via the submission counter advancing even though the observed text never differs from cycle 1's leftover baseline. This is #187's own headline finding, reproduced end-to-end across two cycles with LEVEL structurally excluded, rather than asserted by inspecting `prompt_text_confirms` in isolation.
- **Does not assert:** removing the LEVEL path — that is fork #197 M3's SECOND step, forbidden in the same change as fixing TEXT (see the PRD's Risks: LEVEL stays load-bearing until TEXT works unconditionally) — so this test does not depend on LEVEL's presence or absence to express its property, and stays meaningful once LEVEL is deleted; which specific field the coder adds to represent a submission event (this test only requires SOME implementation recognize the genuine `apply_event` resubmit as confirming) — `last_activity` is explicitly ruled OUT as that field, since it forward-stamps on every event (`src/state.rs:3976-3977`), not just submissions, and keying confirmation on it would falsely confirm on any event; `orchestration/seed/002`'s stand-in reconciliation, deferred to M3's LEVEL-removal step per the PRD; M4's non-duplicating retry (`orchestration/seed/005`'s planned extension, fork #194).
- **Platform coverage:** mac+linux+windows.

##### orchestration/seed/014 — A confirmation-retry after a write that already LANDED must send a BARE submit (empty payload), never the full prompt text a second time into a composer that already holds it (fork #194, decided mechanism submit-only, fork#197 M4).
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` across two frames against a `SendResultPaneController`, with an injected clock; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** frame 1's `Applied` write lands and awaits confirmation. Frame 2, comfortably past `CONFIRMATION_GRACE_PERIOD` with no confirmation ever observed, attempts the one budgeted confirmation-retry. Reads `SendResultPaneController`'s new `texts()` accessor — the exact `text` payload of every `write_and_submit_to_pane_with_identity` call, recorded in order regardless of ledger outcome — to assert AT THE OBSERVABLE WRITE BOUNDARY, not on internal predicate shape: exactly two writes reached the double (`texts.len() == 2`); the first carries the full prompt text (`"orchestrator prompt"`); the second (the retry) is the EMPTY string, i.e. a bare submit riding `write_and_submit_to_pane_with_identity`'s existing empty-text path (`encode_pane_payload("")`, already pinned by `encode_pane_payload_empty`) rather than the prompt text again. This PR fixes it: the retry write now sends the bare empty payload instead of resending `orchestrator_prompt`'s full text, so `texts[1]` is `""` — closing exactly issue #194's observed symptom (two copies concatenated into one message once the retry's own CR submits).
- **Does not assert:** the lost-write half — a genuinely lost write must still get a real retry attempted, not silently skipped to avoid duplication (covered by `orchestration/seed/005`'s extension); the real-agent proof that a standalone bare CR actually submits on Claude AND Codex (covered by `orchestration/seed/015`/`016`); the confirmation mechanic itself (`orchestration/seed/001`/`002`/`004`/`007`/`013`); the retry-budget-on-attempt mechanic (`orchestration/seed/008`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/seed/015 — A real interactive Haiku Claude orchestrator (start) role, hosted behind a test-only CR-suppressing relay that drops the original write's confirming CR, has its spawn-time seed pointer submitted through exactly ONE native prompt-submission event — a positive proof that the confirmation-retry's own CR, not the original write's, is what caused it (fork #194, fork#197 M4's decided submit-only mechanism; fork#257 M2).
- **Layer:** L2, PTY-attached, real agent (drives the actual `dot-agent-deck` binary, which lazily spawns its own daemon; no synthetic hook, no fabricated `SessionStart`, no hand-seeded prompt).
- **Agent:** a real interactive `claude --model claude-haiku-4-5-20251001 --allowedTools Bash` orchestrator (start) role, spawned through the normal Ctrl+N new-pane flow — but not directly: `orchestrator_command` names a test-only Python relay (`tests/fixtures/cr_suppressing_wrapper.py`, `include_str!`-embedded as `CR_SUPPRESSING_WRAPPER_PY` and written into the fixture's workdir at runtime) that hosts the real `claude` process behind its own inner pty and forwards every byte unchanged EXCEPT the very first `\r` on the daemon-to-agent direction, which it drops. The `worker` role stays a `cat` stand-in since only the orchestrator's remit delivery is under test.
- **Asserts:** every `orchestration/seed/001`-`010`/`012`-`014` test drives `deliver_orchestrator_prompt` in-process against an injected clock and a `SendResultPaneController` double — proof the client-side retry/confirmation logic is correct, never proof a REAL agent process genuinely received a non-duplicated submission of the pointer. `orchestration/seed/011` is that proof for the happy path (no retry observed). This test used to be a proof for the same non-duplication contract but only for whichever branch (retry fired or not) happened to run on a given execution — fork#257 M2 upgraded it to a positive proof by suppressing the original write's confirming CR instead of racing it: the relay drops that CR before it ever reaches `claude`, so the seed text lands in the composer but never submits, and the test asserts — via a per-run private marker file the relay writes the instant it drops the byte — that the suppression genuinely engaged during THIS run, closing the gap where a regression that silently stopped suppressing would otherwise still pass by accidentally re-observing the old weaker claim. A post-review hardening pass (fork#257 review round) replaced the marker's original empty-flag content with two independent cumulative byte counts — bytes read from the daemon and bytes actually forwarded to the agent via a write-all loop, tracked outside the branch that performs the drop — and the assertion now requires their difference to be exactly 1, so a relay mutation that stops removing the byte but leaves the marker-creation call intact is still caught (the counts show a difference of 0); mutation-checked locally and confirmed to fail as expected (self-skips in CI for lack of credentials, so CI cannot witness the check). The marker also moved from a fixed global path keyed on the sentinel to a fresh, private, per-run `common::harness_tempdir()`, and the relay validates and exclusively-creates (`O_CREAT|O_EXCL`) the marker file itself, closing a same-user symlink-race window a predictable path left open. Because nothing can ever confirm the suppressed original, `CONFIRMATION_GRACE_PERIOD` (shrunk to 250ms via `DOT_AGENT_DECK_TEST_CONFIRMATION_GRACE_PERIOD_MS`, `confirmation_grace_period()`, `src/ui.rs`) always elapses and the confirmation-retry always attempts — deterministic, not a race against the confirming hook event as it was before. After the fixture's fixed sentinel token (`ORCH-SEED-015-RETRY-CLAUDE-OK-9f21ab`) proves the agent genuinely read `.dot-agent-deck/orchestrator-context.md` and acted on it, the test inspects every collected native `Thinking`-mapped prompt-submission event carrying the literal pointer text (`Read .dot-agent-deck/orchestrator-context.md`) and asserts: exactly ONE such event exists, and the pointer text appears exactly ONCE inside that event's `user_prompt` (never concatenated with itself — issue #194's exact observed symptom). Because the marker already proved the original CR never reached the agent, that single submission can only be attributable to the confirmation-retry's own later, bare CR — the positive proof `orchestration/seed/016` still lacks (see its entry).
- **Does not assert:** the client-side retry/confirmation state-machine internals (covered exhaustively by `orchestration/seed/001`-`010`/`012`-`014` against the double); Codex's equivalent coverage, which stays on the older whichever-branch-ran proof this test used to have (`orchestration/seed/016`; the suppression relay was attempted against Codex and abandoned this round — see that entry); delegation to the worker role or any behavior of the `worker` role itself (it is an inert `cat` stand-in here).
- **Platform coverage:** mac+linux (real Claude Code interactive session; not run on Windows, and gated behind the `e2e` feature with no CI credentials — local-only, rule 5 exception (a); self-skips in CI for lack of credentials). No `[reel]` marker: a regression proof, not a showcase.

##### orchestration/seed/016 — A real interactive Codex orchestrator (start) role on a cheap model gets the older, weaker non-duplication guarantee `orchestration/seed/015` used to have — no duplication observed, whichever branch (retry fired or not) actually ran on a given execution (fork#197 M4, rescoped from OpenCode to Codex; fork#257 M2 left this test unchanged — see **Does not assert**).
- **Layer:** L2, PTY-attached, real agent (same harness and technique as `orchestration/seed/015`'s ORIGINAL form, driving the real `dot-agent-deck` binary against a genuine interactive `codex --model <cheap-model> --sandbox workspace-write --ask-for-approval never` orchestrator role, spawned directly — NOT behind the CR-suppressing relay `orchestration/seed/015` now uses).
- **Agent:** a real interactive Codex orchestrator (start) role on `common::codex_test_model()`'s pinned cheap model; the `worker` role stays a `cat` stand-in.
- **Asserts:** the same contract `orchestration/seed/015` proved before fork#257 M2, driven against Codex instead of Claude, including the same 250ms `DOT_AGENT_DECK_TEST_CONFIRMATION_GRACE_PERIOD_MS` via `TuiDeckBuilder::with_env` (see `orchestration/seed/015`'s entry for why that value does NOT make the retry fire deterministically — the fixed 500ms `send_retry_delay(1)` backoff floor is the actual gate, and this override cannot touch it, so it is a no-op on retry timing by itself). The fixture's fixed sentinel token (`ORCH-SEED-016-RETRY-CODEX-OK-4e91cb`) proves the agent genuinely read and acted on `.dot-agent-deck/orchestrator-context.md`, then exactly ONE native `Thinking`-mapped prompt-submission event (Codex's native `UserPromptSubmit` hook, `src/hook.rs:111`) must carry the pointer text, and exactly ONCE inside it — true whichever branch actually ran, so this test does NOT positively prove that Codex's documented CR-as-submit behavior (`src/agent_pty.rs:3566`, `:3668`) holds specifically for the confirmation-retry's standalone CR arriving seconds after a separate write; it proves only that no duplication was observed in whichever branch a given run happened to take. This was originally OpenCode coverage; the user rescoped M4's real-agent verification to Claude Code and Codex — the two harnesses whose CR-as-submit behavior the codebase already documents — rather than gate the PRD on installing the OpenCode CLI and provisioning an OpenRouter key. OpenCode and Pi are now accepted, documented unknowns rather than verified.
- **Does not assert:** the client-side retry/confirmation state-machine internals (covered exhaustively by `orchestration/seed/001`-`010`/`012`-`014` against the double); Claude's stronger, positive coverage (`orchestration/seed/015`); OpenCode or Pi's handling of a standalone retry CR — both out of scope for this PRD and left as accepted, documented unknowns rather than verified. Fork#257 M2 attempted extending `orchestration/seed/015`'s CR-suppressing relay to Codex and abandoned it: routing Codex through the relay requires bypassing `wrap_launch_command`'s rewrite into `dot-agent-deck wrap --agent codex -- codex …` (Codex is the one `IntegrationStrategy::Wrapper` agent, `src/agent_registry.rs`), and with that wrapper bypassed Codex never visibly reacted to ANY input — no text in its composer, no hook event, no sentinel — even though the relay was independently confirmed (ad hoc instrumentation) to forward the seed text and both CRs byte-for-byte, identically to how it works for Claude. Codex's `Wrapper` strategy evidently provides something beyond PTY hosting and `CODEX_HOME` pinning (`run_wrap_pty`, `src/wrap.rs`) that this round did not identify; a positive proof for Codex would need the relay nested INSIDE `dot-agent-deck wrap`'s own spawn rather than bypassing it, which was assessed as more real-agent iteration risk than this round's budget covered.
- **Platform coverage:** mac+linux (real Codex interactive session; not run on Windows, and gated behind the `e2e` feature with no CI credentials — local-only, rule 5 exception (a); self-skips in CI for lack of credentials). No `[reel]` marker: a regression proof, not a showcase.

##### orchestration/seed/017 — `send_retry_delay`'s fixed 500ms backoff floor becomes overridable in test/debug builds via `DOT_AGENT_DECK_TEST_SEND_RETRY_BASE_MS`, mirroring `confirmation_grace_period()`'s existing override idiom exactly — the seam PRD fork#257 M1 adds so M4's confirmation-retry can be made to fire on demand instead of only opportunistically (PRD fork#257, `src/ui.rs:2239-2270`'s idiom).
- **Layer:** L1, unit (calls `send_retry_delay` AND `send_retry_base` directly against the process-global env var, guarded by a module-level `static Mutex` / RAII restore guard — `SEND_RETRY_BASE_ENV_LOCK` / `SendRetryBaseEnvGuard` — shared with `orchestration/seed/018`, the other test mutating the same var; reviewer P2 found the previous per-test function-local `static Mutex` idiom, mirroring `spawn_readiness_buffer_parses_env_override_default_and_invalid`, left the two tests serialized against unrelated readers but not against EACH OTHER. No clock injection, no pane double, no PTY, no daemon).
- **Agent:** none.
...
- **Does not assert:** the warn-once behavior (a `static AtomicBool`, per M1) is pinned only by mirroring `confirmation_grace_period`'s structural idiom — this repo has no log-capture test infrastructure, and the sibling `spawn_readiness_buffer` test this is modeled on does not capture log output either, so this test does not directly observe the `tracing::warn!` call or that it fires exactly once rather than per-call. `deliver_orchestrator_prompt`'s actual gating of the confirmation-retry on the overridden floor (covered by `orchestration/seed/018`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/seed/018 — With BOTH the confirmation-grace-period override and the new retry-base override set low, a landed-but-unconfirmed write deterministically produces a second write (the confirmation-retry) within a short, fixed window — the positive proof `DOT_AGENT_DECK_TEST_CONFIRMATION_GRACE_PERIOD_MS` alone cannot provide, since `send_retry_delay(1)`'s fixed 500ms floor gated it regardless of that override (PRD fork#257 M2).
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` across two frames against a `SendResultPaneController`, with an injected clock; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** with `DOT_AGENT_DECK_TEST_CONFIRMATION_GRACE_PERIOD_MS` and `DOT_AGENT_DECK_TEST_SEND_RETRY_BASE_MS` both set to 10ms, frame 1's `Applied` write lands and awaits confirmation. Frame 2, only 50ms later — comfortably past both lowered floors but far short of today's fixed 500ms backoff — must deterministically produce exactly two writes at the pane double (`texts.len() == 2`), the second an empty-string bare submit (M4's decided mechanism, the same contract `orchestration/seed/005`/`014` pin against a 5-second-out frame 2). Unlike those tests, which reach the retry by construction (an arbitrarily generous frame-2 timestamp satisfies any floor regardless of whether it is overridable), this proves the retry is reachable BY TIMING once the floor is movable — the short, deterministic window a real-agent e2e test (`orchestration/seed/015`/`016`) needs to positively prove the retry fired, rather than racing a real agent's confirming hook event against an unmovable 500ms floor.
- **Does not assert:** the two-writes-and-empty-payload shape itself in isolation (already covered by `orchestration/seed/005`/`014`, whose harness this reuses); the clamp/warn-once behavior of the override itself (`orchestration/seed/017`); the real-agent proof that a standalone bare CR actually submits on Claude AND Codex (`orchestration/seed/015`/`016`); the confirmation mechanic itself (`orchestration/seed/001`/`002`/`004`/`007`/`013`); the retry-budget-on-attempt mechanic (`orchestration/seed/008`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/seed/019 — A `Thinking` sample from a FRESHLY spawned session (`Idle` at write time, not pre-existing `Thinking`) carrying no attribution (`user_prompt: None`) must not confirm delivery — the case `orchestration/seed/004`'s pre-existing-`Thinking` poison does not cover (PRD fork#254 N1/N3, the falsifiability proof).
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` directly across TWO frames against a `SendResultPaneController`, with an injected clock and an `AppState` snapshot whose session status is mutated between frames; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** frame 1's `Applied` write lands against a session that is `Idle` (not `Thinking`) at write time — `insert_placeholder_session`'s own default — so `AwaitingConfirmation::pre_write_thinking` is captured `false` for this cycle, UNPOISONED, unlike `orchestration/seed/004`'s frame 1, which sets `Thinking` BEFORE the write specifically to poison LEVEL for the whole cycle (asserted as an explicit precondition here, so a future refactor that changes the poisoning trigger fails loudly instead of silently testing the wrong case). Frame 2, 100ms later, flips the session's `status` to `Thinking` while leaving `last_user_prompt` at `None` — exactly the shape the wrapper's generic stdout-classification fallback produces (`DetectedEvent::Working` -> `EventType::Thinking`, `src/wrap.rs:80`, which fires on ANY non-blank Codex TUI output line, attributing nothing to our prompt). Must NOT confirm: the re-entry gate stays open (`prompt.is_some() && !ui.orchestration_prompted.contains(&tab_id)`) and `role_statuses[0] != OrchestrationRoleStatus::Working`. **Expected RED while LEVEL (`level_confirmed`, `src/ui.rs:4202-4203`) is present**: because `pre_write_thinking` is `false` here, LEVEL is unpoisoned and confirms on `status == Thinking` alone, with no attribution check at all — the exact defect PRD fork#254 N1 measured in production (LEVEL confirming at t+549ms while `last_user_prompt` was still `None`, three instrumented real-Codex runs, recorded in the PRD). Must pass unchanged once fork#254 N2 removes LEVEL — no edit to this test — since TEXT (`prompt_text_confirms`) still correctly declines: `last_user_prompt` never became `Some("orchestrator prompt")`.
- **Does not assert:** the pre-existing-`Thinking`-at-write-time poisoning route, or the `Thinking -> Working(tool) -> Thinking` falling-edge sequence (both covered by `orchestration/seed/004`); the replacement-session route of F12 (`orchestration/seed/007`); the genuine-confirmation positive control (`orchestration/seed/002`, and `orchestration/seed/004`'s own frame 4); which real `hook.rs`/`wrap.rs` event produces this `Thinking`+no-attribution shape in production — that lives below the `AppState`/`SessionStatus` boundary this test drives directly, the way `orchestration/seed/004`/`007` do; PRD fork#254 N1's instrumented real-agent runs are the production-side evidence for this shape, recorded in the PRD, not reproduced here; Codex's real stdout classifier itself (`src/wrap.rs`'s `classify_line`/`CODEX` ruleset) — this test proves the CONSUMER side (`deliver_orchestrator_prompt`) cannot be trusted to decline an unattributed `Thinking`, regardless of what produces it.
- **Platform coverage:** mac+linux+windows.
...
##### scheduler/dispatch/015 — An issue already labelled `in-progress` is skipped even with BOTH other signals absent (no worktree on disk, no open PR) — the label read as a third idempotency signal (PRD #421 M1.2).
- **Layer:** L2 (as `scheduler/dispatch/001`; the stub seeds the label via BOTH a `labels` field in the `gh issue list` response and a per-issue `gh issue view` fixture — `GhStub::set_issues_with_labels`). M1.2 settled on the list-embedded mechanism: the label is read from the `labels` field off the already-made `gh issue list --json number,labels` call (no extra `gh` invocation); the stub still seeds both shapes because the per-issue `gh issue view` fixture remains in use for the separate claimant lookup (`scheduler/dispatch/016`).
...
- **Does not assert:** the claimant-reporting text on the skip (covered by `scheduler/dispatch/016`); the per-issue `gh issue view` claimant lookup, used only once the label is already known present (covered by `scheduler/dispatch/016`).
- **Platform coverage:** mac+linux.
...
##### scheduler/dispatch/017 — Three of PRD #421's four skip causes (worktree exists, open PR, `in-progress` label) render DISTINGUISHABLY from one another — no two collapse to the same text once the issue number/branch are normalized out (PRD #421 M1.2 + M1.3).
...
##### scheduler/dispatch/018 — Triage ENABLED (`triage = true`): a successful dispatch ensures PRD #421 M2.0's seven triage labels exist and the prompt delivered to the dispatched agent carries the full vocabulary plus the `needs-triage` uncertainty rule (PRD #421 M2.1/M2.2).
...
##### scheduler/dispatch/019 — Triage OFF by default (no `triage` key): no `gh label ...` call naming the triage vocabulary is ever made and the delivered prompt carries none of PRD #421 M2.0's seven triage-vocabulary terms — a regression guard against the triage feature leaking into the default dispatch path (PRD #421 M2.1/M2.2).
...
- **Does not assert:** the unconditional `in-progress` claim itself, which does use the `label`/`issue edit` command groups — covered by `scheduler/dispatch/010` and `scheduler/dispatch/020`.
- **Note:** GREEN as a regression guard, not RED-first, exactly like `scheduler/dispatch/012`/`014` — M2.1/M2.2 landed with triage correctly gated behind `cfg.triage`, so this pins that the default (off) path never leaks the vocabulary. The vocabulary-only narrowing (see Asserts) is a faithful expression of the test's original intent — "the triage feature does not leak into the default path" — not a weakening of it: `in-progress` was never triage vocabulary.
- **Platform coverage:** mac+linux.

##### scheduler/dispatch/020 — On a repo that does NOT already carry the `in-progress` label, a successful dispatch still ends with the issue genuinely claimed: the label is ensured to exist before the add, so `gh issue edit --add-label in-progress` actually succeeds (PRD #421 review fix — RED until the claim ensures the label first).
