...
- **Layer:** L1 (in-process seed-prompt readiness consumer with a controllable `PaneController`).
- **Agent:** none.
- **Asserts:** for a pane that never signals `SessionStart`, at the exact instant `process_pending_seed_prompts`'s 10s fallback threshold is first crossed, no write has happened yet — the fallback must start the same `SPAWN_TIME_READINESS_BUFFER` wait a real `SessionStart` would, not bypass it. Fixed in `dfdab95`: `buffer_elapsed` now honors the same buffer wait for the fallback instead of `if timeout_ready { true } else { ... }` firing the write with a zero buffer the instant the 10s mark is crossed.
...
##### orchestration/seed/016 — A real interactive Codex orchestrator (start) role on a cheap model gets the SAME non-duplication guarantee as `orchestration/seed/015`, whichever branch (retry fired or not) actually ran on a given execution (fork#197 M4, rescoped from OpenCode to Codex — commit `6e49744`).
