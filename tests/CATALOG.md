...
##### orchestration/worktree/018 — A blank-slug orchestration spawn against a root checkout that a live sibling orchestration's own directory exactly matches auto-generates a slug and provisions an isolated clone, instead of refusing (issue #521: the fix direction agreed for issue #489's `017` refusal — the blank-slug `None` arm should match the typed-slug `Some(worktree_path)` arm's existing auto-isolate behavior (`014`) rather than asking the user to type a slug by hand).
- **Layer:** L1 (pure — `Action::SpawnPane` dispatched directly against a real `CapturingPaneController`/`TabManager`/`AppState`, no PTY, no real daemon process; a stubbed daemon answers `ListAgents` with a crafted `AgentRecord` whose `TabMembership::Orchestration::orchestration_cwd` equals the test's own repo dir, the same stub `017` uses).
- **Agent:** none.
- **Asserts:** submitting a blank-slug (`orchestration_worktree_path: None`) orchestration form against a genuine, one-commit git repo, while a stubbed daemon reports a live sibling orchestration whose `orchestration_cwd` is that same repo: an orchestration tab opens (`tm.active_tab()` is not `Tab::Dashboard`), every role pane is spawned, each role pane's cwd is a freshly created directory distinct from the source repo with its own real `.git` directory (proving a genuine isolated clone, not a linked worktree sharing the source's object store), and the status message does not carry `017`'s manual-slug refusal wording. Currently RED against unfixed code, which takes the `None` arm's `Ok(true)` refusal branch instead.
- **Does not assert:** the exact auto-generated slug/branch naming convention the eventual fix chooses; the typed-slug collision path itself (that's `014`'s job); the daemon-unreachable fail-closed path (that's `015`'s job); origin-URL fixup or ownership-marker content of the auto-isolated clone (covered by `014`'s and `provision_isolated_clone_sync`'s own unit tests in `issue_dispatch_run.rs`).
- **Platform coverage:** mac+linux+windows (no `#[cfg(unix)]` — same stub attach server as `017`; the real `git clone`/`git checkout` subprocesses it also runs are already exercised cross-platform by `provision_isolated_clone_sync`'s own tests).
