...
- **Does not assert:** which specific op the first request carries (a `ClaimOrchestrationName`/token shape does not exist yet on this branch — see the wire-shape serde round-trip tests in `src/daemon_protocol.rs` for that); the confirm/rebind step onto the real pane id once provisioning succeeds (covered at the registry level by `orchestration/identity/014`); refusal behavior (`orchestration/identity/016`).
- **Platform coverage:** mac+linux+windows.
