...
##### orchestration/seed/015 — A real interactive Haiku Claude orchestrator (start) role's spawn-time seed pointer reaches the agent through exactly ONE native prompt-submission event, containing the pointer text exactly once, whichever branch (retry fired or not) actually ran on a given execution (fork #194, fork#197 M4's decided submit-only mechanism).
- **Layer:** L2, PTY-attached, real agent (drives the actual `dot-agent-deck` binary, which lazily spawns its own daemon; no synthetic hook, no fabricated `SessionStart`, no hand-seeded prompt).
- **Agent:** a real interactive `claude --model claude-haiku-4-5-20251001 --allowedTools Bash` orchestrator (start) role, spawned through the normal Ctrl+N new-pane flow; the `worker` role stays a `cat` stand-in since only the orchestrator's remit delivery is under test.
- **Asserts:** every `orchestration/seed/001`-`010`/`012`-`014` test drives `deliver_orchestrator_prompt` in-process against an injected clock and a `SendResultPaneController` double — proof the client-side retry/confirmation logic is correct, never proof a REAL agent process genuinely received a non-duplicated submission of the pointer. `orchestration/seed/011` is that proof for the happy path (no retry observed); this test is a proof for the same non-duplication contract, this time against a genuine interactive process, for whichever branch (retry fired or not) actually happens on a given run. Fork#197 M4 Part 2 added a test-only, `cfg(any(test, debug_assertions))`-gated override for `CONFIRMATION_GRACE_PERIOD` (`confirmation_grace_period()`, `src/ui.rs`), read from `DOT_AGENT_DECK_TEST_CONFIRMATION_GRACE_PERIOD_MS`; this test sets it to 250ms via `TuiDeckBuilder::with_env` — above `SUBMIT_DELAY` (150ms) so it never overlaps the original write's own completion, and below the fixed 500ms confirmation-retry backoff floor (`send_retry_delay(1)`) that `schedule_send_retry` sets after every landed write. That backoff floor is what actually gates the retry, and this override cannot touch it, so the earliest a retry can fire is unchanged at 500ms regardless of this value — **it does not make the retry fire deterministically** (corrected: this entry previously claimed it did). Whether it fires on a given run is a race against the confirming hook event (`UserPromptSubmit`/`session.prompt`), which fires at submission time, before any inference — typically tens of milliseconds — so confirmation usually lands first and the retry usually does not fire at all; issue #194's own incident (*"500 ms was too short for this Claude Code boot"*) is the case where it does. The assertion is written at the observable-contract level so it holds either way: after the fixture's fixed sentinel token (`ORCH-SEED-015-RETRY-CLAUDE-OK-9f21ab`) proves the agent genuinely read `.dot-agent-deck/orchestrator-context.md` and acted on it, the test inspects every collected native `Thinking`-mapped prompt-submission event carrying the literal pointer text (`Read .dot-agent-deck/orchestrator-context.md`) and asserts: exactly ONE such event exists (never a second independent submit from a confirmation-retry), and the pointer text appears exactly ONCE inside that event's `user_prompt` (never concatenated with itself — issue #194's exact observed symptom, where a confirmation-retry re-writing the full prompt text into a composer that already held it let one CR submit both copies as one message). Because both branches satisfy this identically, the test does NOT positively prove the standalone-CR dedup path was exercised on a given run — only that no duplication was observed in whichever branch actually ran.
- **Does not assert:** the client-side retry/confirmation state-machine internals (covered exhaustively by `orchestration/seed/001`-`010`/`012`-`014` against the double); Codex's equivalent coverage (`orchestration/seed/016`); delegation to the worker role or any behavior of the `worker` role itself (it is an inert `cat` stand-in here).
...
##### orchestration/seed/016 — A real interactive Codex orchestrator (start) role on a cheap model gets the SAME non-duplication guarantee as `orchestration/seed/015`, whichever branch (retry fired or not) actually ran on a given execution (fork#197 M4, rescoped from OpenCode to Codex).
- **Layer:** L2, PTY-attached, real agent (same harness and technique as `orchestration/seed/015`, driving the real `dot-agent-deck` binary against a genuine interactive `codex --model <cheap-model> --sandbox workspace-write --ask-for-approval never` orchestrator role).
...
- **Asserts:** identical contract to `orchestration/seed/015`, driven against Codex instead of Claude, including the same 250ms `DOT_AGENT_DECK_TEST_CONFIRMATION_GRACE_PERIOD_MS` via `TuiDeckBuilder::with_env` (see `orchestration/seed/015`'s entry for why that value does NOT make the retry fire deterministically — the fixed 500ms `send_retry_delay(1)` backoff floor is the actual gate, and this override cannot touch it, so it is a no-op on retry timing by itself; corrected — this entry previously claimed determinism). The fixture's fixed sentinel token (`ORCH-SEED-016-RETRY-CODEX-OK-4e91cb`) proves the agent genuinely read and acted on `.dot-agent-deck/orchestrator-context.md`, then exactly ONE native `Thinking`-mapped prompt-submission event (Codex's native `UserPromptSubmit` hook, `src/hook.rs:111`) must carry the pointer text, and exactly ONCE inside it — true whichever branch actually ran, so this test does NOT positively prove that Codex's documented CR-as-submit behavior (`src/agent_pty.rs:3566`, `:3668`) holds specifically for the confirmation-retry's standalone CR arriving seconds after a separate write; it proves only that no duplication was observed in whichever branch a given run happened to take. This was originally OpenCode coverage; the user rescoped M4's real-agent verification to Claude Code and Codex — the two harnesses whose CR-as-submit behavior the codebase already documents — rather than gate the PRD on installing the OpenCode CLI and provisioning an OpenRouter key. OpenCode and Pi are now accepted, documented unknowns rather than verified.
- **Does not assert:** the client-side retry/confirmation state-machine internals (covered exhaustively by `orchestration/seed/001`-`010`/`012`-`014` against the double); Claude's equivalent coverage (`orchestration/seed/015`); OpenCode or Pi's handling of a standalone retry CR — both out of scope for this PRD and left as accepted, documented unknowns rather than verified.
