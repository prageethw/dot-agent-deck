...
##### daemon/status/001 — `dot-agent-deck daemon status` names a managed agent and visibly reflects a driven live status, not a placeholder identical to an agent with no session (fork #47).
...
##### daemon/status/002 — `dot-agent-deck daemon status --json` emits a machine-readable document carrying `schema_version` with an entry for the managed agent (fork #47).
...
- **Asserts:** the subprocess exits successfully; its stdout parses as a JSON object; the parsed document carries a `schema_version` key; and the raw JSON text names the managed agent by its pane id. Deliberately does not pin any field name beyond `schema_version`, since the rest of the document shape is the coder's to choose (fork #47 design doc).
- **Does not assert:** the human-readable table (`daemon/status/001`); full field-by-field JSON shape.
- **Platform coverage:** mac+linux.

##### daemon/status/003 — `dot-agent-deck daemon status` against an unreachable daemon fails distinguishably from a crash and from clap's own unrecognized-subcommand error, and never brings a daemon into existence at the socket it queried (fork #47).
...
- **Agent:** none.
- **Asserts:** the subprocess does not report success; its stderr carries no Rust panic; its exit code is not clap's own generic usage/parse-error code (`2`) and its stderr carries no clap `Usage:` banner — ruling out "this build's CLI does not understand the `status` subcommand" as the reason for the failure, so it stays distinguishable from a genuinely-handled "no daemon reachable" outcome; and the queried socket path still does not exist on disk afterward, proving the diagnostic never spawned a daemon. Deliberately does not pin the exact exit code value or message wording (fork #47 design doc).
- **Does not assert:** the live-agent path (`daemon/status/001`/`002`); prompt redaction (`daemon/status/004`).
- **Platform coverage:** mac+linux.

##### daemon/status/004 — `dot-agent-deck daemon status` never prints prompt text into its output (fork #47).
...
- **Does not assert:** `--json` output (the fork #47 design doc scopes the no-prompt-text requirement to the human view); task-file/delegate text (out of scope for `ListAgents`-derived data).
- **Platform coverage:** mac+linux.
...
##### issue/claim/003 — `--takeover --confirm-stopped` succeeds: the comment log holds both claims in order, the newest still starts with `Claimed by ` and names who it took over from, and the assignee list ends up holding BOTH A's and B's humans — A's NOT removed (PRD fork#235 round-4 author gate: A's claim comment was authored by `alice`, not B's authenticated account, so B's takeover correctly refuses to remove it; this is the PRD's own accepted cost — "a cross-deck takeover no longer removes the previous human … It fails safe: the error is an extra assignee, never a wrongly-removed one" — not a regression from the pre-round-4 "B's human ONLY" behaviour this test previously pinned).
- **Layer:** fast synthetic real-binary-subprocess integration (as `issue/claim/001`).
- **Agent:** none (as `001`; B's second claim adds both `--takeover` and `--confirm-stopped`).
- **Asserts:** B's takeover claim succeeds; the recorded `gh` comment calls for the issue number at least two (A's original plus B's); the LATEST comment call still contains the literal `Claimed by ` prefix (`parse_claim_comment` finds claims via `.rfind` on it, so any other wording would be invisible and the system would still believe A holds the issue) and names A's worktree absolute path and branch in its tail; the final persisted assignee list is exactly `["alice", "bob"]` — B's human added, A's human left in place by the author gate.
...
##### issue/claim/019 — The dispatch path gets the SAME assignee fix as the CLI: `claim_issue` (the unattended `issue_dispatch` claim path) is called directly with a login matching a claim comment already on record, that comment's `author` field ALSO set to the same login (round-4 author gate: without a matching `author`, `prior_login` resolves unconditionally to `None` and this test would exercise only "no prior login to remove" — a different, vacuous path from the same-login self-cancelling-pair logic it is written for; caught during PRD fork#235's round-4 reconciliation, the third instance of a passing-for-the-wrong-reason test in this PRD, after `012`'s vacuous body and `016`'s wrong-reason pass) — a same-identity refresh. Asserts the assignee ends up STILL SET afterward (PRD fork#235 — reviewer R3 / auditor A8: the `--add-assignee X --remove-assignee X` self-cancelling-pair fix landed on `issue_claim::do_claim` only (`issue/claim/011`); `issue_dispatch_run.rs`'s own `claim_issue` still emits the self-cancelling pair unconditionally, so a close/reopen cycle that reaches it nets the issue unassigned).
...
- **Agent:** none.
- **Asserts:** after `claim_issue` runs with `login` equal to the login already named by the (stubbed) prior claim comment AND that comment's `author`, the stub's `assignees.txt` still contains that login.
...
##### issue/claim/024 — A scheduled issue-dispatch task named `nightly, for @torvalds,` — plain hand-edited config, no forged/hostile comment involved. `sanitize_clone_segment` leaves the substring intact in the derived worktree path (and the task-name-decorated label), so it lands in the deck's OWN claim-comment body when rendered. Before fix 3, `claim_issue` reading that SAME comment back would mis-parse it: `parse_worktree_claim`'s `rest.find(", for @")` scanned from the very start of the remaining body, matching the embedded substring inside the label/path — text that precedes the real timestamp clause — long before any genuine trailing `, for @<login>` clause (there is none; the comment was posted with `login: None`), and the parsed login came back `Some("torvalds")`. Fix 3 bounds that search to start AFTER the timestamp clause, so today `parse_claim_fields` correctly returns `login: None` for this body — pinned as one of two independent sanity preconditions, not the bug it used to be. Asserts no `gh` call ever carries `--remove-assignee torvalds` (PRD fork#235 round-4 — companion to `issue/claim/020`, which covered `@mention` injection into the deck's own rendered comment but not this self-inflicted false parse; companion to `issue/claim/026`, which pins the inverse case — a GENUINE trailing login clause winning despite an earlier coincidental match).
...
- **Agent:** none.
- **Asserts:** two independent sanity preconditions — (1) `sanitize_clone_segment` really did leave the substring intact in the derived path (so the test cannot pass vacuously the way the ORIGINAL `issue/claim/012` once did), and (2) `parse_claim_fields` correctly returns `login: None` for the rendered body (today's fix-3 behaviour, not the pre-fix-3 mis-parse), AND that the fixture JSON carries no `author` field at all — the round-4 author gate's own, independent reason the removal would be refused even if precondition (2) ever regressed; after `claim_issue` runs, no `gh` call in the log contains `--remove-assignee torvalds`.
...
##### issue/claim/026 — The mirror image of `024`: the SAME task-name-derived body carries an EARLIER, coincidental `, for @` clause (inside the decorative label/path, BEFORE the timestamp) AND a GENUINE trailing `, for @genuineuser.` clause AFTER it — unlike `024`, whose body carries no genuine clause at all (`login: None`). `024` proves a fabricated match must not fire; this proves a REAL match still wins when a fake one precedes it — fix 3's timestamp-bound search must be PRECISE (finds the real clause), not merely SUPPRESSIVE (finds nothing once any earlier match exists). The comment is authored as the deck's own currently-authenticated account so the round-4 author gate does not itself mask the result — this test is about the PARSE, not the gate (PRD fork#235 round-4).
...
- **Agent:** none.
- **Asserts:** a sanity precondition that `parse_claim_fields` resolves `login: Some("genuineuser")` (not `torvalds`) from the rendered body; after `claim_issue` runs, some `gh` call in the log contains `--remove-assignee genuineuser`.
- **Does not assert:** the coder's chosen fix shape (as `024`).
- **Platform coverage:** mac+linux (`#[cfg(unix)]` — the stub `gh` is a shell script needing `chmod +x`).
...
##### tabs/orchestration/013 — Within an orchestration tab, focus auto-follows the lowest-order pane currently `WaitingForInput`, recomputed continuously and scoped to the active tab only (fork-only, no PRD/issue).
- **Layer:** L1 (in-process unit test; `src/tab.rs`).
- **Agent:** none (mock `PaneController`; synthetic `SessionStatus` map keyed by pane id, no panes/PTYs).
...
- **Layer:** L1 (in-process unit test; `src/tab.rs`, alongside `orchestration_013`/`012`/`020`).
- **Agent:** none (mock `PaneController`; synthetic `SessionStatus` map, no panes/PTYs).
...
- **Layer:** L1 (in-process unit test; `src/tab.rs`, alongside `orchestration_013`/`012`/`020`/`025`/`026`).
- **Agent:** none (mock `PaneController`; synthetic `SessionStatus` map, no panes/PTYs).
- **Asserts:** with TWO Orchestration tabs (`A`, `B`) open: while `A` is active and locked, its `alpha` role goes `WaitingForInput`, latching `A`'s `had_waiting_pane = true` and stealing focus onto `alpha` (as `orchestration_013` pins); the user switches to `B` and unlocks — `TabManager::clear_waiting_pane_latch` fires with `B`, not `A`, active; while unlocked, `A`'s worker resolves to `Idle` unobserved (the chain never runs against a background tab, and wouldn't run at all while unlocked even if `A` were active); the user re-locks and returns to `A`. `A`'s first locked frame back must be a no-op — the deck-global toggle's latch clear must have reached `A` too, even though `A` wasn't the active tab at the moment of the toggle — so focus stays on `alpha` rather than being yanked to the orchestrator role by a stale `true` -> `false` edge. Pins the outcome (every Orchestration tab's edge state is reset on the transition), not the mechanism the coder uses to reach it.
...
##### error/socket/003 — `request_from_socket` returns `None` within a bounded wait against a daemon that reads the request and then never replies and never closes, instead of hanging forever (fork issue #89).
...
##### error/socket/004 — `request_from_socket` still returns the reply from a daemon that is merely slow, not absent — a bound that fires too eagerly must not be mistaken for "no seed" (the regression fork issue #89 explicitly warns against).
...
- **Asserts:** `request_from_socket` returns `Some("{\"seed\":\"abc123\"}")` — the exact reply line, unmodified — proving the timeout bound added for `error/socket/003` does not fire against a daemon that is merely slow. Passes both before and after the fix; it is a correctness control, not a timing measurement, and the delay is deliberately far from the 5s bound to avoid the flake class in fork issue #81.
...
- **Does not assert:** the keystroke-forwarding gate itself (`orchestration/lock/004`/`010`); the `WaitingForInput` carve-out that lets a keystroke through a locked pane (`orchestration/lock/009`/`010`); the `TabManager`-level mechanism in isolation, including the stale-latch guard on re-locking (`tabs/orchestration/013`/`012`/`020`/`025`/`026`); the pure `Ctrl+e` mode-scoping proof (`orchestration/lock/007`/`008`); a real agent genuinely driving `WaitingForInput` in production (out of scope here for the same reason `orchestration/lock/010`'s doc comment gives — a real Haiku worker would self-skip in CI and leave this headline behavior with zero automated coverage); a re-lock after the unlocked stretch (already covered at the `TabManager` level by `tabs/orchestration/026`'s stale-latch assertion).
- **Platform coverage:** mac+linux.
...
##### orchestration/seed/005 — Once a write has genuinely landed (`Applied`), the deadline must finalize as delivered-unconfirmed (`Working`, with an honest status message) rather than as a failure — a prompt the agent actually received must never be reported as lost (issue #424 round 2, regression vs. `main`).
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` directly against a `SendResultPaneController`, with an injected clock; frame 1 lands the write within the deadline, frame 2 runs past `AUTOMATIC_PROMPT_DEADLINE` with no confirmation ever observed; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** frame 1's `Applied` write leaves the prompt awaiting confirmation (`ui.orchestration_awaiting_confirmation` holds the tab, `attempts == 1`); frame 2, run after the deadline with the session's status never having become `Thinking`, must finalize the prompt (`prompt.is_none()`) with `role_statuses[0] == OrchestrationRoleStatus::Working` and a status message that says delivered+unconfirmed — NOT one containing "not delivered" or "abandoned". Round 1's deadline check (top of `deliver_orchestrator_prompt`) fires unconditionally on elapsed time regardless of whether `orchestration_awaiting_confirmation` already holds this tab, and `abandon_orchestrator_prompt` never touches `role_statuses` — so a prompt that genuinely landed is retried, then reported as a loss the same way a prompt that NEVER landed would be, a regression from `main`, which reported `Working` for a single successful delivery.
