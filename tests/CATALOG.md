...
- **Layer:** L1, unit (calls `send_retry_delay` directly against the process-global env var, guarded by a local `static Mutex` mirroring `spawn_readiness_buffer_parses_env_override_default_and_invalid`'s idiom; no clock injection, no pane double, no PTY, no daemon).
- **Agent:** none.
- **Asserts:** unset, `send_retry_delay(1)` returns today's fixed 500ms floor unchanged (precondition). A valid override (`"10"`) replaces the floor exactly, and the exponential doubling shape survives on the lowered base — `send_retry_delay(2) == 20ms`, `send_retry_delay(3) == 40ms` — so a future change that flattens the curve fails here even with a lowered base. Zero is a legitimate override value (retry as soon as the grace period allows), not treated as unset or clamped up. A non-numeric override falls back to the fixed floor rather than panicking. An out-of-range override (past `AUTOMATIC_PROMPT_DEADLINE`, mirroring `CONFIRMATION_GRACE_PERIOD_MAX`'s own ceiling) still lands on `SEND_RETRY_BACKOFF_CAP` (2s) rather than escaping into an effectively-disabled multi-minute wait.
- **Does not assert:** the exact millisecond value the override's OWN clamp ceiling lands on — `SEND_RETRY_BACKOFF_CAP` (untouched by this PR) already caps every `attempts=1` schedule at 2s once the base exceeds it, so an out-of-range override that IS correctly clamped to just under `AUTOMATIC_PROMPT_DEADLINE` and one that is left entirely UNCLAMPED are indistinguishable through `send_retry_delay`'s return value alone — both land on exactly `SEND_RETRY_BACKOFF_CAP`. Genuinely mutation-verifying the override's own ceiling constant requires either exposing it directly or a different attempts/multiplier combination than this test attempts; flagged for the coder rather than solved here. The warn-once behavior (a `static AtomicBool`, per M1) is pinned only by mirroring `confirmation_grace_period`'s structural idiom — this repo has no log-capture test infrastructure, and the sibling `spawn_readiness_buffer` test this is modeled on does not capture log output either, so this test does not directly observe the `tracing::warn!` call or that it fires exactly once rather than per-call. `deliver_orchestrator_prompt`'s actual gating of the confirmation-retry on the overridden floor (covered by `orchestration/seed/018`).
- **Platform coverage:** mac+linux+windows.
