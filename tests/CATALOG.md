...
##### lifecycle/shutdown-graceful/001 — `AgentPtyRegistry::shutdown_all_graceful`'s phase 3 never derives a pgid from, or signals, an agent phase 2 already reaped (fork #163: phase 2's `try_wait` poll reaps a child as a side effect of merely checking its exit status, releasing that pid back to the kernel for recycling up to `AGENT_TERMINATE_GRACE` — 3s — before phase 3 unconditionally `killpg`s every agent's stored pid regardless of whether phase 2 already reaped it; same class as fork #143/`orchestration/worktree/013`, but on the *normal* shutdown path with a ~3s window instead of a sub-millisecond one on an exceptional timeout path).
- **Layer:** L1 (pure — two independent single-agent registries, each hand-seeded with a `RunningAgent` wrapping a hand-written `portable_pty::Child` stand-in with no real pid, recording every `try_wait`/`wait`/`kill` call into its own shared log in call order; no real OS process, no daemon, no PTY-attached child, no `killpg`).
- **Agent:** none.
- **Asserts:** against a solo agent whose `try_wait` reports exited on its very first call (phase 2 reaps it immediately), `shutdown_all_graceful` produces a call log with exactly one `kill` (phase 1's SIGTERM only) and no `wait` call — proving phase 3 never touches an agent phase 2 already reaped; against a solo agent whose `try_wait` never reports exited, the same call produces exactly two `kill` calls (phase 1's SIGTERM, then phase 3's forcing SIGKILL) and does contain a `wait` call — the control case, proving the fix discriminates by reaped-state rather than silencing phase 3 for every agent (which would trade this bug for fork #133's orphan-descendant gap).
- **Does not assert:** the real pid-recycling race itself (not deterministically forceable — the kernel recycling a freed pid within the grace window needs pid-space wraparound); a real `killpg` call or process group (the stand-in's `process_id()` returns `None`, routing every signal through the `ChildKiller::kill` fallback, so this test needs no OS process); signal identity — with `process_id() -> None` both SIGTERM and SIGKILL collapse into the same unqualified `kill` call, so the kill counts prove signal *attempts*, not confirmed SIGTERM-then-SIGKILL identity; multi-agent interaction within a single `shutdown_all_graceful` call (deliberately two independent single-agent registries instead — phase 2's `.all()` short-circuits on the first not-yet-exited agent, and `HashMap` iteration order is unspecified, so a shared multi-agent registry would make which agent phase 2 actually reaches order-dependent).
- **Platform coverage:** mac+linux (`#[cfg(unix)]` — `crate::platform::proc::AgentProcessGroup` is a bare unit struct only on Unix; the Windows equivalent carries a Job Object handle with no direct-construction path from a test).
