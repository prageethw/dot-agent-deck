...
##### dashboard/pane/011 — The role name renders on its own first body row, in `palette::ROLE_NAME`, unconditionally — never on the title row, and never gated by the agent-type badge toggle (PRD fork#405 M1).
- **Layer:** L1 (ratatui `TestBackend` + color-aware `insta` snapshot).
