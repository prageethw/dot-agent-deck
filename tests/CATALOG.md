...
##### daemon/protocol/002 — A retry that reuses the ORIGINAL `delivery_id`/fingerprint must still reach the PTY — the delivery ledger's `Applied`-caches-as-terminal design makes a same-`delivery_id` retry a silent no-op RPC instead of a write (issue #424 round 2, the actual defect round 1's fix never reaches).
...
- **Asserts:** a stable shell command (`printf 'SEED-424-DELIVERED\n'`) is submitted twice through the production ledger sequence with the SAME `delivery_id` and an identical fingerprint (same `expected_agent_id`/pane/text) — mirroring exactly what `deliver_orchestrator_prompt` (`src/ui.rs`) does while a prompt awaits confirmation: mint the id once at enqueue, hold and reuse it across every retry. The first attempt lands (`SendResult::Applied`, the marker appears once in the PTY's scrollback). The second attempt — the retry — must ALSO reach the PTY (the marker must appear TWICE). Against this branch it fails: `admit_delivery` finds the cached `Applied` result for the id and returns `DeliveryAdmission::Replay`, which the production dispatch (and this test's `deliver_with_ledger` helper, which mirrors it line-for-line) answers from cache WITHOUT calling `compute_write_and_submit_outcome` — the marker appears only once. The ledger carries no TTL, so this holds for the full 60s `AUTOMATIC_PROMPT_DEADLINE`: the prompt is written exactly once, followed by ~30 no-op RPCs, then abandoned — a silent loss became a loud one.
- **Does not assert:** which mechanism the coder picks to fix it (a fresh `delivery_id` minted per retry, a changed ledger caching policy that doesn't treat `Applied` as terminal until confirmed, or something else — this test only pins the observable outcome: a same-`delivery_id` retry must reach the PTY); the UI-level retry/backoff/confirmation logic around WHEN a retry is attempted (covered by `orchestration/seed/001`/`004`/`005`/`006`, which — being unable to reach this seam — can only pin what happens client-side).
...
- **Does not assert:** the confirmed-delivery / no-duplicate-write half (covered by `orchestration/seed/002`); the 10s no-SessionStart fallback's own buffer bypass (covered by `orchestration/seed/003`); whether a same-`delivery_id` retry actually reaches a REAL PTY (covered at that seam by `daemon/protocol/002`, which is the test that must drive the fix — this UI-level test can only observe the client-side retry/gate logic, not the daemon's ledger); a real PTY / real Claude Code submit-CR timing reproduction (covered at that layer by `tests/spawn_time_role_prompt_submit_after_session_start.rs`); the exact reconciliation mechanism the coder implements (left open, per the issue's own correction that `UserPromptSubmit` cannot carry a minted `delivery_id`).
- **Platform coverage:** mac+linux+windows.
...
##### orchestration/seed/004 — A `Thinking` session status that PREDATES the write must not confirm delivery — the confirmation check is a LEVEL sample with no happened-after relationship to the write (issue #424 round 2, auditor finding).
- **Layer:** L1 (in-process — drives `deliver_orchestrator_prompt` directly against a `SendResultPaneController`, with an injected clock and an `AppState` snapshot whose session status is set to `SessionStatus::Thinking` BEFORE frame 1 and never changed; no PTY, no daemon).
- **Agent:** none.
- **Asserts:** with the session already `Thinking` before any write is attempted (modeling, without needing a real hook harness, any of the auditor-verified false-positive paths in `src/hook.rs` that produce `EventType::Thinking`/`SessionStatus::Thinking` with no submit behind it — an unrecognised OpenCode `session.status` value at boot, `:376-383`; `permission.replied`, `:388`; `PostCompact`/`PostCompaction`, `:124`/`:128`) — frame 1 performs the `Applied` write; frame 2, 5s later, with the status STILL exactly the same pre-existing `Thinking` sample, must NOT finalize: the re-entry gate stays open (`orchestrator_prompt.is_some()`), the role's status never reports `Working`. Round 1's `snapshot.sessions.values().any(|s| s.status == SessionStatus::Thinking)` check is a level sample of current status with no timestamp/edge relationship to the write it is meant to confirm — the `Instant` `deliver_orchestrator_prompt` stores in `ui.orchestration_awaiting_confirmation` (`src/ui.rs:3454`) is written and never read.
- **Does not assert:** which specific hook.rs event produced the pre-existing `Thinking` (that distinction lives below the `AppState`/`SessionStatus` boundary this test drives; `worker_event_proves_delivery`, `src/state.rs:1539`, already reasons about raw `AgentEvent`s for a related but different purpose and is a candidate the coder may or may not reach for); the exact mechanism the coder uses to make confirmation edge-triggered (left open — this test only pins the observable outcome: a status unchanged since before the write is not proof of that write).
- **Platform coverage:** mac+linux+windows.
