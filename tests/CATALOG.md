...
- **Layer:** fast synthetic real-binary-subprocess integration (as `worktree/reclaim/001`).
- **Agent:** none.
- **Asserts:** stdout parses as JSON, carries a `schema_version` key, and includes the examined worktree. Deliberately does not pin field names beyond `schema_version`, mirroring `daemon/status/002`.
- **Does not assert:** the full document shape; per-verdict field naming.
- **Platform coverage:** mac+linux.
...
##### orchestration/seed/015 — A real interactive Haiku Claude orchestrator (start) role's spawn-time seed pointer reaches the agent through exactly ONE native prompt-submission event, containing the pointer text exactly once, with the confirmation-retry path DETERMINISTICALLY exercised via a shrunk grace period rather than left to real boot timing (fork #194, fork#197 M4's decided submit-only mechanism).
- **Layer:** L2, PTY-attached, real agent (drives the actual `dot-agent-deck` binary, which lazily spawns its own daemon; no synthetic hook, no fabricated `SessionStart`, no hand-seeded prompt).
...
##### orchestration/seed/016 — A real interactive Codex orchestrator (start) role on a cheap model gets the SAME non-duplication guarantee as `orchestration/seed/015` — verifying that Codex's documented CR-as-submit behavior (`src/agent_pty.rs:3566`, `:3668`) actually holds for the confirmation-retry's standalone CR, not only the CR fused to its own payload (fork#197 M4, rescoped from OpenCode to Codex — commit `6e49744`).
...
- **Asserts:** identical contract to `orchestration/seed/015`, driven against Codex instead of Claude — including the same fork#197 M4 Part 2 confirmation-retry determinism: `DOT_AGENT_DECK_TEST_CONFIRMATION_GRACE_PERIOD_MS` is set to the same 250ms via `TuiDeckBuilder::with_env` (see `orchestration/seed/015`'s entry for why that value), so the confirmation-retry this test exists to prove non-duplicating fires deterministically rather than only when a real boot happens to outrun the 2s production default. The fixture's fixed sentinel token (`ORCH-SEED-016-RETRY-CODEX-OK-4e91cb`) proves the agent genuinely read and acted on `.dot-agent-deck/orchestrator-context.md`, then exactly ONE native `Thinking`-mapped prompt-submission event (Codex's native `UserPromptSubmit` hook, `src/hook.rs:111`) must carry the pointer text, and exactly ONCE inside it. This was originally OpenCode coverage; the user rescoped M4's real-agent verification to Claude Code and Codex — the two harnesses whose CR-as-submit behavior the codebase already documents — rather than gate the PRD on installing the OpenCode CLI and provisioning an OpenRouter key. OpenCode and Pi are now accepted, documented unknowns rather than verified.
