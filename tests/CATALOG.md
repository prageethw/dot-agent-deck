...
- **Does not assert:** the client-side retry/confirmation state-machine internals (covered exhaustively by `orchestration/seed/001`-`010`/`012`-`014` against the double); OpenCode's equivalent coverage (`orchestration/seed/016`); delegation to the worker role or any behavior of the `worker` role itself (it is an inert `cat` stand-in here).
...
##### orchestration/seed/016 — A real interactive OpenCode orchestrator (start) role on a cheap mini model gets the SAME non-duplication guarantee as `orchestration/seed/015` — the user-decided coverage gap this PRD's probe surfaced: CR-as-submit is documented only for claude/codex, and nothing documents how OpenCode treats a standalone CR arriving after a separate write.
- **Layer:** L2, PTY-attached, real agent (same harness and technique as `orchestration/seed/015`, driving the real `dot-agent-deck` binary against a genuine interactive `opencode --model openrouter/openai/gpt-4o-mini --auto` orchestrator role).
- **Agent:** a real interactive OpenCode orchestrator (start) role on `openrouter/openai/gpt-4o-mini`; the `worker` role stays a `cat` stand-in.
- **Asserts:** identical contract to `orchestration/seed/015`, driven against OpenCode instead of Claude — including the same fork#197 M4 Part 2 confirmation-retry determinism: `DOT_AGENT_DECK_TEST_CONFIRMATION_GRACE_PERIOD_MS` is set to the same 250ms via `TuiDeckBuilder::with_env` (see `orchestration/seed/015`'s entry for why that value), so this is the specific test the user decision required it for — OpenCode's handling of a standalone retry CR was the unknown, and without a deterministic retry this test could pass without ever exercising it. The fixture's fixed sentinel token (`ORCH-SEED-016-RETRY-OPENCODE-OK-3c58de`) proves the agent genuinely read and acted on `.dot-agent-deck/orchestrator-context.md`, then exactly ONE native `Thinking`-mapped prompt-submission event (OpenCode's `session.prompt` hook, `src/hook.rs:375`) must carry the pointer text, and exactly ONCE inside it. This is the specific gap the probe recorded in the PRD: the write path is agent-agnostic by construction (`b"\r"` written with no branch on `AgentType`) but CR-as-submit is documented only for claude/codex, so whether a standalone CR arriving seconds after a separate write actually submits on OpenCode was, before this test, an accepted unknown rather than a verified one.
- **Does not assert:** the client-side retry/confirmation state-machine internals (covered exhaustively by `orchestration/seed/001`-`010`/`012`-`014` against the double); Claude's equivalent coverage (`orchestration/seed/015`); the Pi harness, an accepted, documented unknown out of scope for this PRD.
- **Platform coverage:** mac+linux (real OpenCode interactive session; not run on Windows, and gated behind the `e2e` feature with no CI credentials — local-only, rule 5 exception (a); self-skips in CI for lack of credentials). No `[reel]` marker: a regression proof, not a showcase.
