...
- **Does not assert:** the ordinary (non-adversarial) `SwitchedToMain` path, already covered by `020`; the sibling `merge` call's own `--` separator (already present, not under test here); the missing-upstream-tracking case for a validly-named `default_branch`, covered by `032`.
