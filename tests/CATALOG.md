...
- **Does not assert:** that a Mode tab's builder output actually reaches this renderer today (that is `tabs/label/001`'s RED, at the `run_tui` call site, not this seam); the Idle/empty colouring case (covered by `tabs/label/003`).
- **Platform coverage:** mac+linux+windows.
...
- **Does not assert:** the active-tab Idle colouring (covered by `tabs/orchestration/014`, same underlying render rule); that the builder actually produces `Some(&[])` for a not-yet-live Mode tab today (that is part of `tabs/label/001`'s RED).
- **Platform coverage:** mac+linux+windows.
