...
##### dashboard/pane/014 — The role name renders on its own first body row, in `palette::ROLE_NAME`, unconditionally — never on the title row, and never gated by the agent-type badge toggle (PRD fork#405 M1).
- **Layer:** L1 (ratatui `TestBackend` + color-aware `insta` snapshot).
...
- **Does not assert:** the model/badge segment's own content or toggle wiring (covered by `dashboard/agent-badge/001`/`004`); the embedded terminal pane, which keeps its single-line role title and is out of scope for this PRD; `card_height()`'s exact per-density values (covered by `card_height_001_content_derived_values` in `src/ui.rs`). This catalog ID was originally assigned `dashboard/pane/011` by the commit's own RED round, renumbered to `012` in an earlier re-curation, and renumbered again to `014` at this Stage B upstream sync because `012` was independently claimed there by issue #670's hostile-`display_name` test. The idle-art overlay test this PRD's delta-review (F2) originally added was dropped in an earlier re-curation: `fork-only`'s upstream rebase had already removed the idle-art feature (`src/ascii_art.rs`, the `[idle_art]` config section, `IdleArtPhase`/`IdleArtEntry`/`AsciiArtResult`) by the time this commit was folded in, so the overlay F2 fix and its test no longer apply to anything in the tree.
- **Platform coverage:** mac+linux+windows.
...
##### orchestration/delegate/031 — Two panes reporting `work-done` for the same role name and the same cwd (fork #76's two-orchestrations collision) must not clobber each other's report.
...
##### orchestration/delegate/032 — A report already sitting at the daemon's own work-done output path must survive a subsequent, differently-sourced `work-done` call, and the collision must be announced rather than silent (upstream #331).
...
##### orchestration/delegate/033 — `dot-agent-deck work-done --task-file <path>` must be refused, client-side, when `<path>` resolves inside the daemon's own `.dot-agent-deck/work-done-*.md` output namespace (upstream #331's own proposed fix).
...
- **Agent:** none.
- **Asserts:** with a real, reachable daemon, a real file `work-done-coder.md` sits inside `.dot-agent-deck`, and a symlink at `external-report.md` (a name that does NOT match `work-done-*.md`, and does not live in `.dot-agent-deck`) points at it. `work-done --task-file external-report.md` must exit NON-ZERO — told apart from "daemon unreachable" the same way `033` is. A second, otherwise-identical call with no symlink at all must still exit 0. Fixed: `src/main.rs`'s `is_work_done_output_path` now resolves `task_file` via `resolve_work_done_candidate` (which canonicalizes as much of the path as the filesystem allows, following symlinks) BEFORE classifying it, so a symlink whose own name/location sit outside the namespace is still caught by what it resolves to.
- **Does not assert:** the exact refusal wording (stderr is not inspected — only the exit code); the intermediate-directory-symlink variant (covered by `orchestration/delegate/036`).
- **Platform coverage:** mac+linux (unix-only — spawns a real daemon subprocess; symlinks).

##### orchestration/delegate/036 — `work-done --task-file` must be refused when reached through an INTERMEDIATE DIRECTORY SYMLINK that aliases into `.dot-agent-deck`, even though the argument's own literal parent segment is not named `.dot-agent-deck` (PR #90 pre-merge review P1).
- **Layer:** fast real-binary-subprocess integration (same technique as `orchestration/delegate/033`/`035`).
- **Agent:** none.
- **Asserts:** with a real, reachable daemon, a real file `work-done-coder.md` sits inside `.dot-agent-deck`, and a directory symlink `alias` points AT `.dot-agent-deck` itself. `work-done --task-file alias/work-done-coder.md` must exit NON-ZERO, told apart from "daemon unreachable" the same way `033`/`035` are. A second, otherwise-identical call with no symlink at all must still exit 0. Fixed by the same resolution change as `035`: `resolve_work_done_candidate` canonicalizes the intermediate directory too, so what the literal parent segment ("alias") resolves to is what gets classified, not its own name.
- **Does not assert:** the exact refusal wording; the file-symlink variant (covered by `orchestration/delegate/035`); the cwd-independence of the resolved-parent comparison itself (covered by `orchestration/delegate/037`).
- **Platform coverage:** mac+linux (unix-only — spawns a real daemon subprocess; symlinks).

##### orchestration/delegate/037 — `work-done --task-file` naming the daemon's real work-done output must be refused even when the CLI process's OWN cwd is not the pane's cwd the daemon actually writes under — a regression guard for PR #90's re-review (upstream #331, fork #76).
- **Layer:** fast real-binary-subprocess integration (same technique as `orchestration/delegate/033`/`035`/`036`).
- **Agent:** none.
...
##### scheduler/idle-worker/022 — Removing the ORCHESTRATOR's registry entry directly (bypassing `begin_pane_close` entirely, so neither the close-transition sweep nor issue #465's EOF sweep ever runs) still leaves an unrelated agent inheriting its pane id receiving nothing — the `write_and_submit_guarded` agent-id gate is the only guard left standing on this path (issue #465 F1 review).
