...
##### orchestration/workspace/009 — Two near-simultaneous `provision_isolated_clone_sync` calls, racing to resume the SAME already-eligible directory (created once via a real prior call, so it carries matching M4b evidence and ancestry), serialize on the reused attach lock into DISTINGUISHABLE outcomes across many trials — never both reporting the identical outcome (PRD fork#544 M3's Design step 3: the attach-lock reuse "closes a race that resume introduces... two racers passing the eligibility check would otherwise attach live sessions to the same tree concurrently").
- **Layer:** L1 (in-process — two-thread barrier-synchronized direct calls to `provision_isolated_clone_sync`, mirroring the existing `provision_isolated_clone_sync_concurrent_calls_never_both_create` sibling test's own pattern in this file, but racing a RESUME of an already-present directory rather than a fresh CREATE; real `git` subprocess and filesystem, no PTY, no daemon, no real agent).
- **Agent:** none.
- **Asserts:** across 10 trials, each racing two threads against one pre-created, resume-eligible directory (barrier-synchronized so both start at nearly the same instant): the two threads' `Debug`-formatted outcomes are never identical to each other. `DOT_AGENT_DECK_STATE_DIR` pinned per-test, as `006`. Today's code has no `Resumed` outcome and no winner/loser distinction on resume — `clone_dir.exists()` alone makes BOTH racers report the identical generic `AlreadyClaimed`, harmlessly but with no winner at all: RED against it (`assert_ne!` on two identical debug strings).
- **Does not assert:** the eligibility check's individual branches (`006`-`008`, which this race deliberately bypasses by pre-creating an already-eligible directory); a genuine two-PROCESS (as opposed to two-thread) race — real two-client PTY concurrency against a shared daemon/state-dir proved impractical to set up cleanly in this harness for the resume case specifically (see the tester's `work-done` report for PRD fork#544 M3's RED round); the exact lock primitive's internal behavior beyond the observable serialization.
