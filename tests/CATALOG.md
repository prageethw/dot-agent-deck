...
- **Does not assert:** the provenance-marker half of the story (`session/live/013` — the marker surviving the wire); recovery of events lost during a LATER subscription outage (out of scope for #36 — the TUI does not re-hydrate on re-subscribe); rendered card output; a real daemon's timing.
- **Platform coverage:** mac+linux.
