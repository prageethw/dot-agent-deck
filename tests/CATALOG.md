...
- **Asserts:** against a real `TabManager`-opened 2-role Orchestration tab, calling the REAL `mirror_selection_into_focus` to land focus on the non-orchestrator `coder` role — with no `Action::ForwardToPane` ever run in the test — stamps `ui.last_pane_keystroke_at` on its own, exactly as a forwarded keystroke would; replaying `src/ui.rs`'s real per-frame `else if` chain (`auto_focus_waiting_pane` → `auto_focus_all_clear` → `auto_focus_after_inactivity`) verbatim, with a synthetic 31-second-stale timestamp, applies the snap-back through the SAME pane controller `mirror_selection_into_focus` focused `coder` on, landing it on the orchestrator role. It then pins the scope leak: `Action::SelectCard` (`src/ui.rs:7265-7278`) calls `mirror_selection_into_focus` unconditionally regardless of active-tab kind — only the `selected_session_id` write is gated on `Tab::Dashboard` — so a plain Dashboard-tab card click stamps the SAME global timestamp an Orchestration-tab snap-back reads. With `coder` re-focused and genuinely 31+ seconds stale, a Dashboard-tab click (mirroring the real `switch_tab_with_focus` / `TabManager::restore_focus_on_switch_in` tab-switch sequence) resets the global timestamp; switching back to the Orchestration tab and replaying the per-frame chain again shows the snap-back does NOT fire — `coder` stays focused — because the chain reads the fresh Dashboard stamp, not `coder`'s actual staleness. This is a real scope leak (a Dashboard click can delay/suppress a legitimately overdue Orchestration-tab snap-back), not a harmless one — flagged for a follow-up fix, not fixed by this test-only task.
- **Does not assert:** the resolver's own resolution rules for `auto_focus_waiting_pane`/`auto_focus_all_clear` (covered by `tabs/orchestration/010`/`012`) or `auto_focus_after_inactivity`'s own timing rules in isolation (covered by `tabs/orchestration/013`); any visible rendering (no `compute_frame_layout`/`render_frame` pass here, unlike `tabs/orchestration/011` — this test asserts on the pane-controller-level focus outcome only); the blocked-keystroke reset (PRD #373 M3, deferred until #374 lands); a fix for the documented Dashboard→Orchestration scope leak (flagged, not remediated, here).
- **Platform coverage:** mac+linux+windows.
