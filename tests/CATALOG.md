...
##### orchestration/seed/015 — A real interactive Haiku Claude orchestrator (start) role's spawn-time seed pointer reaches the agent through exactly ONE native prompt-submission event, containing the pointer text exactly once, even across whatever confirmation-retry the real boot timing genuinely triggers (fork #194, fork#197 M4's decided submit-only mechanism).
- **Layer:** L2, PTY-attached, real agent (drives the actual `dot-agent-deck` binary, which lazily spawns its own daemon; no synthetic hook, no fabricated `SessionStart`, no hand-seeded prompt, no artificial shrinking of `CONFIRMATION_GRACE_PERIOD` — none exists to shrink).
