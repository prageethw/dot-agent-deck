...
- **Precondition (upstream #471 review fix):** `claim_issue` (PRD #421 M1.0/M1.1) writes an `in-progress` label on the fixture issue on every successful dispatch and nothing ever removes it, so a prior run of this test permanently claims issue #1 for every later run — the daemon reads the label and skips, and the test then hangs to the readiness wait below before panicking with a blank pane and `Records: []`. Before firing, the test reads the issue's current labels (`gh issue view --json labels`, needs no elevated permission) and, only if `in-progress` is present, attempts to remove it (`gh issue edit --remove-label`). This account has `pull`-only access to the fixture repo, so the removal always fails with a permission denial there; the test recognizes that specifically (textual match on `gh`/GitHub's 403 wording) and SKIPS with a message naming the exact remediation command, rather than panicking or attempting the fire against an issue it cannot un-claim. Any other removal failure (network, repo gone, ...) panics directly, not a skip.
...
- **Note:** `ensure_claim_label` landed — it ensures `in-progress` exists before the unconditional add, so the tightened stub's real-`gh`-accurate rejection no longer fires; GREEN today. This closes the exact defect the PRD #421 review flagged: on any repo without a pre-existing `in-progress` label, the headline claim feature used to silently do nothing.
- **Platform coverage:** mac+linux.
