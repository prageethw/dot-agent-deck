...
- **Does not assert:** the suggestion/refusal MECHANISMS in isolation (covered by `orchestration/identity/002`/`003`); the blank-slug collision refusal itself, which the second open here is careful to avoid rather than exercise (that's `orchestration/worktree/017`'s job); the CONTENT of the worktree the second open's typed-slug arm creates, or its ownership marker (covered by `orchestration/worktree/005`, `worktree/reclaim/022`/`023`).
...
##### orchestration/worktree/017 — A blank-slug orchestration spawn (the default — no typed Worktree field) against a root checkout that a live sibling orchestration's own directory exactly matches is refused (issue #489: `Action::SpawnPane`'s `req.orchestration_worktree_path` match calls `root_checkout_has_live_sibling` only in its `Some(worktree_path)` arm — the `None` arm, taken by the blank-slug default most orchestrations use, fell straight through to `req.dir` with no daemon consultation at all, silently rooting a second concurrent orchestration in the same shared checkout). Unlike a typed-slug spawn colliding with a live sibling (`014`), which isolates into its own clone and succeeds, a blank-slug collision has no slug to isolate into, so it refuses instead — and, per the fix round's `SiblingScope` split, it refuses only on this exact-cwd shape, not on a live sibling merely sharing the root checkout's `--git-common-dir` from a worktree carved off it (that shape — this project's own everyday worktree-per-fix model — must NOT refuse; see `orchestration_dispatch_001`, which pins the opposite case).
...
- **Agent:** none.
- **Asserts:** submitting a blank-slug (`orchestration_worktree_path: None`) orchestration form against a genuine git repo, while a stubbed daemon reports a live sibling orchestration whose `orchestration_cwd` is that same repo: the orchestration tab is never opened (`tm.active_tab()` stays `Tab::Dashboard`), no role pane is spawned, and the status message contains wording unique to the `Ok(true)` (live-sibling-found) refusal — distinguishing it from `015`'s `Err(reason)` (daemon-unreachable) refusal, which a bare non-empty check would not.
- **Does not assert:** the exact byte-for-byte status message text (only that it contains the `Ok(true)`-specific substring); the typed-slug collision path itself (that's `014`'s job); the daemon-unreachable fail-closed path (that's `015`'s job); the worktree-sibling (non-collision) shape for a blank-slug spawn (that's `orchestration_dispatch_001`'s job, an e2e test).
