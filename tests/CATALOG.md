...
- **Agent:** none.
- **Asserts:** after `claim_issue` runs with `login` equal to the login already named by the (stubbed) prior claim comment, the stub's `assignees.txt` still contains that login.
