//! RED tests for fork #47's `dot-agent-deck daemon status [--json]` — a
...
async fn daemon_status_001_reports_live_agent_status_inner() {
    ...
    // fields the fork #47 design doc scopes `record.live` to (hook-session
...
async fn daemon_status_002_json_output_lists_the_managed_agent_inner() {
    ...
    assert!(
        ...
        "`daemon status --json` must carry a `schema_version` field (fork #47 design doc); \
         got {parsed:?}"
    );
    assert!(
...
async fn daemon_status_003_no_daemon_reports_unavailable_without_spawning_one_inner() {
    ...
    assert!(
        ...
        "a diagnostic query must never itself bring a daemon into existence at the socket path \
         it queried (fork #47 design doc's non-perturbation contract); the socket now exists \
         at {}",
        attach_path.display()
    );
}
...
async fn daemon_status_004_human_output_never_leaks_prompt_text_inner() {
    ...
    assert!(
        ...
        "`daemon status` output leaked prompt text (fork #47 design doc: \"do not print prompt \
