...
///
/// Same value, and the same rationale, as [`OBSERVATION_BUDGET`] — defined in
/// terms of it rather than a second literal, so the two constants cannot
/// silently drift apart (fork issue #81).
...
impl TuiDeck {
    ...
    fn try_launch_inner(builder: TuiDeckBuilder, fixture_name: &str) -> Result<Self, String> {
        ...
        if builder.claude_trust_workdir {
            ...
            // sanitized typed/suggested Name segment (`sanitize_workspace_segment`,
            // `src/ui.rs`). Every real-agent orchestration test in this
            // suite opens exactly one, the FIRST, orchestration against a
            // fresh deck and leaves Name untyped, so the segment is
            // deterministically the form's own suggested default,
            // `"{work-basename}-orchestrator-1"` (`suggest_orchestration_name`'s
            // first candidate, always free on a fresh deck with no prior
            // sibling on disk). Pre-trust that predicted sibling path (raw
            // and canonicalized) the same way as `work` above — mirroring,
            // not calling, `suggest_orchestration_name`/
            // `sanitize_workspace_segment`/`resolve_workspace_path`, since
            // all three are private to `src/ui.rs` and this harness cannot
            // depend on that crate's internals. A test that types its own
            // Name, or opens a second orchestration, needs its own
            // `with_claude_project_trust(path)` call for that path — this
            // only covers the common, blank-Name, first-orchestration case
            // every caller in this suite actually uses.
    ...
    ///
    /// **Mechanically**, use this — not `wait_for_string(<some other
    /// text>)` followed by a bare `find_in_grid(needle).expect(…)` —
    /// whenever a test needs a widget's coordinates in order to click it.
    /// The deck writes one rendered frame as a single burst, but the reader
    /// thread feeds the vt100 parser in `read()`-sized chunks (≤4 KiB, and
    /// as little as ~1 KiB on macOS PTYs), so a frame taller than one chunk
    /// lands in the parser in pieces. Between two of those pieces the grid
    /// is a legitimate but TORN view of the frame: the rows written by the
    /// earlier chunk are already painted while the later rows still show
    /// the previous screen. A `wait_for_string` on a proxy string that
    /// happens to sit in an earlier chunk therefore returns while the
    /// widget the test actually wants has not been parsed yet, and the
    /// immediately-following `find_in_grid` legitimately misses it. Polling
    /// for the needle itself closes that window: the wait ends only once
    /// the chunk carrying it has been processed.
    ///
...
pub fn import_claude_credentials(test_home: &Path) -> std::io::Result<Vec<String>> {
...
/// `orchestrator_index = 1`. (PRD fork#760 Part A briefly changed the
/// production formula this predicts — a blank Worktree-slug field, rather
/// than Name, drove the segment — then reverted it; this mirror follows
/// suit, restoring the Name-derived formula.)
///
