        ...
        let task = tokio::spawn(async move {
            loop {
                match rx.recv().await {
                    ...
                    Ok(BroadcastMsg::WorktreeKept(_)) => {}
...
async fn delegate_026_settled_interface_fact_is_upgraded_before_the_pointer_is_released_inner() {
    ...
    let pointer = common::expected_delegate_pointer(run._cwd.path(), WORKER_ROLE, WORKER_PANE);
    let delivered = wait_for_snapshot_needle(
        &run.daemon.registry,
        &run.new_agent_id,
        &pointer,
...
impl SilentWorkerArm {
    ...
    async fn delegate_and_confirm_delivery(&mut self) {
        self.state
            ...
            .await;
        let pointer = common::expected_delegate_pointer(self._cwd.path(), WORKER_ROLE, WORKER_PANE);
        let delivered =
...
#[test]
fn delegate_031_same_role_same_cwd_concurrent_work_done_does_not_clobber() {
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .enable_all()
        .build()
...
#[test]
fn delegate_032_existing_report_survives_and_collision_is_announced() {
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .enable_all()
        .build()
...
#[test]
fn delegate_034_third_collision_destroys_the_first_archived_report() {
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .enable_all()
        .build()
...
#[test]
#[cfg(unix)]
fn delegate_040_unknown_role_does_not_report_success() {
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .enable_all()
        .build()
        .expect("build unknown-role-confirmation-truthfulness runtime")
        .block_on(delegate_040_unknown_role_does_not_report_success_inner());
}

#[cfg(unix)]
async fn delegate_040_unknown_role_does_not_report_success_inner() {
    let daemon = common::spawn_inprocess_daemon().await;
    let cwd = common::race_safe_tempdir();
    let cwd_str = cwd.path().to_string_lossy().into_owned();
...
#[test]
#[cfg(unix)]
fn delegate_041_self_target_does_not_report_success() {
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .enable_all()
        .build()
        .expect("build self-target-confirmation-truthfulness runtime")
        .block_on(delegate_041_self_target_does_not_report_success_inner());
}

#[cfg(unix)]
async fn delegate_041_self_target_does_not_report_success_inner() {
    let daemon = common::spawn_inprocess_daemon().await;
    let cwd = common::race_safe_tempdir();
    let cwd_str = cwd.path().to_string_lossy().into_owned();
...
#[test]
#[cfg(unix)]
fn delegate_042_duplicate_role_reports_what_was_armed() {
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .enable_all()
        .build()
        .expect("build duplicate-role-confirmation-truthfulness runtime")
        .block_on(delegate_042_duplicate_role_reports_what_was_armed_inner());
}

#[cfg(unix)]
async fn delegate_042_duplicate_role_reports_what_was_armed_inner() {
    let daemon = common::spawn_inprocess_daemon().await;
    let cwd = common::race_safe_tempdir();
    let cwd_str = cwd.path().to_string_lossy().into_owned();
...
#[test]
#[cfg(unix)]
fn delegate_043_partial_resolution_names_both_armed_and_unresolved() {
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .enable_all()
        .build()
        .expect("build partial-resolution-confirmation runtime")
        .block_on(delegate_043_partial_resolution_names_both_armed_and_unresolved_inner());
}

#[cfg(unix)]
async fn delegate_043_partial_resolution_names_both_armed_and_unresolved_inner() {
    let daemon = common::spawn_inprocess_daemon().await;
    let cwd = common::race_safe_tempdir();
    let cwd_str = cwd.path().to_string_lossy().into_owned();
