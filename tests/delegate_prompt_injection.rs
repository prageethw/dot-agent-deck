...
#[test]
fn delegate_029_same_role_same_cwd_concurrent_work_done_does_not_clobber() {
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .enable_all()
        .build()
        ...
        .block_on(async {
            ...
            // Issue #448: a `work-done` with no outstanding commission is reported
            // as unsolicited and writes nothing at all, so these two calls have to
            // stand in for genuinely commissioned work (two live orchestrations
            // that each delegated to their own `coder`) rather than a completion
            // nobody asked for — arming a commission per pane is what makes that
            // true; the orchestrator pane id itself is never read back by anything
...
async fn run_two_work_done_calls(
    ...
    // completion needs its own commission armed first — otherwise there would
...
#[test]
fn delegate_030_existing_report_survives_and_collision_is_announced() {
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .enable_all()
        .build()
...
#[test]
fn delegate_032_third_collision_destroys_the_first_archived_report() {
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .enable_all()
        .build()
        ...
        .block_on(async {
            ...
            for report in [REPORT_A, REPORT_B, REPORT_C] {
                // Issue #448: an uncommissioned work-done writes nothing at all, so
...
#[test]
#[cfg(unix)]
fn delegate_038_unknown_role_does_not_report_success() {
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .enable_all()
        .build()
        .expect("build unknown-role-confirmation-truthfulness runtime")
        .block_on(delegate_038_unknown_role_does_not_report_success_inner());
}

#[cfg(unix)]
async fn delegate_038_unknown_role_does_not_report_success_inner() {
    let daemon = common::spawn_inprocess_daemon().await;
    let cwd = common::race_safe_tempdir();
    let cwd_str = cwd.path().to_string_lossy().into_owned();
...
#[test]
#[cfg(unix)]
fn delegate_039_self_target_does_not_report_success() {
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .enable_all()
        .build()
        .expect("build self-target-confirmation-truthfulness runtime")
        .block_on(delegate_039_self_target_does_not_report_success_inner());
}

#[cfg(unix)]
async fn delegate_039_self_target_does_not_report_success_inner() {
    let daemon = common::spawn_inprocess_daemon().await;
    let cwd = common::race_safe_tempdir();
    let cwd_str = cwd.path().to_string_lossy().into_owned();
...
#[test]
#[cfg(unix)]
fn delegate_040_duplicate_role_reports_what_was_armed() {
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .enable_all()
        .build()
        .expect("build duplicate-role-confirmation-truthfulness runtime")
        .block_on(delegate_040_duplicate_role_reports_what_was_armed_inner());
}

#[cfg(unix)]
async fn delegate_040_duplicate_role_reports_what_was_armed_inner() {
    let daemon = common::spawn_inprocess_daemon().await;
    let cwd = common::race_safe_tempdir();
    let cwd_str = cwd.path().to_string_lossy().into_owned();
...
/// Scenario: Register a real orchestrator pane and a single `cat`-stub `coder` worker, then run the REAL `dot-agent-deck delegate` CLI from the orchestrator pane with `--to coder --to nonexistent-role` — one requested role resolves to a real pane, its sibling does not. The fork #92 partial-resolution decision requires the call to succeed (something really did arm, so failing it would make a retry-on-non-zero orchestrator double-delegate the role that DID arm — upstream #330's harm) while naming BOTH the armed role and the unresolved one, distinguishably. Today `handle_delegate` replies `DelegateResponse::accepted(signal.to)` unconditionally, so `main.rs` folds both roles into one undifferentiated, comma-joined echo of the raw request — the same silent-drop failure `038`/`039` pin, wearing the partial-resolution shape. Assert the call succeeds, both roles are named somewhere observable, and they are not simply reported as one plain comma-joined pair (fork #92 partial-resolution decision).
#[spec("orchestration/delegate/041")]
#[test]
#[cfg(unix)]
fn delegate_041_partial_resolution_names_both_armed_and_unresolved() {
    tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .enable_all()
        .build()
        .expect("build partial-resolution-confirmation runtime")
        .block_on(delegate_041_partial_resolution_names_both_armed_and_unresolved_inner());
}

#[cfg(unix)]
async fn delegate_041_partial_resolution_names_both_armed_and_unresolved_inner() {
    let daemon = common::spawn_inprocess_daemon().await;
    let cwd = common::race_safe_tempdir();
    let cwd_str = cwd.path().to_string_lossy().into_owned();
    ...
    assert!(
        ...
        "the armed role and the unresolved role must not be reported as a single \
         ...
         roles genuinely armed, the same silent-drop failure `038`/`039` exist to stop, now in \
