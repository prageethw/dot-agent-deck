...
/// may be EXECUTED.
///
/// The record path descends from `common::harness_tempdir()`, i.e. `TMPDIR`, so this
...
fn manager_010_blank_default_command_falls_back_to_claude() {
    ...
    // drawing over the line the `Paragraph` reserved) — mechanically, one
    // rendered frame reaches the harness's vt100 parser in `read()`-sized
    // chunks, and this form's frame splits between the title row and the
    // button row, so a single-shot read can land between the two and fire on
    // a frame that is merely incomplete — issue #807's class, and what
    // reddened `e2e-deterministic` for this test on run 33847276913.
    // `wait_for_in_grid` also dumps the grid if the button genuinely never
    // arrives, which `.expect()` does not.
    let (scol, srow) = deck.wait_for_in_grid("[Submit]");
    deck.click(scol, srow); // submit → spawn the seeded authoring agent
