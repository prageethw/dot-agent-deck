...
/// production path as `session/live/012`), then force the live
...
#[test]
fn live_013_lagged_teardown_mid_session_is_recovered_by_reconnect_rehydration() {
    let rt = tokio::runtime::Builder::new_multi_thread()
        .worker_threads(4)
        .enable_all()
        .build()
...
/// Scenario: Same bootstrap as `session/live/013`, but the subscription dies
...
#[test]
fn live_014_transport_drop_mid_session_is_also_recovered_by_reconnect_rehydration() {
    let rt = tokio::runtime::Builder::new_multi_thread()
        .worker_threads(4)
        .enable_all()
        .build()
