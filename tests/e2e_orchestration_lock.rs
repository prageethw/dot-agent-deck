...
fn lock_017_paste_gated_by_lock_state() {
    ...
    // infer whether the drop had stamped `last_pane_keystroke_at`. That PTY
    // stopwatch is gone (issue #390) — a genuine SUBMIT_DEBOUNCE signal is
    // bounded at 150ms, but observed deltas on a loaded CI runner reached
    // 424ms, so the assertion's unbounded margin was blaming tier-load
    // jitter for the debounce roughly half of all `main` runs. The property
    // itself — `gate_paste_delivery` must not stamp `keystroke_at` on a
    // denied paste — moved to deterministic unit tests in `src/ui.rs`:
    // `lock_020` (denied while locked, carries no timestamp to stamp at
