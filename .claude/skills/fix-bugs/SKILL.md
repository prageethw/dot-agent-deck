...
7. **Upstream offer** (rule 19), once merged: apply rule 19's decision test — "would upstream want this?" If the merged fix is a genuine correction upstream would also want, and it lands on code the fork had already diverged on (so it wasn't filed-and-left-for-upstream at discovery time), file the upstream-offer tracking issue now, in the shape of `#322` — naming what to port, where upstream's matching issue is if one exists, and what makes the port non-trivial. If the issue already carries `affects-upstream` (the defect was confirmed to still reproduce on `vfarcic/dot-agent-deck` and an upstream issue was linked at filing time — rule 19's "defect upstream still carries" path), that filing already happened; don't re-file, just note it in the merge report. Read CLAUDE.md rule 19 directly before applying this — the decision test, the four "file upstream" shapes, and the maintainer-only closing decision are easy to paraphrase wrong.
