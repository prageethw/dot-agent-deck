...
This repo's actual label set is the source of truth for classification — run `gh label list --repo prageethw/dot-agent-deck` and check live open-issue usage (`gh issue list --repo prageethw/dot-agent-deck --state open --label <name>`) rather than trusting doc prose over what's actually in use. `docs/develop/work-types.md` documents the intended BUG/PRD/DOC/CHORE vocabulary but is itself stale against the live repo on three points as of this writing: it says a `PRD` label should not exist ("would duplicate `enhancement`") though one does exist and is in active use; it gives `doc`-type work no label though `documentation` is in active use; and it maps `prd`-type work to `enhancement` alone, which live usage does not follow. **Don't file a new documentation-correction issue for this** — issue #340 already covered this exact ground (reconciling the fork's work-type vocabulary, including `work-types.md`) and closed `COMPLETED` via merged PR #348. If `work-types.md` still looks stale on the three points above, cite #340 as prior art rather than duplicating it; only reopen it if the specific gap you're seeing is something #348 genuinely didn't address. Either way, don't fix the doc as part of this pass — just don't let it mislead a future reader here. Until that's resolved, classify against the live label set: `bug`, `PRD`, `enhancement`, `chore`, `documentation`, plus cross-cutting labels (`ci-cd`, `config`, `tests`, `source`, `affects-upstream`). Map the requested BUG/PRD/DOC/CHORE buckets onto these:
- **BUG** → `bug`
...
Group by subject, not by title wording — two issues describing the same underlying defect from different angles (a symptom report and a root-cause report) are duplicates even with no shared keywords. `gh issue list --repo prageethw/dot-agent-deck --search "<keywords>"` per candidate cluster, but the actual read is manual: skim bodies and comments for the same file/function/mechanism. **Compose `<keywords>` from a handful of distinctive terms you pick out of the issue** — the affected component, the symptom, the mechanism — never interpolate the raw issue title verbatim into the `--search` string: a title can carry quoting characters that break the query, and its exact wording is often too narrow (misses a duplicate phrased differently) or too broad (a common word matches unrelated issues) to be a useful search term on its own.
...
Re-list open issues (Step 1's query — note its `--limit 300` is a stated cap; if the returned count ever equals the limit, raise it and re-run rather than assuming full coverage) and confirm every one now carries at least one classification label, one size, one priority (or `needs-triage` where priority is genuinely uncertain), and no unresolved duplicate. **`PRD` alongside `bug` or `chore` is an intentional, valid combination** — some issues are defects large enough to run the full PRD workflow and own a doc under `prds/`; never strip a second classification label just to satisfy this check. Also exempt any issue carrying `wontfix`/`duplicate`/`invalid`/`question` from the size/priority requirement — `issue-triage.yml` makes the same four exemptions and does not stamp `needs-triage` on them, so holding this pass to a stricter bar than the workflow itself would just manufacture disagreement. **A fifth exemption: `in-progress`.** Step 2 forbids every write to an issue another orchestration holds, including a label add — so an `in-progress` issue missing size/priority (live right now: #282) can't be brought into compliance by this pass without violating Step 2's own rule. Report it in Step 2's "held by another orchestration" bucket instead, and exclude it from this step's "needs classification" completion requirement rather than leaving that requirement silently unsatisfied.
