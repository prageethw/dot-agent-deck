...
`-D` here, and the reason is worth knowing: `git branch -d` **always refuses** a review branch, because it holds the contributor's commits and those are by definition not on `main` — squash-merging the PR does not change that, since the commits never land verbatim. This used to say the same refusal was "real signal (work that should have been merged and wasn't)" over in `/tag-release` (`dot-ai-tag-release`'s Step 7), but that overclaims: `git branch -d` is ancestry-only, and ancestry says nothing about a squash merge anywhere, not just here — measured 2026-09-14 upstream, the naive check would have refused all 13 correctly-merged dispatch branches in a real sample. `dot-ai-tag-release`'s own `cleanup.sh` already verifies merge state correctly (PR-state SHA matching, not plain ancestry) before ever offering a branch, but Step 7 still hands the offered branch to plain `-d` first as a "safety net" — so a refusal there is still ancestry-based and known to false-positive on a squash-merged branch; treat it as a prompt to check manually (has `cleanup.sh` already vetted this branch as merged?), not as certain proof the branch is unmerged. Here, the branch is a disposable local copy of `refs/pull/<n>/head`, which lives on GitHub and `setup.sh` re-fetches on demand, so deleting it destroys nothing regardless — that is exactly why the `git log` line comes first: it is the one thing `-D` skips, so check that no commit in there is *yours* before dropping it.
