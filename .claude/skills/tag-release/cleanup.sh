...
# line, the same shape as `long_lived` above. Held as newline-delimited
# strings rather than `declare -A`, which is bash 4: macOS ships /bin/bash
# 3.2.57, where `declare -A` does not go quietly unsupported -- the
# assignment degrades to an ordinary INDEXED one, the subscript is evaluated
# arithmetically, and `set -u` kills the script on that line with a message
# naming a variable that appears nowhere in it.
merged_shas_lines=""
open_prs=""
...
is_merged() {
  ...
  if merged_sha_matches "$name" "$tip"; then
    vetted_tip="$tip"
    return 0
  fi
  return 1
...
while IFS= read -r b; do
  ...
  [ -z "$b" ] && continue
  [ "$b" = "HEAD" ] && continue
  # Fix-round note (14th upstream sync, `release_cleanup_003`): this used to
  # check only `[ "$b" = "$default_branch" ]`, disagreeing with the comment
  # right above it ("never long-lived") and with this file's own
  # `is_long_lived` used by the local-branch loop a few lines up -- `origin/
  # fork-only` is a real, trivially-"merged" remote branch that must never be
  # offered here, and the narrower check silently let it through.
  is_long_lived "$b" && continue
