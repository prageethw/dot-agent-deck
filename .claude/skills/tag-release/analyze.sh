...
#
# A maintainer runs it from `/tag-release` Step 1 to see the proposed version
# before tagging by hand (fork #152/#140/#1089-derived: this fork's own `main`
# carries no branch-protection ruleset, so it does not adopt upstream's
# `.github/workflows/tag-release.yml` dispatch-and-push-under-a-PAT automation
# — see CLAUDE.md rule 8 and `docs/develop/fork-sync-workflow.md`'s 14th-sync
# entry). The 0.x recalibration below is what `docs/develop/versioning.md`
# cites as the implementation of its bump table — keep the two in sync if you
# change either.
...
#
# `/tag-release` Step 3 acts on this directly: tagging a `[skip ci]` HEAD
# would stop `release.yml` running at all, so a `true` here means create an
# empty preparation commit first. The usual cause is `docs-publish.yml`, which
# ends every release by committing `chore: update docs chart to <tag> [skip
# ci]` to `main`.
