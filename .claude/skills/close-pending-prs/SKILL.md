...
4. **Delegate to `release` in two hops**, matching its actual role prompt: first, delegate normally (mark the PR ready if it's still draft, confirm CI green, report back) — it will stop there and report via `work-done` without merging. Only once you've verified the checklist above against its report, re-delegate explicitly instructing it to continue and merge: `gh pr merge <n> --repo prageethw/dot-agent-deck --match-head-commit <sha>`, where `<sha>` is the exact final head SHA you just confirmed CI is green against in the release role's own report — not a value to look up separately. Its prompt is written around `/prd-done`'s PRD-shaped flow (create branch → push → PR → merge → close issue); for a takeover PR that already has a PR and may have no PRD file, tell it explicitly that the PR already exists and only the ready/merge/close steps apply — it already treats "PR already exists" as the normal case, not an error.
