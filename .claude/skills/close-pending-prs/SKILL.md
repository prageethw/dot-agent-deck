...
```bash
gh pr list --repo prageethw/dot-agent-deck --state open --json number,title,author,assignees,isDraft,mergeable,mergeStateStatus,reviewDecision,statusCheckRollup,updatedAt,headRefName,headRepositoryOwner,labels,body,comments,commits
```

`comments,commits` are what Step 2's "push or comment in the last 7 days" and "bot comments don't count" signals actually need — without them the rule as stated has no data to run on. This is every PR in scope for this pass — open, draft or ready, whatever its CI/review state. There is no separate "incomplete" filter to apply; an open PR that is fully green and merge-ready is just a PR whose remaining step is Step 6's merge, not a different category.
...
**Also resolve push-ability now, before Step 3's assignment**, so you never assign yourself to a PR you cannot actually finish directly: check `headRepositoryOwner.login`. If it is not `prageethw` (a genuine fork-authored PR from an external contributor, not Renovate — Renovate's branches live on this repo), you generally cannot push fixes to it — the one exception is a contributor who left "Allow edits by maintainers" on, which `gh pr view <n> --json maintainerCanModify` reports; check it before assuming, but treat `false` (the common case) as the default. When you cannot push, route it to a review-only disposition: use `gh pr review --comment`/`--request-changes` to say what's needed, do not assign it to yourself as if you were going to finish it, and report it under "still blocked" with "cross-fork, cannot push" as the reason.
...
From this, write down exactly what remains before the PR can be complete: requested changes to address, unresolved threads, failing CI, merge conflicts with `main`, missing tests for a user-visible TUI change (CLAUDE.md rule 4 — keyed to what the change actually does, not to its labels: panes, statuses, prompts, focus, layout, modes, embedded panes, hook delivery all need coverage; a pure refactor with no observable behavior change does not — and if the PR is large enough to count as a *major* user-facing feature rather than a small tweak, rule 4's bar is higher than "some test exists": check for at least one PTY-attached L2 test and one real-agent e2e test on a cheap model, and whether it was validated as a user actually sees it), a missing/stale changelog fragment, or nothing at all (already done, just needs review/merge — go straight to Step 6).
...
**Before creating the worktree, give the diff the same lightweight look `verify-pr`'s Phase 0 gives an unfamiliar PR** — you are about to pull this content onto disk and potentially build/run against it; a quick scan for anything that shouldn't be treated as ordinary code costs little and this skill has no other safety-scan step.

**Fetch by PR number, not by branch name — this avoids ever putting attacker-influenceable text into a shell command.** `headRefName` is ordinary branch-name text from a public repo's API response; git ref names may contain shell metacharacters (`;`, `$()`, backticks, `&&`, `|` are all valid in a ref name — only whitespace and a handful of specific characters are forbidden), so a template that interpolates it unquoted into a command is a real injection surface, not a theoretical one. GitHub exposes every PR — including fork-authored ones — at the numeric, attacker-uncontrolled `refs/pull/<n>/head`. Use that, with the worktree path stated **absolutely** (this harness resets shell `cwd` between tool calls — CLAUDE.md rule 2 — so a relative `../` path is fragile; resolve it once against the root checkout's parent):

```bash
WORKTREE_ROOT="$(cd .. && pwd)"
git fetch origin "pull/<n>/head:pr-<n>"
git worktree add "$WORKTREE_ROOT/dot-agent-deck-pr<n>" "pr-<n>"
```

`pr-<n>` is a local branch name you construct from the PR number alone — never derived from PR-supplied text — and this checks out a real branch, not detached HEAD, because the fetch's refspec already created `pr-<n>` locally before `worktree add` references it. State the resulting absolute path, exact commit SHA, and branch name in every delegation for this PR.

**This is not idempotent — handle the retry/re-entry case explicitly.** Step 8 loops back over PRs still open, and a fetch into a `pr-<n>` that's already checked out somewhere fails (`refusing to fetch into branch`), as does `worktree add` onto a path that already exists. If a worktree for this PR already exists from an earlier pass: verify its `HEAD` still matches what you expect before reusing it (same discipline as any other worktree precondition check); if it's stale or you're unsure, remove it first (`git worktree remove`) and redo the fetch. **Once a PR is merged or closed (Step 6/7), clean up**: `git worktree remove` and `git branch -D "pr-<n>"` — this fork's root checkout accumulates worktrees fast, and a sweep through many stalled PRs multiplies that if nothing is ever removed.
...
```
If it refuses because another identity already holds the issue, do not take over — back out completely: remove the worktree, delete the local `pr-<n>` branch, **and undo Step 3's self-assignment** (`gh pr edit <n> --repo prageethw/dot-agent-deck --remove-assignee "$ME"`). The assignment must not survive a refused claim, or the PR is left looking taken over when it was not — the two are separate writes to separate objects (PR vs. issue) and only one of them is protected by a refusing command. Report the PR under "already owned by others" (the issue's claim is a stronger signal than anything Step 2 could infer from the PR alone) and move to the next candidate. Only escalate to a `--takeover --confirm-stopped` override after the user has explicitly confirmed the prior orchestration actually stopped — never infer that yourself. Note also that a PR with **no** linked issue gets no factual ownership check at all beyond Step 2's own signals — that is a real gap, not something this step closes, so weight Step 2's judgment call accordingly when a PR has no issue to claim.
...
2. **Check the full CLAUDE.md rule 25 checklist**, which is the actual gate on this fork (see "Governance" above — not GitHub's ruleset, which doesn't exist here). Every condition below must hold:
   - Every expected CI check present and passing — for the `e2e` job specifically, quote the literal `Summary [...] N tests run: …` line from the job log, never read the run's `conclusion` field (rule 8).
   - For a Renovate-authored PR, a missing CI row is expected, not a defect (rule 25's reduced-matrix carve-out: `ci.yml`'s `changes` job deliberately draws a smaller matrix when `PR_AUTHOR == renovate[bot]`).
   - Reviewer and auditor both returned on the final head.
   - Every agreed finding resolved and every behavioural fix pinned by a test (record the disposition and reason for anything a test cannot pin).
   - No blocker accepted rather than fixed.
   - `mergeable: MERGEABLE`.
   - `mergeStateStatus: CLEAN` — meaning no conflict/other structural block, **not** "approved" (per the Governance section above).
   - Closing-keywords audited clean on both the PR body and every commit message (`gh pr view <n> --json closingIssuesReferences`, plus a grep of every commit message for `close/fix/resolve #<n>`).
   - The changelog fragment present.

   **`mergeStateStatus` values other than `CLEAN`/`BLOCKED`** — with no ruleset and no required checks, `BLOCKED` is not something this fork's current setup can produce (that could change if branch protection is ever added, so treat this as descriptive of today's state, not a permanent guarantee), so in practice you will see: `DRAFT` (still draft — `gh pr ready <n>` first, rule 8 notes CODEOWNERS/draft interaction even though routing itself doesn't apply here), `UNSTABLE` (CI still pending or a check failing — since nothing here is a *required* check, a failure surfaces as `UNSTABLE`, not `BLOCKED`; back to Step 5/CI, not yet ready to evaluate), `BEHIND` or `DIRTY`/`CONFLICTING` (needs a rebase — Step 5's conflict-resolution path, and remember `CONFLICTING` means no CI run exists to read at all).
...
4. **Delegate to `release` in two hops**, matching its actual role prompt: first, delegate normally (mark the PR ready if it's still draft, confirm CI green, report back) — it will stop there and report via `work-done` without merging. Only once you've verified the checklist above against its report, re-delegate explicitly instructing it to continue and merge: `gh pr merge <n> --repo prageethw/dot-agent-deck --squash --match-head-commit <sha>`, where `<sha>` is the exact final head SHA you just confirmed CI is green against in the release role's own report — not a value to look up separately. Its prompt is written around `/prd-done`'s PRD-shaped flow (create branch → push → PR → merge → close issue); for a takeover PR that already has a PR and may have no PRD file, tell it explicitly that the PR already exists and only the ready/merge/close steps apply — it already treats "PR already exists" as the normal case, not an error.
...
Separate from Step 2's ownership question — these can be *anyone's* PR, including an actively-owned one, and the evidence bar is the same either way; ownership does not lower it. This step can also apply to a PR you never took over — one still owned by someone else, or one you haven't yet built a worktree for — so **if `pr-<n>` doesn't already exist from Step 5, fetch it the same safe way first** (`git fetch origin "pull/<n>/head:pr-<n>"`, no worktree needed just for a diff) rather than reusing a variable Step 5 never actually supplied for this PR.

- **Already integrated**: the PR's actual changes already landed on `main` some other way. **Fetch `origin/main` fresh and compare against it, not the local `main`** — the worktree's local `main` is routinely behind `origin/main` (this is not hypothetical: it was measured 2 commits behind during this skill's own review), and comparing against a stale `main` can miss an integrating commit that already landed. Use a **three-dot diff from the merge-base**, not a plain two-dot diff — a two-dot diff also shows everything `main` has gained since the branch point and will essentially never read as a no-op for a stale branch, which defeats the check:
  ```bash
  git fetch origin main
  git diff "origin/main...pr-<n>"
  ```
  An empty result means the PR's own changes are already fully present on `main`. Also check `git log origin/main --oneline --grep "#<n>"` (the PR or its linked issue number) for a commit that explicitly references it.
- **Superseded**: search **both trackers, issues and PRs** (CLAUDE.md rule 20) before concluding this — not PRs alone:
  ```bash
  gh pr list --repo prageethw/dot-agent-deck --state all --search '<keywords>'
  gh issue list --repo prageethw/dot-agent-deck --state all --search '<keywords>'
  ```
  A superseding PR that is open and further along has not landed on `main` yet by definition, so the diff check above cannot see it; only a live search will.
