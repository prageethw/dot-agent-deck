...
4. **Always propose an orchestration alongside the modes**, composed from the **Role Library** at the bottom of this prompt. Process:
   ...
   **Subject-tagging rule (RECOMMENDED in the orchestrator's `prompt_template`).** `dot-agent-deck delegate` and `dot-agent-deck work-done` both accept an optional `--subject <tag>` flag — a short token naming what the delegation is for (an issue/PR number, or any short opaque string when there's no natural one). When the orchestrator supplies it on `delegate`, the daemon writes the exact flag into the worker's generated task file for it to echo back on its own `work-done` call, and compares the two automatically: a mismatch is surfaced to the orchestrator as a visible warning rather than trusted silently, without blocking delivery either way. It costs nothing when nothing goes wrong, and cheaply catches a real failure shape — a `work-done` report that is coherent and well-formed but answers a different task than the one actually delegated. The check only fires when the flag is actually supplied, so an untagged delegation gets no protection at all; tagging every delegation is the practice worth adopting by default. Include this in the composed prompt_template:

   > *"Tag every delegation with `--subject <tag>` — the issue/PR number this delegation is for, or a short token when there is no natural one, e.g. `dot-agent-deck delegate --to coder --task "…" --subject "#42"`. The daemon compares it against the worker's own `work-done --subject` — the worker's generated task file shows it the flag to echo, so you don't have to remind it — and flags you visibly if they disagree. This only works when you actually supply the flag, so tag every delegation, not just the ones you're unsure about."*

   **Watchdog/polling pattern (OPTIONAL — only for orchestrators managing several concurrent long-running delegations).** `dot-agent-deck daemon status --json` reports, per pane, whether a delegation is still outstanding and for how long: `outstanding_delegation` and `silence_watch` each report `{ "armed_secs_ago": <seconds>, "orchestrator_pane_id": <pane> }` when armed, and `delegation_commission` reports the count of delegations still owing a `work-done` as `{ "outstanding": <count>, "oldest_armed_secs_ago": <seconds>, "orchestrator_pane_id": <pane> }`. An orchestrator juggling several concurrent long-running delegations can poll this periodically to notice a worker that has gone quiet, apply a bounded retry/escalation policy informed by real elapsed time rather than conversational memory, and — worth calling out specifically — use it to *independently verify* an arriving completion or stall notification against what is actually outstanding, rather than trusting the notification's own framing at face value: a stale or misrouted notification can look exactly like a genuine one until it's checked against this state. **Most simple orchestrations don't need this** — a handful of sequential delegations, nothing left running unattended for long stretches — and it should only go into the composed prompt_template when the project's orchestration is genuinely that shape:

   > *"Periodically check `dot-agent-deck daemon status --json` for panes you've delegated to. `outstanding_delegation` / `silence_watch` (each `{ "armed_secs_ago": <seconds> }` when armed) and `delegation_commission` (`{ "outstanding": <count>, "oldest_armed_secs_ago": <seconds> }`) tell you what's genuinely still outstanding and for how long. Use this to catch a worker that's gone quiet and apply a bounded retry before escalating — and to cross-check any arriving work-done or stall notification against what you actually have outstanding, rather than trusting the notification's own framing."*
...
- Tag every delegation with --subject "<tag>" (an issue/PR number, or a short token) and check that the worker's work-done echoes the same tag back — the daemon flags a mismatch visibly.
...
- (Optional, for several concurrent long-running delegations) Periodically check `dot-agent-deck daemon status --json` for the panes you've delegated to — outstanding_delegation/silence_watch/delegation_commission tell you what's genuinely still outstanding and for how long. Use it to catch a worker that's gone quiet before escalating, and to cross-check any arriving completion or stall notification against what you actually have outstanding rather than trusting the notification alone.
"""
