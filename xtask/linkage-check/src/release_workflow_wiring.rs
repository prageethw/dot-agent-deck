...
///
/// Token-precise rather than a `contains("--repo")`: a substring test also
/// accepts `--repository` and a bare trailing `--repo` with no value -- both
/// of which `gh` itself rejects, as an unknown flag and a missing argument
/// respectively -- so it would pass commands that still cannot run, which is
/// the one thing this guard exists to stop. `-R` is accepted because it is the
/// same flag, and rejecting it would fail a correct command.
...
/// Fork issue #712, ported from upstream's own fix for the same bug
/// (vfarcic/dot-agent-deck#852, PR #853): `desktop-publish` is the one job in
/// this file with no `actions/checkout` -- it downloads artifacts and calls
/// the API, so it needs no source tree -- and none of its three `gh` calls
/// passed `--repo`. `gh` then falls back to inspecting git for its target
/// repository, finds none, and dies with `failed to run git: fatal: not a git
/// repository`. Every release that reached the job failed inside it, so on
/// v0.44.4 through v0.44.6 the desktop alpha bundles were built correctly and
/// then never attached, and the release-body note the two later calls append
/// was never appended either.
///
...
/// the failure log would have been satisfied by a fix that merely relocated
/// the failure to the next call. Stated as a property it also covers the next
/// job added without a checkout, wherever it comes from.
///
/// Either remedy satisfies it: a `--repo` on each call, or a checkout for the
/// job. This does not pin which.
///
/// **Scoped to `release.yml`**, which is this module's file and the workflow
/// that never runs on a pull request -- so a static assertion is the only
/// thing that can catch this at all. It is *not* a claim about the other
/// workflows, and does not generalise to them as written: `ci.yml`'s
/// `changes` job reaches the API as `gh api repos/$REPO/…`, carrying its
/// repository in the request path rather than in a flag, and the
/// `*.lock.yml` files are generated.
#[test]
fn gh_calls_in_checkoutless_jobs_name_their_repository() {
        ...
        for call in gh_invocations(block) {
            assert!(
                ...
                "`{name}` has no `actions/checkout` step, so its workspace \
                 ...
                 repository`. That is fork issue #712 (upstream #852): every \
    ...
    // regardless of whether the job checks out, so this stays live under
    // either remedy above.
    ...
    assert!(
        ...
        "expected `desktop-publish` to still hold the three `gh` invocation \
         LINES that issue #712 was about, matched {publish_lines}. Lines, not \
