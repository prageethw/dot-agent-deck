...
//!  12. No `##### <id>` catalog heading appears more than once
...
fn main() -> ExitCode {
    ...
    // Check 12 (fork #148): a catalog heading ID must not repeat. Two
    ...
    for (id, count) in &catalog_ids {
        if *count > 1 {
            failures.push(format!(
                "[12] catalog id `{id}` has {count} `##### {id}` headings in {} (duplicate heading, fork #148)",
