...
//!      [`UNARMED_SPAWN_RULE`].
//!  11. No `##### <id>` catalog heading appears more than once
...
/// printed "9 rules", issue #259 added a twelfth check without touching
/// any of them, and fork #281 added a thirteenth. The same shape as
/// `work_type`'s own (private, unrelated, five-rule) `RULE_COUNT`, which
/// exists for the identical reason one module over.
const CHECK_COUNT: usize = 13;
...
fn main() -> ExitCode {
    ...
    // the thirteen Decision-7 / Decision-30 / issue #322 / fork #148 / issue
    ...
    // Check 11 (fork #148): a catalog heading ID must not repeat. Two
    ...
    for (id, count) in &catalog_ids {
        if *count > 1 {
            failures.push(format!(
                "[11] catalog id `{id}` has {count} `##### {id}` headings in {} (duplicate heading, fork #148)",
                catalog_path.display()
            ));
        }
    }

    // Check 13 (fork #281): a catalog id this branch adds must not also
    ...
    let check_13_note = match check_cross_branch_catalog_collisions(&root) {
        Ok((origin_main_sha, compared, check_13_failures)) => {
            let note = format!(
                ", check 13 compared {compared} newly-added id(s) against {}'s tip {}",
                work_type::DEFAULT_BASE,
                &origin_main_sha[..origin_main_sha.len().min(12)],
            );
            failures.extend(check_13_failures);
            note
        }
        Err(reason) => format!(", check 13 skipped ({reason})"),
    };
    ...
    if failures.is_empty() {
        ...
        println!(
            "linkage-check: ok ({} catalog ids, {} annotations, {} allowlisted, {CHECK_COUNT} \
             rules{check_12_note}{check_13_note})",
            catalog_ids.len(),
            discovered.len(),
            allowlist.len()
        );
...
fn check_cross_branch_catalog_collisions(
    ...
    let origin_main_sha = match rev_parse(root, ORIGIN_MAIN_REF) {
        ...
        Err(e) => {
            ...
            eprintln!("linkage-check: [13] skipped ({reason})");
            return Err(reason);
        }
    };
    ...
    for (id, branch_bodies) in &entries_on_branch {
        ...
        failures.push(format!(
            "[13] catalog id `{id}` is newly added by this branch with content that differs \
