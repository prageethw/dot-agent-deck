...
/// and the has-task variant jumped straight to "carry out that task" — both
/// pointers named the file's role/agents/delegation content but never the
/// workspace-sync step. Both variants now name it explicitly. This runs the
/// same way through [`reassert_orchestrator_prompt`] (used on
...
///
/// The has-task sentence "Then carry out that task, delegating to the agents
/// listed there." is kept byte-for-byte at the end, on its own sentence —
/// `tests/e2e_orchestration_remit.rs`'s `CARRY_OUT_TASK_POINTER` const pins
/// the literal substring "Then carry out that task" (capital T,
/// sentence-initial) against a real spawned pane; folding the workspace-sync
/// mention into the SAME sentence (e.g. "...first, then carry out...")
/// lowercases that "then" and silently breaks that real e2e assertion.
...
mod tests {
    ...
    fn prepare_orchestrator_prompt_pointer_names_the_workspace_sync_step() {
        ...
        let no_task = prepare_orchestrator_prompt(&config(), &cwd, None, Attendance::Attended)
            .expect("written");
        assert!(
        ...
        let has_task = prepare_orchestrator_prompt(
            &config(),
            &cwd,
            Some("Verify PR #232 and report."),
            Attendance::Attended,
