...
/// {cwd}" wording is actively false — a config *was* found, one line above,
/// and was declined rather than absent — and it names `cwd`, which can be a
/// different directory from the one the resolution actually froze at (the
/// coda names the real one). In that case this swaps to a clause that
/// doesn't claim absence and doesn't name a directory the decline lines
/// above already covered.
fn missing_config_warning(path: &std::path::Path, declined_any: bool) -> Option<String> {
...
pub fn init_and_watch() -> Vec<String> {
    ...
    INIT.call_once(|| {
        ...
        if let Some(message) = missing_config_warning(&path, declined_any) {
            tracing::warn!("{message}");
            warnings.push(message);
        }
