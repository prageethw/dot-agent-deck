...
/// `agent_hook_config::build_command`'s `HookShell::Posix` arm rather than
...
/// `cmd.exe` will run — see [`cmd_quote_if_needed`], selected via
/// [`native_shell_command_word`] — and it stays the unconditional one for
