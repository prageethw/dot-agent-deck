...
///
/// **The exit status is deliberately not consulted, unlike the bulk-phase
/// sample's.** `ps -p <list>` reports failure when *no* pid in
...
///
/// The bulk-phase table is not atomic: it is captured at one instant and the
/// `getsid(2)` pass happens at a later one, so a pid that exits in between can
/// be recycled and `getsid` then answers about a *different* process than the
/// row describes (fork issue #30). Every row's identity is therefore confirmed
/// with a SECOND bulk-phase `ps` sample taken right after the first — see
/// [`sample_table`] for why the ordering matters and
/// [`super::scan::invalidate_unconfirmed_session_ids`] for the exact invariant.
/// The argv phase (issue #862) rides on top of the confirmed table and costs a
/// THIRD fork/exec only when some root actually has a detached descendant,
/// batched across every root. The session id of each row comes from
/// [`getsid_or_negative`], not from `ps`.
///
/// **Synchronous — never call this from an async task** (issue #429). It
/// blocks the calling thread for the whole run — two bulk-phase `ps`
/// invocations back to back, measured at ~49 ms each on an idle 16-core Linux
/// box with ~620 processes, plus the argv phase when it fires.
/// [`process_table_async`] is the variant for a Tokio context. This one
/// remains for synchronous callers and tests.
///
/// Every capture in this call — both bulk-phase samples and the argv phase —
/// goes through [`capture_bounded`], so a `ps` wedged in D-state is killed and
/// contributes `None`/an empty map after roughly [`PS_SAMPLE_BUDGET`] rather
/// than blocking the calling thread forever, and an oversized capture is
/// rejected the same way (fork issue #212's byte cap — see
/// [`PS_SAMPLE_BYTE_CAP`]).
///
...
pub fn process_table(roots: &[i32]) -> Option<Vec<super::ProcessInfo>> {
    ...
    let pid_list = ps_pid_list(&wanted);
    let argv_args: Vec<&str> = PS_COMMAND_LINE_ARGS
        .iter()
        .copied()
        .chain(["-p", pid_list.as_str()])
        .collect();
    let argv = capture_bounded("ps", &argv_args, PS_SAMPLE_BUDGET)
        .map(|stdout| command_lines_from_ps_output(stdout.as_bytes()))
        .unwrap_or_default();
...
///   the queue for the ~49 ms each bulk-phase sample takes instead of sitting
///   in `waitpid`. That cost, twice per tick for the confirmation pass (plus a
///   third fork/exec when the argv phase fires), is what previously stalled
///   hook ingestion, client requests and daemon shutdown behind this signal —
///   including the pre-#493 500 ms poll with zero panes open. `spawn_blocking`
///   would only relocate that stall to the blocking pool; worse,
///   `tokio::time::timeout` around a `spawn_blocking` handle does not cancel
///   the thread, so a permanently-wedged `ps` at 2 Hz would leak one pool
///   thread per tick until the 512-thread cap. Awaiting an async child is what
///   actually fixes it.
...
///   abandoning it. Every phase is inside this one future, so the caller's
///   single deadline bounds the whole sample and the argv phase needs no
///   deadline of its own.
///
...
pub async fn process_table_async(
    roots: &[i32],
) -> Result<Vec<super::ProcessInfo>, super::ProcessTableOutcome> {
    let mut table = sample_table_async(
    ...
    let argv_args: Vec<&str> = PS_COMMAND_LINE_ARGS
        ...
        .chain(["-p", pid_list.as_str()])
        .collect();
    let argv = capture_bounded_async("ps", &argv_args, PS_SAMPLE_BUDGET)
        .await
        .map(|stdout| command_lines_from_ps_output(stdout.as_bytes()))
        .unwrap_or_default();
    super::fill_command_lines(&mut table, &wanted, &argv);
...
mod tests {
    ...
    #[spec("status/shell-activity/013")]
    #[test]
    fn shell_activity_013_capture_bounded_async_remediation_arms_never_leave_kill_and_wait_unbounded()
     {
