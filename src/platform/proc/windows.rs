...
/// is nothing to await and nothing that can block, and it never awaits
/// anything: there is no subprocess to bound, so the caller's timeout simply
/// never fires here (issue #429).
///
