...
pub enum Tab {
    ...
    Orchestration {
        ...
        /// observer runs once per frame UNCONDITIONALLY (never inside a
        ...
        /// [`TabManager::observe_waiting_panes`] and consumed by
        /// [`TabManager::auto_focus_all_clear`]. Splitting the observation
        /// from the focus move is what makes the edge survive: the move
        /// only runs when the chain reaches its branch, while the
        /// observation must happen every frame regardless of which branch
        /// wins — recording the edge in the mover meant a waiting episode
...
impl TabManager {
    ...
    /// PRD #373 M1 — fork-only. The OBSERVATION half of the all-clear
    /// edge trigger, and the sole writer of the active Orchestration
    /// tab's `had_waiting_pane`: records whether any role pane on that tab
    /// is `WaitingForInput` right now, and latches the `true` → `false`
    /// transition into `all_clear_pending` for
    /// [`Self::auto_focus_all_clear`] to consume. Moves no focus and
    ...
    /// (`role_pane_ids[start_role_index]`) exactly once. No-op for any
    /// active tab that isn't `Tab::Orchestration`.
    ///
    /// Deliberately edge- rather than level-triggered: the move fires only
    ...
    /// the call site in `src/ui.rs`. That gate is safe *because* the
    /// observation is no longer done here: on the latch frame nothing is
    /// waiting, so `auto_focus_waiting_pane` returns `None` by
    /// construction and this method is always reached. Skipping the call
    /// for a frame (the render loop does, when input is already pending)
    /// only DEFERS the move — the latch survives until it is consumed.
    /// Already-correct focus (already on the orchestrator) still consumes
    /// the latch, and is otherwise a no-op matching
    /// `auto_focus_waiting_pane`'s no-flicker behavior.
...
mod tests {
    ...
    /// `SessionStatus` map through `TabManager::auto_focus_waiting_pane` to
    /// pin the exact resolution rule: no waiting panes leaves manual focus
    /// alone, a newly-waiting pane steals focus, ties resolve to the
    /// lowest-order waiting pane (even stealing focus mid-input from a
    /// higher-order pane that is itself still waiting), an already-lowest
    /// focused pane is a no-op, and resolving the focused pane advances to
    ...
    #[test]
    fn orchestration_010_auto_focus_follows_lowest_order_waiting_pane() {
        ...
        // `beta` (non-focused, highest order) transitions to
        // WaitingForInput: it steals focus from the manually-set `orchestrator`.
        ...
        // `alpha` is already the lowest-order waiting pane and already
        // focused: a repeat call must be a no-op (no flicker).
        assert_eq!(tm.auto_focus_waiting_pane(&status), None);
        assert!(matches!(
            &tm.tabs[orch_idx],
            Tab::Orchestration { focused_role_pane_id: Some(p), .. } if *p == alpha
        ...
        // `orchestrator` (lower order than the currently-focused `alpha`)
        // newly transitions to WaitingForInput while `alpha` is itself
        // still waiting: focus jumps to `orchestrator` anyway — the
        // confirmed "steal focus mid-input" tradeoff.
        ...
        // Resolving the focused pane (`orchestrator` -> Idle) advances focus
        // to the next-lowest STILL-waiting pane (`alpha`), even though the
        ...
        // now-BACKGROUND first tab — the lowest-order role, which would
        // steal focus if that tab were active.
        status.insert(orchestrator.as_str(), SessionStatus::WaitingForInput);
        let result = tm.auto_focus_waiting_pane(&status);

        // The active tab (`orch-2`) has none of its own panes represented
        // in `status`, so there is nothing to focus there.
        ...
        // unaffected by `orchestrator`'s newly-arrived WaitingForInput
        // status while that tab isn't the active one.
        assert!(matches!(
            &tm.tabs[orch_idx],
            Tab::Orchestration { focused_role_pane_id: Some(p), .. } if *p == alpha
        ));
    ...
    fn orchestration_012_all_clear_focus_move_is_edge_triggered() {
        ...
        // `auto_focus_all_clear` only runs when `auto_focus_waiting_pane`
        // found nothing to steer toward.
    ...
    /// focused. Drives the real per-frame sequence (unconditional
    /// `observe_waiting_panes`, then `auto_focus_waiting_pane` →
    /// `auto_focus_all_clear`): the first frame steers focus onto the
    /// waiting role and the second must still fire the all-clear move back
    /// to the orchestrator. `orchestration_012` always has a still-waiting
    /// frame in between, which is what let the dropped edge hide.
    #[spec("tabs/orchestration/020")]
    #[test]
    fn orchestration_020_all_clear_survives_a_single_frame_waiting_episode() {
        use crate::state::SessionStatus;
        ...
        // Mirrors the real per-frame call site in `src/ui.rs`: the
        // observation runs first and unconditionally, then the chain.
        fn frame(tm: &mut TabManager, status: &HashMap<&str, SessionStatus>) -> Option<String> {
            tm.observe_waiting_panes(status);
            tm.auto_focus_waiting_pane(status)
                .or_else(|| tm.auto_focus_all_clear())
        ...
        // The human is looking at `alpha`, and nothing has ever waited on
        // this tab yet, so a quiet frame moves nothing.
        if let Tab::Orchestration {
            focused_role_pane_id,
            ..
        } = &mut tm.tabs[orch_idx]
        ...
        // Frame 1: `beta` starts waiting. `auto_focus_waiting_pane` steers
        // focus onto it and therefore WINS the chain, so
        // `auto_focus_all_clear` never runs on this frame — the only frame
        // in which `beta` is observed waiting.
        status.insert(beta.as_str(), SessionStatus::WaitingForInput);
        assert_eq!(frame(&mut tm, &status).as_deref(), Some(beta.as_str()));

        // Frame 2: `beta` resolves. The all-clear move must still fire —
        // the waiting history has to be recorded by the unconditional
        // observation, not by whichever branch of the chain happened to
        // run. Recording it inside `auto_focus_all_clear` lost this edge
        // entirely and left focus stranded on the resolved `beta`.
        status.insert(beta.as_str(), SessionStatus::Idle);
        assert_eq!(
            frame(&mut tm, &status).as_deref(),
            Some(orchestrator.as_str()),
    ...
    /// Scenario: Greptile's confirmed P1 on PRD #393's lock-governed focus
    /// chain (`.dot-agent-deck/waiting-focus-race-findings.md`) — the
    /// waiting-focus branch has no pending-input guard, unlike its
    /// all-clear sibling right below it in `src/ui.rs`. Within a locked
    /// orchestration tab (roles `orchestrator` < `alpha` < `beta`), `beta`
    ...
    /// clears on a later frame, the deferred steer to `alpha` must still
    /// fire — proving the guard defers the move rather than dropping it,
    /// mirroring `TabManager::auto_focus_all_clear`'s existing "no
    /// one-shot latch" contract. Drives the not-yet-existing
    /// `TabManager::auto_focus_locked` — the seam this test asks the coder
    /// to add, folding both `auto_focus_waiting_pane` and
    /// `auto_focus_all_clear` behind ONE `input_pending` guard shared by
    /// both branches, mirroring the real per-frame call site's shape.
    #[spec("tabs/orchestration/028")]
    #[test]
    fn orchestration_028_waiting_focus_defers_while_input_pending() {
        use crate::state::SessionStatus;
        ...
        // Mirrors the per-frame call site `src/ui.rs` SHOULD have: both
        // branches gated on the SAME `input_pending` guard, computed once
        // per frame in production from
        ...
        assert_eq!(
            ...
            "a waiting-focus steal must be deferred, not applied, while a \
             keystroke is still queued for the currently-focused waiting \
             pane -- this is the Greptile P1 on PRD #393's lock-governed \
             focus chain: the waiting branch had no pending-input guard"
        );
        assert!(
            matches!(
                &tm.tabs[orch_idx],
        ...
        // The queued keystroke has now been drained (input no longer
        // pending). `alpha` is still WaitingForInput and still lower-order
        // than `beta`, so the deferred steer must fire NOW -- proving the
        // guard defers the move rather than dropping it, exactly as
        // `auto_focus_all_clear`'s existing pending-input guard already
        // behaves for its own branch.
        assert_eq!(
            frame(&mut tm, &status, false).as_deref(),
            Some(alpha.as_str()),
