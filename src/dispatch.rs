...
// `RollbackOutcome`/`rollback_dispatched_worktree` are `#[cfg(test)]`-only
// (PRD fork#325 M3, Model B) — see that function's doc comment.
#[cfg(test)]
use crate::issue_dispatch_run::RemoveOutcome;
...
pub async fn handle_dispatch(
    ...
    .await
    {
        Ok(handle) => {
            ...
            //
            // The base clause is appended rather than interpolated into both arms
            // so a failed probe degrades to the pre-#674 sentence exactly, instead
            // of leaving a dangling "cut from ".
...
///
/// PRD fork#325 M3 (Model B, issue #490): `handle_dispatch`'s own rollback no
/// longer calls this — its two mechanisms (shared-checkout vs isolated-clone)
/// need different rollback shapes, handled inline above. Kept `#[cfg(test)]`
/// rather than deleted: `the_rollback_leaves_a_worktree_that_a_live_agent_is_rooted_in`
/// below still pins this exact guard directly, at its own seam, independent
/// of whichever production call path exercises the shared-checkout case.
#[cfg(test)]
