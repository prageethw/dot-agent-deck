...
impl AgentEvent {
    ...
    /// (issue #684, [`CARD_SURFACE_SESSION_START_ORIGIN`]), and the flat
    /// live-surface `SessionStart` [`crate::spawn`]'s `surface_spawned_pane`
    /// forges to paint an already-attached TUI's card for a freshly-spawned
    /// scheduled pane (issue #549 fix round 2, reviewer H1 follow-up). Each
    /// hazards `crate::ui::evidence_channel_is_unidentified` a different way,
    /// so excluding all of them closes several distinct gaps rather than one:
    ///
...
pub enum BroadcastMsg {
    ...
    /// Issue #717 / PRD 236: a dispatched worktree the daemon KEPT on tab close
    /// instead of removing (uncommitted work, or a status probe that itself
    /// failed) — see [`crate::issue_dispatch_run::RemoveOutcome`]. The close
    /// handler runs detached from the close response (`daemon_protocol.rs`),
    /// after `close_agent` reaped the agent and after the pane and its card
    /// were destroyed — there is no card left to put it on, which is exactly
    /// why issue #424's `DeliveryNotice` cannot carry it (its pane-ownership
    /// guard drops a report for an agent that no longer owns the pane) — so
    /// this is a deck-level broadcast rather than a reply riding the close
