...
pub enum EventType {
    ...
    /// hooks/wrapper) when a pane's PTY foreground process group differs from
    /// its shell's own — i.e. a shelled-out command is actively running in
    /// the foreground with no agent-emitted event to say so. See
    /// [`crate::state::AppState::apply_event`]'s `ShellBusy`/`ShellIdle` arms
    /// for the precedence rules against real, agent-emitted status.
    ShellBusy,
    /// PRD #370 M2: the paired synthesized event — the foreground pgid has
    /// reclaimed the shell's own, i.e. the previously-running foreground
    /// command has finished. See [`ShellBusy`](Self::ShellBusy).
    ShellIdle,
