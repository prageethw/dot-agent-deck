...
// Issue #232: every production call site above has moved to
// `sanitize_path_for_terminal_display` (`src/terminal_sanitize.rs`), which
// additionally escapes bidi/format control characters this function's plain
// `escape_debug`-based rendering does not. Kept — not deleted — because its
// own two unit tests below still pin genuinely distinct properties (raw-byte
// non-UTF8 injectivity) that predate this round's fix; confirmed via
...
///   ever reached, so it is never vouched for (`worktree/reclaim/081`).
///
...
mod tests {
    ...
    #[test]
    fn worktree_reclaim_080_forged_directory_inherits_a_vacated_linked_worktree_lock() {
        let scratch = tempfile::tempdir().unwrap();
        let repo = scratch.path().join("repo");
        init_repo_with_origin(&repo);
    ...
    #[test]
    fn worktree_reclaim_081_pre_planted_directory_survives_already_claimed_as_owned() {
        let scratch = tempfile::tempdir().unwrap();
        let repo = scratch.path().join("repo");
        init_repo_with_origin(&repo);
