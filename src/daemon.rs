...
async fn ingest_event(
    ...
    ingest_event_with_hook(state, event_tx, registry, event, || async {}).await;
}
...
/// concurrent producer cannot get in (`status/shell-activity/009`).
///
                        ...
                        if let Ok(msg) = serde_json::from_str::<DaemonMessage>(&line) {
                            match msg {
                                ...
                                DaemonMessage::Dispatch(signal) => {
                                    ...
                                    // `deliver_dispatch_result`'s guarded write silently
                                    // gate on a value we never actually resolved.
...
mod ingest_event_ordering_tests {
    ...
    #[test]
    fn shell_activity_009_ingest_event_is_atomic_across_broadcast_and_apply() {
        ...
        tokio::runtime::Builder::new_current_thread()
            ...
            .block_on(shell_activity_009_ingest_event_is_atomic_across_broadcast_and_apply_inner());
    }

    async fn shell_activity_009_ingest_event_is_atomic_across_broadcast_and_apply_inner() {
        ...
        let mut second = Box::pin(ingest_event(
            &state,
            &event_tx,
            &registry,
        ...
        let monitor_handle = tokio::spawn({
            ...
            async move {
                run_shell_activity_monitor_with(registry, state, event_tx, move |_roots| {
                    ...
                    async { Err(crate::platform::proc::ProcessTableOutcome::Failed) }
                })
                .await
            }
        });
