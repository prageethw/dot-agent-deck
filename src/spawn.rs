    ...
    match target {
        ...
        SpawnTarget::Orchestration {
            ...
            for (idx, role) in roles.iter().enumerate() {
                ...
                //
                // Fork #358 M2: confirm the generation reserved BEFORE this
                // role's spawn (and already injected into its env) via
                // `confirm_orchestration_role`, rather than the plain
                // `register_orchestration_role`, which would reserve a SECOND
                // generation here and desynchronize the map from what the
                ...
                if let Some(state) = state.filter(|_| {
                    ...
                    state.write().await.confirm_orchestration_role(
                        ...
                        reserved_generation.expect(
                            "reserved_generation is Some whenever `state` was Some at \
                             reservation time, which this branch (state.filter(..).is_some()) \
                             requires — same `state` binding as the reservation above",
                        ),
                    );
                }
...
///
/// The nonce+sequence carry the uniqueness, so it is the SANITIZED NAME that
/// gets truncated — never the suffix. Two long task names sharing a prefix
/// therefore produce ids that differ only in the nonce+sequence, which is
/// exactly what that suffix is for.
...
mod tests {
    ...
    async fn dispatch_016_detached_retry_stops_before_replacement_or_clear() {
        ...
        // Wait for the payload's own echo rather than betting a fixed sleep on
        // the retry loop plus a PTY round trip (issue #892) — issue #422 item 2
        // widened the first unconfirmed-retry window to 10s, so a fixed sleep
        // here would need to clear that before the arm-then-write can happen.
        // Unlike the forged twin above — whose sleep IS its observation,
        // because it asserts the payload never arrives — this case asserts
        // that it DOES, so there is a byte to be keyed on and load can make
        // the wait slower instead of wrong. Nothing wrote this payload to this
        // pane before, so the first copy to appear is the one under test.
        let spawned_output =
    ...
    async fn dispatch_018_user_input_disarms_detached_submit_probe() {
        ...
        // that follows.
        let after_attempt_one = wait_for_detached_payload_echo(&registry, &agent_id, PROMPT).await;
        assert!(
            after_attempt_one
                .windows(PROMPT.len())
                .any(|window| window == PROMPT.as_bytes()),
