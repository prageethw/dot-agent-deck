...
/// (upstream #309/#330). Shares [`request_from_socket_inner`] with
/// [`request_from_socket`], so `delegate` gets the same total-operation-
/// deadline coverage against a dribbling peer that `get-seed` does, scaled to
/// whatever `timeout` the caller passes rather than
/// [`GET_SEED_REQUEST_TIMEOUT`].
...
/// deadline rather than the per-read idle timeout `timeout` already puts on
...
/// or a clean EOF — see [`request_from_socket`].
///
/// When `timeout` is `None` this falls back to unbounded blocking reads with
/// no re-arming, for callers that pass no deadline at all.
fn read_reply_line(
