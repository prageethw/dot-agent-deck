...
The primary intended use of `pane restart` is not a human reaching for it from the TUI — it is the orchestrating agent recovering on its own. When a worker pane's agent crashes, the daemon already writes a notice straight into the orchestrator's own pane (fork issue #465's existing `deliver_worker_exited_notice` mechanism, pre-existing and unrelated to this feature):

```
⚠ delegated worker exited without work-done (dot-agent-deck daemon report): the process
behind pane <pane-id> ended and no work-done was ever received for its outstanding
delegation. Check that pane's scrollback for what happened; the daemon log names the
role and how long it had been delegated.
```

Before this feature, reading that notice was as far as an orchestrator could go — recovering the pane still needed a human to notice and act from the TUI. `pane restart <role>` closes that gap: the orchestrator reads the notice, identifies the crashed role, and calls `dot-agent-deck pane restart <role>` itself, bringing the worker back without anyone needing to look at the deck. A `prompt_template` line that tells the orchestrator to do exactly that turns a crash into a self-recovering pipeline rather than a human-operator escape hatch:

```
If you receive a "delegated worker exited without work-done" notice, run
`dot-agent-deck pane restart <role>` for that role yourself, then re-delegate
the task it was working on. Only ask the user if the restart itself fails.
```
...
Pin or unpin an isolated clone against `worktree reclaim`'s automatic removal directly from the CLI, without hand-editing the provenance artifact yourself: `dot-agent-deck worktree pin <path>` and `dot-agent-deck worktree unpin <path>` (fork issue #597). `<path>` may be relative or absolute, with or without a trailing slash — it is resolved to its real, absolute location before use, so `worktree pin <name>`, `worktree pin ./<name>`, and `worktree pin <name>/` (what shell tab-completion produces) all behave identically. Both fail cleanly, naming the reason, when `<path>` doesn't resolve to anything on disk, or when it resolves to something that isn't an isolated clone this deck provisioned (a linked worktree, or any other directory with no provenance record) — neither ever fabricates a provenance artifact for a path it doesn't recognize. `worktree list --json`'s `pinned` field reports the result: `true` once pinned, `false` once explicitly unpinned. Like `owner_kind`/`owner_reason` above, this field has one semantic edge worth knowing: `pinned` is only meaningful for an isolated-clone row that has a resolvable pin state at all. An isolated-clone row with no attach-lock provenance artifact reports `pinned: false`, which is not "explicitly unpinned" — it is "this row was never eligible for pinning in the first place," since such a row already fails the first reclaim-eligibility gate on its own. An isolated-clone row whose provenance artifact exists but can't be read reports `pinned: true` — the same fail-closed posture the reclaim-eligibility gate itself uses, treating "can't tell" as "protected" rather than as "not pinned."
...
This works immediately after a daemon restart: ownership is matched by comparing the worktree's on-disk marker against the pane's own `DOT_AGENT_DECK_WORKTREE_OWNER` environment variable (set automatically in every pane of an orchestration that **created a worktree** — not every orchestration pane; one started directly in an existing checkout, with no worktree slug typed, carries no variable at all. `DOT_AGENT_DECK_PANE_ID` really is set in every orchestration pane, so do not read the two as equivalent), never by asking the daemon. This identity is also saved in your session, so closing and reopening a tab restores it with the SAME identity — `--mine` still matches the worktrees it created earlier, with two exceptions: a tab whose saved session predates this — one saved before the identity was captured at all, or before this specific field was added — restores with no identity; and a tab that was reattached to a still-running ("warm") daemon has its metadata rebuilt without the saved identity, so a session saved in that state also restores with none. Either way, `--mine` from that tab still fails loudly — `DOT_AGENT_DECK_PANE_ID` is set in every orchestration pane regardless of whether the owner variable survived, so the deck can always tell it is still inside an orchestration pane and refuses to guess, which could hand it another orchestration's worktrees. A pane whose orchestration created no worktree fails loudly for the same reason. Only truly outside any orchestration pane — no `DOT_AGENT_DECK_PANE_ID` at all, the case of a human running the command from an interactive terminal — does `--mine` fall back instead of refusing: it resolves your own identity via the same `gh api user` lookup unmarked worktrees already use, and lists the ones you created by hand under that identity. `--mine` still fails loudly there if that resolution itself fails (`gh` unauthenticated or unavailable) — rather than falling back further to "everything" or silently printing "none." So `--mine` now means "this orchestration's worktrees" inside any orchestration pane, or "this human's own worktrees" everywhere else — never a guess in between.
...
The warning is non-blocking: pressing `Enter` opens the tab as usual, and it exists to make the shared working tree explicit at the moment it starts to matter, so proceeding is a deliberate choice rather than a surprise. A **blank** Worktree slug submitted against a directory that **exactly matches** — same working directory, not a worktree carved off it — a live orchestration's own directory used to be refused outright (issue #489); [issue #521](https://github.com/prageethw/dot-agent-deck/issues/521) changed that refusal into automatic isolation instead, so a blank slug now auto-generates the next free `orchestrator-N` segment and opens into its own isolated clone with no manual retyping required. A **typed** Worktree slug follows the same isolation path either way (or, if a live orchestration already shares that root checkout's object store, an isolated clone) — unaffected by which case above applies.

This auto-isolation only applies when the two opens resolve to *different* workspace directories. If a live orchestration is already open under a given typed Worktree slug and you try to open a second one under the **identical** slug against the **identical** picked directory while the first is still running — even under a completely different Name — the second open is refused outright rather than isolated or silently shared: the two would otherwise resolve to the same on-disk workspace directory, and one orchestration per workspace directory is enforced at claim time. If the two orchestrations genuinely need to run at once, give them two **different** Worktree slugs; the identical-slug case still works fine sequentially (close the first, then reopen under the same slug against the same picked directory to resume its workspace).

There is one narrower case where the slug is identical but the *picked directory* is not: typing the same Worktree slug against two genuinely different subdirectories of the same project (e.g. `myrepo/team-a/proj` and `myrepo/team-b/proj`, both typing `features`) gives both a clean, identically-named workspace directory (`myrepo-features`) — the slug field intentionally no longer bakes the picked subdirectory into the name. This is **not** enforced at claim time — the two picks still claim two different resolved directories under the hood, so both claims succeed — it is caught only once the second one attempts to provision, and refused there permanently, not just while the first is running: reopening the second directory under the same slug is refused even after closing the first, since the two remain permanently distinguishable identities pointed at the one directory name. Pick a different Worktree slug for each subdirectory to avoid this.
