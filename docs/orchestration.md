...
The warning is non-blocking: pressing `Enter` opens the tab as usual, and it exists to make the shared working tree explicit at the moment it starts to matter, so proceeding is a deliberate choice rather than a surprise. A directory that **exactly matches** — same working directory, not a worktree carved off it — a live orchestration's own directory used to be refused outright (issue #489); [issue #521](https://github.com/prageethw/dot-agent-deck/issues/521) changed that refusal into automatic isolation instead, so opening a second orchestration there now provisions its own isolated clone with no manual retyping required, named from the typed orchestration Name (`<root-checkout-basename>-<sanitized Name>`).

This auto-isolation only applies when the two opens resolve to *different* workspace directories. If a live orchestration is already open under a given Name in a given directory and you try to open a second one that resolves to the **identical** on-disk workspace directory while the first is still running — either the identical Name, or a different Name whose sanitized segment happens to collide with it — the second open is refused outright rather than isolated or silently shared: one orchestration per workspace directory is enforced at claim time. If the two orchestrations genuinely need to run at once, give them two Names whose sanitized segments differ; the identical-Name case still works fine sequentially (close the first, then reopen under the same Name to resume its workspace).
