...
This auto-isolation only applies when the two opens resolve to *different* workspace directories. If a live orchestration is already open under a given typed Worktree slug and you try to open a second one under the **identical** slug while the first is still running — even under a completely different Name — the second open is refused outright rather than isolated or silently shared: the two would otherwise resolve to the same on-disk workspace directory, and one orchestration per workspace directory is enforced at claim time. If the two orchestrations genuinely need to run at once, give them two **different** Worktree slugs; the identical-slug case still works fine sequentially (close the first, then reopen under the same slug to resume its workspace).
