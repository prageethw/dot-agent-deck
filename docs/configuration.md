...
| `DOT_AGENT_DECK_LOG` | *(unset)* | When set, enables file-based tracing logs. Empty value or `1` writes to `<state dir>/deck.log` (e.g. `~/.local/state/dot-agent-deck/deck.log` on Linux, `%LOCALAPPDATA%\dot-agent-deck\deck.log` on Windows), created owner-only; any other value is treated as the target log file path. |
