...
When triage is on, a fire first ensures a seven-label vocabulary exists on the repo (`gh label create --force`, with a fixed colour and description per label, so re-running converges each label onto the deck's own values rather than a fresh random colour on every fire — `--force` means an existing label of that name has its colour/description **overwritten** to match, which is intended here since these seven are the deck's own vocabulary): `priority-high`, `priority-medium`, `priority-low`, `size-high`, `size-medium`, `size-low`, and `needs-triage`. The `in-progress` claim label (above) is handled differently: it is created only when the repo doesn't already have one, and its colour/description are left completely alone when it does, so a repo's own pre-existing `in-progress` label survives every dispatch run unchanged. It then appends a triage instruction to the prompt of **every issue it actually dispatches** — a skipped issue never sees it.
...
Dispatched tabs/cards persist until **you** close them — you stay in control of review, further iteration, or discarding the work. **Closing a dispatched tab/card removes its worktree** (`git worktree remove`); the **clone is preserved** (only the per-issue worktree goes away). Closing the tab does **not** release the issue: the `in-progress` label written on dispatch stays, and that label is itself a skip signal — subsequent fires keep skipping the issue until someone removes the label (`gh issue edit --remove-label in-progress <n>`) or the issue is closed.
