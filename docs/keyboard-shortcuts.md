...
| `toggle_orchestration_lock` | `Ctrl+e` | Toggle command-entry lock — while locked (the default), only the orchestrator pane accepts direct input, plus any pane that is waiting for input — on **orchestration tabs**, **command mode only**; one lock shared by every Orchestration tab, and in a pane the chord is ordinary input (end-of-line) for whatever is running there |
