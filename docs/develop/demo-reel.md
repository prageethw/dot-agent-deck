...
The demo reel is **not** built during the pre-PR gate. The casts are *recorded* there (a filtered local e2e run with `DOT_AGENT_DECK_RECORD=1`), but the reel itself is built, published, and linked only **after** the PR is open and its CI is green, and **before** merge — in the pre-merge window. Building it then means the link is posted once, against a PR that has already validated, and the changelog/PR-body update it adds gets one final quick CI pass before merge.
...
2. **Open the PR and let it validate.** `/prd-done` opens the PR; the orchestrator waits for CI to settle **green**. No reel yet.
