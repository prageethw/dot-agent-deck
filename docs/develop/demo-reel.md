...
The demo reel is **not** built during the pre-PR gate. The casts are *recorded* there (the e2e gate runs with `DOT_AGENT_DECK_RECORD=1`), but the reel itself is built, published, and linked only **after** the PR is open and its CI is green, and **before** merge — in the pre-merge window. Building it then means the link is posted once, against a PR that has already validated, and the changelog/PR-body update it adds gets one final quick CI pass before merge.
