...
`DAD_RELEASE_REPO` resolves more simply, in two steps rather than three (issue #398): a pre-set, valid value in the build environment, or the hardcoded default `vfarcic/dot-agent-deck` — there is no git-derived middle step, because the repo a checkout was cloned from doesn't tell you which GitHub repo's releases to poll.
...
```sh
DAD_VERSION=1.2.3 DAD_BUILD_ID=1.2.3-nixpkgs DAD_RELEASE_REPO=your-org/your-fork cargo build --release
```

Five properties of these paths are worth knowing:
...
- **An injected `DAD_RELEASE_REPO` must be a single-line `owner/name` GitHub slug**, where `owner` is ASCII alphanumerics and `-`, and `name` is ASCII alphanumerics plus `.`/`-`/`_` — GitHub's own slug alphabet. A rejected injection is *ignored with a `cargo:warning`* and resolution falls back to the default, exactly like the other two values, because a malformed or attacker-influenced slug must degrade to "poll the known-good feed" rather than a broken URL.
...
- **Changing an injected value invalidates the cached build** (`cargo:rerun-if-env-changed`), so a second build with a different `DAD_VERSION`, `DAD_BUILD_ID` or `DAD_RELEASE_REPO` does not quietly keep the first one.
