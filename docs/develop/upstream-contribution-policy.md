...
The test is **at the defect, not at the file**, and it takes two commands rather than a grep alone. `git fetch upstream` first, then locate the defect with `git grep -n '<the defective expression>' upstream/main -- <path>` and confirm the fork's copy of that expression is byte-identical. File-level divergence does **not** disqualify: fork #389's `&session.session_id[..11]` is byte-identical to upstream's inside a `src/ui.rs` the fork has diverged on by 6141 insertions. What disqualifies is divergence that changes the defective behaviour itself — the fork having already guarded it, or replaced the code path in a way that still leaves something here to fix — and that returns the change to the fork-first ordering above. A differing helper signature, or a fork-only wrapper around an otherwise identical call, is not divergence at the defect. One shape looks like a disqualifier and is not: where the fork has **already fixed** the defect and upstream still carries it (fork #303 / upstream [#577](https://github.com/vfarcic/dot-agent-deck/issues/577)), filing upstream is the only action available, and there is no fork behaviour left to protect.
