...
- **Yes — it is an improvement that merely happens to have been written here.** It goes upstream. A bugfix, a test-determinism fix, a genuine feature, a correction to an upstream doc. **Then ask one more question: does upstream still carry this defect, in code the fork has not already fixed?** If it does, it is filed upstream *at the point it is found* rather than fixed here and offered later — see ["An upstream-code defect is filed upstream, not offered later"](#an-upstream-code-defect-is-filed-upstream-not-offered-later) below. Everything else follows the fork-first ordering.

If the honest answer is "yes, but not yet" — that is a "yes" with an unscheduled second step, and the second step is the one that historically does not happen. See the backlog below for what that cost. On the fork-first path, "not yet" is the *expected* state between merging here and offering upstream, which is exactly why that gap has to be closed by a tracked issue rather than by intention.
...
A defect that **upstream still carries, in code the fork has not already fixed**, is filed upstream at the point it is found, by the orchestrator, rather than fixed here and offered afterwards.

Evidence is gathered **at the defect, not at the file**, with two greps. `git fetch upstream` first, then `git grep -nF '<the defective expression>' upstream/main -- <path>` and `git grep -nF '<the same literal>' HEAD -- <path>`. **`-F` is load-bearing**: `git grep` defaults to basic regex, so a literal such as `&session.session_id[..11]` (where `[..11]` is a character class) returns no hit on *either* side without it — and a no-hit reads as "the fork has diverged", which points the wrong way. Compare the `-n` line numbers rather than the exit status, since the same literal can occur more than once in a file. File-level divergence does **not** disqualify: fork #389's expression is byte-identical to upstream's inside a `src/ui.rs` the fork has diverged on by 6141 insertions.

The greps are **evidence, not the verdict.** CLAUDE.md rule 19 names the four shapes the nine precedents fall into — (1) the expression is byte-identical; (2) incidental fork divergence around an identical defect, such as a differing helper signature or a fork-only `let _ =` wrapper; (3) the fork has **already fixed** what upstream still carries, so there is no fork-side hit at all (fork #303 / upstream [#577](https://github.com/vfarcic/dot-agent-deck/issues/577), fork #276 / upstream [#578](https://github.com/vfarcic/dot-agent-deck/issues/578)); and (4) a defect of **absence**, where there is no expression to grep on either side (fork #366 / upstream [#580](https://github.com/vfarcic/dot-agent-deck/issues/580)). All four are filed upstream. What disqualifies is divergence that changes the defective behaviour itself *and still leaves something here to fix*, which returns the change to the fork-first ordering above. Anything that does not land cleanly in one of the four shapes is a judgement for the maintainer, not something to settle silently.
