...
`build_orchestrator_context()` composes four parts: the start role's `prompt_template` verbatim, then a hard-coded **"Workspace sync"** section (issue #760 Part B — instructs the orchestrator to check the mapped workspace is clean and caught up with the repository's default branch before doing anything else), then hard-coded **"Available agents"** and **"Delegation protocol"** sections, then a hard-coded **"Important"** section. Only the first part is configurable — the agent list is derived from the non-start roles' `name`/`description` fields, and the workspace-sync, delegation-protocol, and Important text are string literals in `src/orchestrator_context.rs`. Changing those requires a code change and a rebuild; no amount of TOML editing will move them.

Read `build_orchestrator_context()` and `prepare_orchestrator_prompt()` in `src/orchestrator_context.rs` to confirm any of the above rather than taking this page's word for it.
