...
`RemovalPolicy::KeepIfDirty` is why a dirty worktree survives: this sibling's name was chosen by an LLM, so closing must not destroy uncommitted work. PRD 236 unified issue-dispatch onto the same `KeepIfDirty` policy — it used to run `Force` instead, on the theory that its slot-reclaim model depended on the name actually being freed, but `worktree/reclaim/046` established that a kept, deck-marked `#120` worktree still reads as "claimed" to `dispatch_decision` and remains visible to `worktree reclaim`, so keeping it is safe too. Do **not** restore `Force` for issue-dispatch on the strength of this paragraph's old wording; `RemovalPolicy`'s own doc comment (`issue_dispatch_run.rs`) is the up-to-date reference. `Force` remains only for `dispatch.rs`'s own spawn-failure rollback.
