...
| `619d0f60` (pre-sync SHA; dropped 2026-08-15) | `PRD fork#197: the seed/prompt delivery state machine (#219)` | **DROPPED 2026-08-15** — the row's original justification read: *"the PRD's own header states this mechanically: `orchestration_awaiting_confirmation`, `CONFIRMATION_GRACE_PERIOD`, `prompt_text_confirms` and `AwaitingConfirmation` all have 0 occurrences on `upstream/main`, and upstream issue #424 (the confirmation defect) is still open there with no fix."* Both halves are now false. Upstream fixed #424 in this sync — `prompt_submission_evidence`/`SubmissionEvidence` (`src/ui.rs`) implement the identical contract under different names, reading the pane's `recent_events` journal rather than this commit's `SessionStatus::Thinking` LEVEL heuristic — and the fork's own symbol names now have **0** occurrences anywhere under `src/`, confirming the mechanism itself, not just the identifiers, is gone. See the lesson below: the "0 occurrences of our symbol names" test never proved what it claimed. Purely historical now. |
