...
**Outcome (2026-08-15): resolved, and the stack table above corrected — the mechanism is upstream's now.** The 2026-08-15 upstream sync replayed `754f0ba`/`8111027`/`d021c35`/`78068ed`/`cc4e3dc` against an `upstream/main` that already carried PR #404's implementation. `cc4e3dc` reduced to empty and was dropped; the other four survive only as small residues (PRD docs, one keybinding wire-up, one small `tab.rs` field, one changelog fragment) — see each row above for the byte-level evidence (identical function bodies, upstream's own test names surviving in the live tree). The fork's only remaining divergence on this surface is `da2f214b` (fork #346 / PR #350): the deliberate CLAUDE.md rule 9 decision to graduate the lock out of the `experimental` flag, which upstream's own `eff62561` still gates behind it. That is a policy decision, not a mechanism difference, and it is the one thing a future rebase should expect to keep resolving in the fork's favour under rule 24.
