...
Two smaller gaps belong in the same list. On **Windows** `process_table()` returns `None` unconditionally, so the signal never fires there at all — a documented gap, not a bug. And if Linux **sandboxing** (bwrap / PID namespaces) is ever enabled for the Bash tool, the payload runs in its own PID namespace and a host-side descendant walk may not enumerate it; neither predicate is known to survive that, and it needs its own measurement before anyone relies on it. Two more gaps belong alongside these, covered below in their own detail because — unlike the two above — both are things this fork has actually observed firing: **process-table budget exhaustion**, the one actually firing in this fork's CI, and the **per-row unreadable-candidate** case fork issue #160's per-row fail-safe closes.

A third gap belongs alongside them, and unlike the two above it is not hypothetical — it is the one actually firing in this fork's CI (fork issue #160). `process_table()` / `process_table_async()` bound the whole `ps` sample to `PS_SAMPLE_BUDGET` (`src/platform/proc/unix.rs`, 2s) and report `None` if it is exceeded. Under real machine contention `ps -A` can take longer than that — measured directly: `main` and PR #158, from the same base commit, both timed out around 2.3–2.5s; PR #159, from the same commit, did not. If the daemon's poll hits this while a pane's shell command is running, the signal simply does not update that tick — the pane keeps reading whatever it last read, stale, until a later sample succeeds. This is **transient and self-healing** rather than the permanent silence of failure mode 1 above, but consecutive attempts are not independent chances to succeed — they share the same load — so under *sustained* contention the effective retry period degrades to roughly the 500ms poll interval plus the 2s budget (~2.5s) and the condition can persist for as long as the contention does, which is exactly the condition under which a long agent run is most likely to be in progress.

Fork issue #160 gave the daemon's poll path (`process_table_async`, the async form) a way to tell this apart from the Windows case: it returns `Result<Vec<ProcessInfo>, ProcessTableOutcome>` rather than a bare `Option`, where `ProcessTableOutcome::Unsupported` is Windows's permanent "will never work" and `ProcessTableOutcome::Failed` is this transient, load-dependent miss. `run_shell_activity_monitor` (`src/daemon.rs`) logs a `tracing::warn!` on the first `Failed` sample after a success and then every 20th consecutive one after that — a rate limit, not a one-off, so a machine chronically hitting this stays visible without a line landing on every tick — distinct from the lower-level "process-table sample exceeded its budget" line the capture itself already emits. On `Unsupported` the monitor logs one `tracing::info!` naming the platform gap and ends the poll outright, since the condition is permanent and there is nothing to gain from waking every 500ms to learn it again. The synchronous `process_table()` used by every other caller (tests, `agent_pty.rs`'s one-shot lookups) still returns a plain `Option` — those callers are not part of a continuous poll where a stale reading can persist, so the distinction was not worth the wider churn there.

A fifth case is per-**row** rather than per-sample: `descendant_shell_activity()` (`src/platform/proc/scan.rs`) can fail to read one *candidate descendant's own* session id even when the whole `ps` sample succeeded and the root's own session id was readable — the same transient race any `getsid` call is subject to (the process exits between the sample and the call), just scoped to a single row instead of the whole table. Before fork issue #160's per-row fail-safe, an unreadable candidate's session id was `continue`d identically to a genuinely same-session one, so a candidate set where *every* row happened to be unreadable fell through to a confident `Some(false)` — a total false negative, the same failure direction as failure mode 1 above, just triggered by a transient read race on one tick instead of a permanent Claude Code behaviour change. The fix tracks "unreadable" separately from "same session": if at least one candidate's session id could not be read and no *other* candidate resolved a confirmed `Some(true)`, `descendant_shell_activity()` now reports `None` (unknown) instead of asserting idle, so `run_shell_activity_monitor` leaves the pane's last known status alone for that tick rather than asserting it idle. A confirmed-busy candidate elsewhere in the same set still short-circuits `Some(true)` immediately — this does not turn a genuinely busy pane's reading into "unconfirmed" by association with an unrelated unreadable row.
