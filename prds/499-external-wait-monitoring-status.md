...
**No product or daemon code change was made.** The gap was in *how panes waited*, not in the deck's status model — the existing `descendant_shell_activity` mechanism already reports `Working` correctly for the entire duration a pane is blocked inside one sustained foreground command, and that mechanism skips only panes with no hook session at all (`pane_hook_session_id`, `src/daemon.rs:1857`), applying no role filter, so it covers the orchestrator's own pane exactly like delegated-worker panes.
