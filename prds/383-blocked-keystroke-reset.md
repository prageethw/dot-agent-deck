...
> **Shipped via PRD [#373](https://github.com/vfarcic/dot-agent-deck/issues/373) as its M3, then REMOVED IN FULL by [#393](https://github.com/vfarcic/dot-agent-deck/issues/393) (`prds/393-command-mode-lock-and-lock-governed-focus.md`). The behaviour described below does not exist in the product.** This PRD was carved out of #373's M3 while #374 was still in flight. Once #374's command-entry lock landed (`936a603`), the user decided the blocked-keystroke reset should ship in #373's own PR rather than separately, so it was folded back in as #373's M3: the lock's drop site in `handle_key_event`'s `UiMode::PaneInput` arm stamped `Tab::Orchestration::last_role_pane_activity_at` before returning `Action::Continue`, pinned by `tabs/orchestration/019`. #393 then removed #373's M2 and M3 together — see that PRD's Status line, which records the same removal from the other end. Everything below is retained as the original design record; no implementation work remains here, and none should be started against it without first re-reading #393.
>
> *(Corrected 2026-08-14: this note previously read "Delivered via PRD #373 … pinned by `tabs/orchestration/019`", in the present tense, with no mention of #393's removal. Both halves had stopped being true. Measured against `origin/main` at `0005d739`: `tabs/orchestration/019` does not exist — `git grep 'tabs/orchestration/019' -- src tests` returns nothing — and `last_role_pane_activity_at` survives only in prose, appearing in `prds/373-*.md`, `prds/383-*.md`, `prds/393-*.md` and `docs/develop/fork-sync-workflow.md` with **zero** references anywhere in `src/`. `auto_focus_after_inactivity`, the timer the reset fed, is gone outright. So the note named a test as pinning behaviour that no longer exists, which is worse than naming no test at all: it invites a reader to go looking for coverage, find none, and conclude the test was lost rather than that the feature was deliberately deleted.)*

**Status**: **Superseded** — shipped via PRD #373's M3, then removed in full by [#393](https://github.com/vfarcic/dot-agent-deck/issues/393); the behaviour is not in the product and no work remains here. *(Status corrected 2026-08-14 — it previously read "Delivered via PRD #373 … no separate work remains", which stated the delivery and omitted the removal.)*
**Priority**: Medium
**Created**: 2026-08-05
