...
- [ ] `orchestration/seed/018` — L1: with the floor lowered, a landed-but-unconfirmed write **deterministically** produces a second write whose payload is the empty string. This is the assertion that currently cannot be made. **Not restored on the 2026-08-17 pass — see the restoration note at the top: the gate it targeted no longer exists, having been replaced by `unconfirmed_retry_delay` in `src/prompt_delivery.rs`.**
...
**2026-08-17 restoration status:** only `seed/017` and the M1 override seam are restored. `seed/015`, `/016`, `/018`, the CR-suppressing relay, and the corresponding catalog/module-doc corrections remain **not restored** — they belong to `tests/e2e_orchestration_seed_retry_real.rs` and the `orchestration_awaiting_confirmation` retry path, which is a separate, larger gap tracked informationally against issue #257 rather than attempted in this narrowed pass.
