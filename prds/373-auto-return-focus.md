...
- [ ] **M3 — Blocked-keystroke reset (deferred until #374 lands).** Extend the timer reset to also fire on a blocked keystroke attempt against a locked pane, once #374's lock exists to attempt one against; combined-interaction test per Technical Approach. Until it lands, see the known-limitation entry under Risks.
...
- **Known limitation until M3 lands: blocked keystrokes don't reset the timer.** M1+M2 ship without M3, so a keystroke blocked by #374's lock on a non-orchestrator role pane does NOT currently reset the inactivity clock — only M3 adds that, per the Decision above. A user actively typing at a locked pane therefore looks idle to M2 and can still be auto-snapped to the always-unlocked orchestrator after 30 seconds; their next keystrokes then land in the orchestrator instead of being dropped by the lock. This is a known, accepted limitation of shipping M1+M2 ahead of M3 — not a regression introduced by this work.
