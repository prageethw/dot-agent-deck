...
- [ ] M3 — The Nth-concurrent-orchestration gate: `provision_repo`'s clone-if-absent pattern generalized and reused by Model A at `Action::SpawnPane`'s provisioning point, gated on `live_orchestration_in_same_cwd`'s existing query (repurposed from cosmetic warning to correctness gate); the fail-open-vs-fail-closed question (Design step 4) resolved explicitly, not inherited silently. Model B (`dispatch.rs`) gets the equivalent query added, since it has none today.
