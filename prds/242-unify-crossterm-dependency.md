...
**Status**: In progress — the unification and the M4 safety follow-up have both landed; M5 (terminal-state regression pass) is still open. *(Status corrected 2026-08-14 on branch `fix/242-crossterm-doc-contract`, superseding the previous line: "In progress — the unification landed, the safety follow-up did not." That correction had itself found the unification already met by a Renovate bump (`79e2c218`, `crossterm = "=0.29.0"`), and had catalogued the resulting staleness: `src/ui.rs` still citing `crossterm-0.28.1` and `=0.28.1`, and `Cargo.toml`'s comment still asserting a "0.28.1 decoder" and that "the two coexist". **All of that is now fixed.** Every `0.28.1`/`0.28` version citation in `Cargo.toml` and `src/ui.rs` was corrected to `0.29.0`/`0.29` — five in the originally-catalogued locations plus two more the earlier correction did not enumerate (`src/ui.rs:5372` and `:27978`, both describing crossterm's process-global raw-mode state rather than citing a line number). Critically, this was a re-derivation, not a search-and-replace: every hand-read `parse.rs` line-number citation in `ui::ctrl_c0_byte`'s doc comment (`:35-91` through `:114-117`, plus `:303` and `:497`/`:540-566` for the CSI-u path) was independently checked against the vendored `crossterm-0.29.0` source and matched exactly — the citations were stale only in the version string, never in the line numbers, so `ui::ctrl_c0_byte` remains the exact inverse of the compiled decoder. `Cargo.toml`'s "the two coexist" NOTE was rewritten rather than version-bumped, since it was substantively false, not just outdated: `ratatui-crossterm` offers crossterm 0.28 and 0.29 as mutually-exclusive optional dependencies and defaults to 0.29, so `Cargo.lock` carries exactly one `crossterm` entry (confirmed via `cargo tree -i crossterm`), not two. This PRD's Problem Statement — "the build compiles two copies of crossterm" — remains obsolete, as the prior correction already noted; it describes the state before the Renovate bump, not the current one. Issue #242 stays open: M5's terminal-state lifecycle regression pass was not performed here.)*
**Priority**: Medium
**Created**: 2026-07-28
...
- [x] **M4**: Decoder contract re-verified against the compiled version; `ui::ctrl_c0_byte`'s documentation in `src/ui.rs` corrected, and `Cargo.toml`'s stale "0.28.1 decoder" / "the two coexist" comment (`:23-32`) corrected alongside it; round-trip test green and still RED against both known regressions — Done (branch `fix/242-crossterm-doc-contract`: all 7 stale `0.28.1`/`0.28` citations across `Cargo.toml` and `src/ui.rs` corrected to `0.29.0`/`0.29`; every hand-read `parse.rs` line-number citation independently re-derived against the vendored `crossterm-0.29.0` source rather than trusted, and confirmed unchanged; `Cargo.toml`'s "two coexist" NOTE rewritten to describe `ratatui-crossterm`'s default-to-0.29 feature and the single `Cargo.lock` entry; `keyevent_ctrl_c0_matches_crossterm_decoder` is unmodified and is CI's gate on this push)
