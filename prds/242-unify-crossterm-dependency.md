...
**Status**: In progress — the unification landed, the safety follow-up did not. *(Status corrected 2026-08-14: this line read "Not started" after the headline success criterion had already been met, by a Renovate dependency bump nobody connected back to this PRD. `Cargo.lock` shows exactly one crossterm, at `0.29.0`; `Cargo.toml:33` reads `crossterm = "=0.29.0"`. Landed in `79e2c218` — "fix(deps): update rust crate crossterm to v0.29.0 (#267)", authored by `renovate[bot]`, first released `v0.35.1`, two days after this PRD was written. So **M1/M2/M3's mechanical part is done** — the graph resolves to one version, and Option B (move to 0.29) is the option that landed. **M4 did not happen and its absence is now a live defect, not just an open milestone**: `src/ui.rs` still documents crossterm 0.28.1, still cites 0.28.1 `parse.rs` line numbers, and still asserts that `Cargo.toml` pins the direct dependency to `=0.28.1` — directly contradicted by the actual pin, `crossterm = "=0.29.0"` (`Cargo.toml:33`). **`Cargo.toml` itself is stale in exactly the same way**: its comment (`:23-32`), directly above that pin, still documents "crossterm 0.28.1's decoder" behavior and still asserts "the two coexist" — a claim that stopped being true the moment the graph unified to one version. `keyevent_ctrl_c0_matches_crossterm_decoder` still exists and, being a live round-trip against whatever decoder is compiled, has very likely been passing against 0.29 ever since the bump — it is the human-readable contract that decayed, not (as far as this correction can tell) the behavior. This PRD's own Problem Statement — "the build compiles two copies of crossterm" — is now factually obsolete and should be read as historical framing for M4's remaining work, not as the current state. M5's terminal-state lifecycle regression pass was never explicitly performed for this bump either. Issue #242 stays open.)*
**Priority**: Medium
**Created**: 2026-07-28
...
- [ ] **M4**: Decoder contract re-verified against the compiled version; `ui::ctrl_c0_byte`'s documentation in `src/ui.rs` corrected, and `Cargo.toml`'s stale "0.28.1 decoder" / "the two coexist" comment (`:23-32`) corrected alongside it; round-trip test green and still RED against both known regressions
