...
- [x] M6 — Offer upstream per rule 19 once merged here (this is a general `dot-agent-deck` limitation, not fork-specific divergence) — comment on upstream #205 with the shipped approach and a link, since #205's own thread is where the "forward vs. signal" design choice was discussed. Done: reimplemented against `upstream/main`'s current code (not a cherry-pick — `agent_pty.rs`/`state.rs` had diverged 1300+/1800+ lines, and upstream carries a commission ledger, issue #448, the fork doesn't have), opened as [vfarcic/dot-agent-deck#602](https://github.com/vfarcic/dot-agent-deck/pull/602), and commented on [upstream #205](https://github.com/vfarcic/dot-agent-deck/issues/205#issuecomment-5337035909) linking it.
