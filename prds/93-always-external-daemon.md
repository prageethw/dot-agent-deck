...
**Status**: **Complete.** *(Status corrected 2026-08-14: this line read "Phase 4 in flight" for some time after the work had actually shipped. Merged as `48b9180d` — `PRD #93: always-external daemon (unify local and remote architecture) (#102)` — via [vfarcic/dot-agent-deck#102](https://github.com/vfarcic/dot-agent-deck/pull/102), first released in `v0.25.0`; M4.3's changelog fragment (`changelog.d/93.feature.md`) landed in that same squash merge; the success-criterion grep — `grep -r "PaneBackend::Pty\|ControllerMode::LocalDeck" src/ tests/` — returns zero matches on `main`.)*
**Priority**: Medium
**Created**: 2026-05-17
...
- [x] **M4.3** — Changelog fragment via `dot-ai-changelog-fragment`. Focus on user-visible behavior change (daemon now persistent across deck restarts; agents survive). — Done (`changelog.d/93.feature.md`, squashed into `48b9180d`; the pre-squash commit `913b5b22` is no longer reachable from any published ref).
- [x] **M4.4** — PR, review, audit, merge, release. — Done: merged as `48b9180d` via [vfarcic/dot-agent-deck#102](https://github.com/vfarcic/dot-agent-deck/pull/102), first released in `v0.25.0`.
