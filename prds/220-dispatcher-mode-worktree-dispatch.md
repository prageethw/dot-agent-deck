...
**Status**: **Phase 1 landed; Phase 2 (return-edge routing) deliberately deferred; Phases 3, 4 and 5 partial** — M2.0/M2.1/M3.1/M4.0/M4.2/M5.1 unchecked, notably **M5.1 (rule-12 cross-version check, not run)**. Merged via https://github.com/vfarcic/dot-agent-deck/pull/232 (`feat: PRD #220 — dispatch CLI verb + dispatcher mode`, merge commit `7616d2f537b7ffc0657e508f52ca971fc5710627`); the dispatcher mode graduated out of the `experimental` flag in `9248e932d40493304676cd1655b19e970677b8e2`, first released in **v0.35.9**. Corroborated by `changelog.d/220.feature.md`. The Phase 2 deferral is a recorded decision (maintainer call, 2026-07-28; full rationale under Phase 2 below), not an omission — but PRD 220 stays open until Phase 2 gets its own follow-up issue, as stated at M5.2 and in the Phase 2 tracking blockquote. *(Status corrected 2026-08-14: this line previously read "Phases 1, 3 and 5 landed", which was false for both Phase 3 and Phase 5 — Phase 3's M3.1 is only PARTIAL (verified for Claude only) and was omitted from the "still open" list entirely, and Phase 5's M5.1, the rule-12 cross-version contract check, has not been run even though the same sentence called Phase 5 "landed". The milestone checkboxes below already recorded the per-item state accurately; this correction brings the top-level Status line back in line with them, including the previously-omitted M3.1.)*
**Priority**: Medium
**Created**: 2026-07-19
