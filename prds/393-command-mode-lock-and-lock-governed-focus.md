...
- [ ] **M7 — Upstream contribution — DROPPED (not planned).** See "Upstream" below. This is a *net-new feature proposal*, not a port of this branch's diff, and it must not gate the fork work. **Gated on two things that are not code**: answering the three questions vfarcic left on [#369](https://github.com/vfarcic/dot-agent-deck/issues/369), and accumulating real usage evidence on the fork — he explicitly said that is the best possible argument for bringing it upstream. **Dropped deliberately at close-out**: neither gate had been met, so there was never anything to start, and the box is left unticked to record a decision rather than an oversight. The "Upstream" section below stays as the standing record of what such a proposal would have to say, should anyone ever want to revive it.
...
**M7 (upstream contribution) was dropped as not-planned.** Both of its gates were non-code and neither had been met: the three questions on [#369](https://github.com/vfarcic/dot-agent-deck/issues/369) and a period of real fork usage to report on. The "Upstream" section stays as the standing record of what a revived proposal would have to lead with.
