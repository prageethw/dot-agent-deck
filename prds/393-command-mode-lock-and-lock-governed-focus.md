...
**Status**: **M1–M6 implemented and CI-green** on branch `prd-393-command-mode-lock-and-lock-governed-focus` (worktree `../dot-agent-deck-prd-393`, cut from `origin/main` at `7747e6a`), open as fork PR [#51](https://github.com/prageethw/dot-agent-deck/pull/51). M1–M5 verified green at `82f3aeb` — CI run `31141556876`: fast tier green on Linux/macOS/Windows, `cargo nextest run --features e2e` **3614 passed, 0 skipped**. M6 (docs, changelog, PRD amendments, status-message wording) is this update. **One item still owed before merge: the rule 12 cross-version manual test** — see that section. M7 remains optional and non-blocking.
...
**Is the cross-version manual test required? Yes, and it is STILL OWED — it has not been run.** Rule 12 triggers on daemon/protocol/**orchestration**/hooks, and this PRD touches orchestration behaviour, so the check is owed even though the three answers above are all "no contract change". It needs two builds and a live daemon with an agent under it, which is a human/orchestrator step rather than a coder one, and it is the sole reason M6's checkbox is unticked. The procedure: branch TUI against the previous release's daemon with an agent under it, confirm a delegate still routes and hooks still arrive. Note #387's procedural finding, which applies directly: you cannot test a version boundary by pointing a newer TUI at an older daemon, because the build-version handshake silently restarts a mismatched daemon when no agents are running. The procedure that works is old binary serves → its own TUI opens an orchestration and **detaches** → new TUI raises the mismatch warning → decline the restart.
