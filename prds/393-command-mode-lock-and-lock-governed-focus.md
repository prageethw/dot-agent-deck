...
That reading is coherent, but it is a genuine narrowing of the rationale as stated, and it should be an explicit choice rather than something discovered later. **See Open Question 4 — this is live and may change decision 4 before M3 is implemented.** M1 and M2 are unaffected either way and proceed regardless.
...
- [ ] **M7 — Upstream contribution (optional, non-blocking, not startable from this branch).** See "Upstream" below. This is a *net-new feature proposal*, not a port of this branch's diff, and it must not gate the fork work.
...
4. **LIVE — does decision 4's carve-out survive the rationale?** Raised the moment the "Why the lock exists at all" rationale was written down, because the two are in tension: the rationale says the reflex to answer an agent's prompt on the spot is exactly what the lock should interrupt, and decision 4 makes that prompt answerable with no pause. Three ways out, and the choice is the user's:
   - **Keep decision 4 as-is**, on the narrower reading that answering a *solicited* question from an already-blocked agent is not an interruption. Usability stays; the rationale is narrowed in writing so the inconsistency is deliberate rather than accidental.
   - **Drop decision 4** — no carve-out, a locked pane is locked even when its agent is asking. Maximally faithful to the rationale, and it makes answering any worker prompt a deliberate `Ctrl+D`, `Ctrl+E`. Costs the usability win and reopens Problem 3.
   - **Route prompts through the orchestrator instead.** Most faithful to "one workflow, one coordinator" and by far the largest change — a new mechanism, not a gate tweak. Out of scope here; would be its own PRD.

   **Blocks M3 only.** M1 (chord scoping) and M2 (deck-global state) are unaffected and proceed. `orchestration/lock/009`/`010` are the tests that would change.
