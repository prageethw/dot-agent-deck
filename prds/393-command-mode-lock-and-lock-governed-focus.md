...
**Two contention episodes: a test failed 3/3 and then passed on a straight re-run of the identical commit.** Twice during implementation a failure looked deterministic — three consecutive failures, no intervening change — and then went green on a re-run of the same SHA with nothing edited. The lesson is not "flaky, ignore it": it is that **repeat-count is not evidence of determinism on this suite** when the machine is loaded, because the L2 tier's timing budgets shrink under contention rather than failing randomly. Both are consistent with the known-noisy set tracked in **issue #395** and with CLAUDE.md rule 5's fork-only addendum, which exists precisely because running the tier locally starved this machine. The practical rule that came out of it: before treating an L2 failure as a real defect, re-run the identical commit once on an unloaded machine or read CI's result — and read the `--retries 2` output for `FLAKY` / `TRY 1 FAIL` rather than trusting the summary line.
