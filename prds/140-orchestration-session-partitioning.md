...
2. **Guard + steer (the product stance).** When a user opens an orchestration in a directory that *already* has a live orchestration, warn them — non-blocking — that same-directory orchestrations share coordination files (`.dot-agent-deck/*-{role}.md`) and one working tree, and point them at a worktree (`/worktree-prd`) as the isolated alternative. The daemon already knows every live orchestration's cwd (via `pane_orchestration_map` / `orchestrator_pane_ids`), so detection is a lookup at new-orchestration spawn time. The warning informs power users without forbidding them; it makes layers 2 and 3 explicit at the exact moment they become a risk.
   ...
   > **Superseded 2026-08-09 by fork PRD [fork#166](https://github.com/prageethw/dot-agent-deck/issues/166).** Prong 2's non-blocking stance has been **deliberately reversed** on this fork: a second orchestration in a directory that already hosts a live one is now *blocked* until a distinct worktree slug is given. The judgement above was reasonable and has since been tested by events — the warning was in place and the collision recurred anyway (fork #74 twice, and again on 2026-08-09 when three worker reports collided at one `work-done` path). A hint that is routinely stepped past is not informing anyone. This note exists so the reversal is visible from here and is not later "fixed" back as an oversight. Prongs 1 and 3 are unaffected.
