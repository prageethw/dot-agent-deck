...
| `scheduler/dispatch/029` | create | Orchestration dispatch names the **orchestration** in the claim, not the scheduled task. |
| `scheduler/dispatch/030` | create | `gh api user` fails: label and comment still land, no assignee, dispatch **not** reported failed. **GREEN today** — see below. |
