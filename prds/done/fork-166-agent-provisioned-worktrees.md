...
1. Every orchestration starts from the deck's own checkout (`main`). **Intended** — not something to prevent.
   ...
   ***Narrowed 2026-08-18, corrected 2026-08-20 — PRD fork#325 M1-M5, all shipped.*** This step's "every orchestration" is no longer unconditional, and the dividing line is **not** human-vs-automated — that was fork#325 M1's originally-approved trigger, and it was corrected the same day it was approved, before any code shipped against it, once implementation-prep investigation found it matched no actual incident shape or code signal. The line this paragraph originally drew (a human's own interactive TUI session is unaffected; concurrent/automated provisioning isolates) is **not what shipped** and should not be relied on.

   **What actually shipped**: the trigger is **Nth-concurrent**, full stop — whether a live orchestration already shares the target root checkout's git object store, regardless of whether either orchestration is human-driven or automated. The **1st** orchestration against any root checkout is unaffected, exactly as this step originally described. The **2nd or later**, while an earlier one is still live, is isolated into its own fresh clone instead — but only when it names an explicit Worktree slug. **Corrected again 2026-08-20** following [issue #489](https://github.com/prageethw/dot-agent-deck/issues/489)/[PR #506](https://github.com/prageethw/dot-agent-deck/pull/506): a **blank**-slug spawn — the default, unnamed-slug path most orchestrations use — whose cwd exactly matches a live orchestration's own cwd is **refused outright**, not isolated; a blank slug has nothing to isolate into, so the user is asked to re-submit with a typed slug instead. A human opening a second interactive TUI session against a checkout they already have one open against therefore hits that refusal, not silent isolation, unless they type a Worktree slug — the opposite of what this note previously said. Shipped for the TUI's own `Action::SpawnPane` path (Model A, PR #481, the blank-slug refusal arm added in PR #506) and for the `dispatch` CLI (Model B, PR #504, which always supplies a slug and so only ever isolates, never hits the refusal); reproduced against the actual multi-orchestration incident shape and proven to close it (M5, `orchestration/worktree/016`); the refusal arm itself is pinned by `orchestration/worktree/017`. Full reasoning, the ownership-attribution mechanism's own history, and what remains open: [`docs/develop/shared-clone-architecture.md`](../../docs/develop/shared-clone-architecture.md) and `prds/fork-325-shared-clone-architecture.md`.
