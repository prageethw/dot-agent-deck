...
1. Every orchestration starts from the deck's own checkout (`main`). **Intended** — not something to prevent.
   ...
   **What actually shipped**: the trigger is **Nth-concurrent**, full stop — whether a live orchestration already shares the target root checkout's git object store, regardless of whether either orchestration is human-driven or automated. The **1st** orchestration against any root checkout is unaffected, exactly as this step originally described. The **2nd or later**, while an earlier one is still live, is isolated into its own fresh clone instead — a human opening a second interactive TUI session against a checkout they already have one open against gets isolated exactly as a scheduled/automated 2nd would. Shipped for the TUI's own `Action::SpawnPane` path (Model A, PR #481) and for the `dispatch` CLI (Model B, PR #504); reproduced against the actual multi-orchestration incident shape and proven to close it (M5, `orchestration/worktree/016`). Full reasoning, the ownership-attribution mechanism's own history, and what remains open: [`docs/develop/shared-clone-architecture.md`](../../docs/develop/shared-clone-architecture.md) and `prds/fork-325-shared-clone-architecture.md`.
