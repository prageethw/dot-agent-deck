...
- **Add `orchestration_id: Option<String>` to `TabMembership::Orchestration`** (`src/agent_pty.rs:231`) with `#[serde(default, skip_serializing_if = "Option::is_none")]` so older peers round-trip cleanly. A per-tab instance token, shared by every role pane in one orchestration tab, opaque to the daemon; distinct from `name` (config identity), `orchestration_cwd` (directory disambiguator), and `display_title` (**presentation-only when this PRD shipped it — updated by fork#192, see below**).

**Update (fork#192, M1.1): `display_title` is no longer presentation-only.** This PRD documented it as decoration — "presentation-only" above, and `src/agent_pty.rs`'s own field doc said "purely cosmetic" — because at the time nothing read it for anything but the tab label. fork#192's M1.0 changed that: the new-pane form's uniqueness/suggestion check (`live_orchestration_cwds_and_titles`) now reads every live orchestration's `display_title`, applying the same `filter(nonempty).unwrap_or(name)` fallback the tab-title computation uses, to decide whether a submitted name collides and must be refused. So `display_title` now feeds a correctness decision (can a user open a second orchestration under a name that is already taken), not only a label. It is still not a ROUTING identity — `orchestration_id` above and `name` remain what `lookup_orchestration_role`/delegate routing use, and that split (PRD #107) is unchanged. The wire shape is unchanged (`Option<String>`, `skip_serializing_if`, so older peers still round-trip); only the field's meaning grew a second consumer. See `changelog.d/192.breaking.md` for the rule-12 write-up and `src/agent_pty.rs`'s `display_title` doc comment and `validate_orchestration_surface`/`validate_tab_membership` for the re-decided (not inherited) validation posture that follows from this.
