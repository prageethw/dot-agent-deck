...
**Status** *(corrected 2026-08-16)*: **Merged into the fork — M1 through M5 all complete.** PR [#348](https://github.com/prageethw/dot-agent-deck/pull/348) merged 2026-08-15 as `bf24d171`, first released in **v0.38.2**. M1 and M2 landed as described below; **M3** landed as `8e9bc9b6` (the `cargo xtask work-type-check` gate, R0, wired into `ci.yml` with `fetch-depth: 0`), **M4** as `564f5fa2` (RED) and `7cfa301a` (GREEN, rules R1–R4), and **M5** as `7cfa301a` (the `chore` label, `TYPE_LABELS`); `reviewer` and `auditor` findings were resolved in `c99bbe00`. Verifiable on `main` today: `xtask/linkage-check/src/work_type.rs`, `docs/develop/work-types.md`, `TYPE_LABELS` in `src/issue_dispatch.rs`, and the `work-type-check` step in `.github/workflows/ci.yml`. Step 9 — the rule 19 upstream-offer issue for **M1/M3/M4 only** — is discharged as [fork #372](https://github.com/prageethw/dot-agent-deck/issues/372). That scoping follows the "Fork-only?" paragraph below for M1/M3/M4; note the paragraph predates the M5 decision and is silent on it, so **#372 is where the call to keep M5 fork-side is actually made**. The worktree `../dot-agent-deck-worktype` and the *local* branch were removed on 2026-08-16; the **remote** branch `origin/feat/340-work-type-vocabulary` (`bc9ab6f0`, PR #348's head) still exists and is left in place deliberately, since GitHub keeps it as the merged PR's record.

*(This line read "M1 and M2 complete; M3–M5 unstarted" until 2026-08-16 — three milestones and a merge out of date. It was written before the M3 round started and never revised as the PRD finished, so the file understated shipped work rather than overstating it: a queue sweep reading only this line would have re-started M3 against a gate already live in CI.)*

**Sibling**: [fork #339](https://github.com/prageethw/dot-agent-deck/issues/339) — the other PRD parked in `12ccb6df`, merged as PR [#342](https://github.com/prageethw/dot-agent-deck/pull/342) (`5d2a9205`, GitHub's merge commit; the content lives on `main` as the replay `a9ca72cc` — see that PRD's Status line for why both SHAs are correct). Its file was renamed to `prds/fork-339-agent-type-badge-toggle.md` on that branch, so **M4's optional rename is already done** — it is no longer the 1-of-129 `prds/` file lacking an issue number, and R4 has nothing left to fix up there.
...
**This checklist is spent — every step below was executed and the PRD merged (see Status). It is retained as the execution record, not as work to do.**
