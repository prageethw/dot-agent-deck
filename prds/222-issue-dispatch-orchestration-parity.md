...
**Status**: In progress — two narrow slices shipped under PRD #220; M1's unification proper, M2, and M4–M7 are genuinely unstarted. *(Status corrected 2026-08-14: this line read "Not started" while two pieces of this PRD's own scope had already landed, under PRD #220's issue number rather than this one. **Role-named dispatched panes shipped**: each role's card is labelled by role name, not task name (`src/spawn.rs:530-580`, `orchestration/dispatch/002`) — this is **M1's** "Role names correct live" clause, not M3 (M3 is purely reconnect: "Role names survive reconnect"). **The shared orchestrator-context composition shipped**: `src/orchestrator_context.rs` now exists, explicitly documented as "shared by BOTH spawn paths (PRD #220 / #222)". It answers this PRD's own Open Question — "does the scheduler path inject `prompt_template` today?" — with a documented **no, deliberately**: `src/spawn.rs:105-117` states the `compose_orchestrator_context` flag is `true` only for PRD #220 `dispatch`, "Deliberately NOT enabled for the scheduler (#127) or issue-dispatch (#120), even though both have the identical defect... That is #222's job to do deliberately, with those tests updated as part of it." **That answer covers only the orchestrator's own template, though**: `build_orchestrator_context` (`src/orchestrator_context.rs:25-30`) composes just the start role's `prompt_template`; every worker role gets its `prompt_template` at delegate time via `handle_delegate` (`src/project_config.rs:251-252`), path-agnostically, regardless of which spawn path opened the orchestration. Since the PRD says this distinction determines "whether M1 is a fix or a regression-guard", the Open Question is answered for the orchestrator role only, not for every role. So M1's ordering half has shrunk to flipping one boolean plus updating the three #120/#127 e2e tests that assert the prompt text arriving verbatim — a much smaller task than the milestone as written implies. **M1's unification proper is not shipped**: `SpawnTarget::Orchestration` is still alive in `src/spawn.rs` (constructed at `:274`, `:315`, `:352`), and `open_orchestration_tab` (`src/tab.rs:743`) still has no daemon-side caller — the two implementations this PRD exists to converge have not converged. **M2 (unattended startup) is not shipped**: no seeded trust/onboarding/`--allowedTools` harness pattern exists on the production dispatch path. **M4 (authoring UX) is not shipped**: `src/ui.rs:493` still hardcodes the example repo `vfarcic/dot-ai`; no structured `sort` field exists. **M3's reconnect half is unverified** — the live-surface naming is confirmed, but no test asserts the detach/reconnect rebuild path specifically preserves role names. M5–M7 (tests, docs, graduation handoff) are untouched. Issue #222 stays open. Note: this correction should not be confused with **fork issue #222**, an unrelated truncation-collision matter referenced elsewhere in `src/issue_dispatch.rs`.)*
**Priority**: High
**Created**: 2026-07-25
