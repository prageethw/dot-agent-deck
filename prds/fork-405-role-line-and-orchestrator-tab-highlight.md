...
5. **The role row is hidden while idle art is showing** (Spacious only), because the overlay `Clear`s the whole inner area (`src/ui.rs:19696-19711`). Identity survives in the title today and will not afterwards. Consistent with `Dir:`/`Prmt:`, which are already hidden, but new for identity.
