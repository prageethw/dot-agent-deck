...
- [ ] Issue #358 closes with the actual mechanism fixed, not a rescope. **Blocked on M4 — see below.** M1/M2's mechanism does not actually close this issue; M4 is required before this box can be checked honestly.
...
- [ ] Make the generation survive a daemon restart, or make it otherwise impossible for a pre-restart and post-restart registration to compare equal. Two sketched approaches (pick one, or another — this is a real design decision for the implementing worker, not a foregone conclusion):
  - **Persist** `pane_registration_generation` (or enough of it) to disk, so a restart resumes rather than resets it.
  - **Compound key**: pair the generation with a fresh per-daemon-boot identifier (a UUID or high-resolution boot timestamp minted once at daemon startup) — a pre-restart signal's boot id can never match a post-restart pane's, which catches exactly the case an in-memory counter alone cannot, with no persistence/IO required.
- [ ] Add at least one fast-tier test (CI-executing, not credential-gated) that exercises the REAL chain — calls `reserve_registration_generation`, then the actual env-injection path (or as close to it as a fast test can reach), then `confirm_orchestration_role`, then `handle_work_done` — rather than hand-writing the same literal on both sides of the comparison. This is what closes reviewer's F1 / auditor's B3.
- [ ] Fix the watchdog-cancellation-before-refusal ordering (auditor B2) so a refused signal does not silently disarm the current tenant's own silence-watch/outstanding-delegation tracking — at minimum for the mixed-version case this PRD's own fail-closed design makes newly common. Coordinate with issue #444's scope (the general ordering issue) rather than duplicating it; extending #444's fix to cover this, or landing a narrower fix here with a comment cross-referencing #444, are both acceptable — implementer's call, documented.
- [ ] Re-verify PRD Success Criterion 1 (the actual repro test) against the NEW mechanism, since M1's existing pinning test (`handle_work_done_refuses_a_stale_cross_orchestration_signal_after_pane_reuse`) hand-registers the same pane_id twice within one test — a sequence reviewer/auditor both confirmed no production path produces. Add or adapt a test that reproduces the cross-restart scenario specifically (reset/replace whatever in-memory state a restart would reset, confirm the OLD mechanism would have let it through, confirm the NEW mechanism catches it).
- [ ] Once M4 lands, M3's "close honestly" box can be checked for real.
