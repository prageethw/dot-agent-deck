...
- Unifying `spawn.rs`'s `next_pane_id`/`PANE_COUNTER` with the new minter — deferred from M2 after review (it would have been in-scope; see M1 Decision 4), tracked as [#430](https://github.com/prageethw/dot-agent-deck/issues/430). Success Criterion 3 below is qualified accordingly: it holds for `pane-`-origin panes, not yet for `sched-`-origin ones.
...
- **`src/spawn.rs:1961-1977`** (`next_pane_id`) — its `PANE_COUNTER: AtomicU64` (`spawn.rs:82`) has the exact restart-reset defect named in Q2; ideally M2 would unify this with whichever new minting function the `StartAgent` path uses rather than leaving two divergent daemon-side schemes, so there is one collision-resistant minter for every spawn origin (headless and TUI-attached alike) — **deferred from M2 after review; tracked as [#430](https://github.com/prageethw/dot-agent-deck/issues/430)**.
